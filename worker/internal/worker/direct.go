...
type DirectBackendConfig struct {
	...
	OzPath          string // Path to the neodev CLI binary. If empty, looks up "oz" in PATH.
	SetupCommand    string
...
type DirectBackend struct {
	...
	ozPath string // resolved path to the neodev CLI
}

// NewDirectBackend creates a new direct backend, verifying the neodev CLI is available.
func NewDirectBackend(ctx context.Context, config DirectBackendConfig) (*DirectBackend, error) {
	...
	if ozPath == "" {
		...
		if err != nil {
			return nil, fmt.Errorf("neodev CLI not found in PATH: %w", err)
		}
	}
	log.Infof(ctx, "Using neodev CLI at: %s", ozPath)
...
func (b *DirectBackend) ExecuteTask(ctx context.Context, params *TaskParams) ExecuteResult {
	...
	// 6. Invoke neodev CLI with base args.
	...
	log.Infof(ctx, "Running neodev agent in workspace %s", workspaceDir)
	...
	if err := cmd.Run(); err != nil {
		...
		return executeError(newBackendFailure(metrics.TaskFailurePhaseBackend, metrics.TaskFailureReasonAgentInvocation, fmt.Errorf("neodev agent exited with error: %w", err)))
	}
