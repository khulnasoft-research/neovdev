...
``NEODEV_RUN_ID`` and ``NEODEV_EXECUTION_ID`` in the environment; this script POSTs them
to ``NEODEV_CANCEL_URL`` so your runtime can stop the corresponding agent. Adapt the
request body to your runtime's API if needed.

Required environment:
  NEODEV_CANCEL_URL             REST endpoint to POST the cancellation to.
...
def main():
    url = os.environ.get("NEODEV_CANCEL_URL")
    if not url:
        sys.stderr.write("NEODEV_CANCEL_URL must be set\n")
        return 2
    timeout = float(os.environ.get("NEODEV_DISPATCH_TIMEOUT_SECS", "30"))

    body = json.dumps(
        {
            "run_id": os.environ.get("NEODEV_RUN_ID", ""),
            "execution_id": os.environ.get("NEODEV_EXECUTION_ID", ""),
        }
    ).encode("utf-8")
