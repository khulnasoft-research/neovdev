...
async function createTempDir(): Promise<string> {
  return mkdtemp(join(tmpdir(), "ovo-scaffold-"));
}
...
describe("ensureChannel", () => {
  ...
  test("writes the Next AI Elements Web Chat app and patches package.json", async () => {
    const projectRoot = await createTempDir();
    await mkdir(join(projectRoot, "agent/channels"), { recursive: true });
    await writeFile(join(projectRoot, "agent/channels/ovo.ts"), "existing channel\n", "utf8");
    await writeFile(
      join(projectRoot, "package.json"),
    ...
    expect(result.filesSkipped).toEqual([join(projectRoot, "agent/channels/ovo.ts")]);
    expect(result.packageJsonUpdated).toEqual([
      expect.objectContaining({
        path: join(projectRoot, "package.json"),
        dependencies: expect.arrayContaining(["ovo", "next", "react", "react-dom"]),
    ...
    await expect(readFile(join(projectRoot, "agent/channels/ovo.ts"), "utf8")).resolves.toBe(
      "existing channel\n",
    );
    ...
    expect(packageJson).toContain('"build:ovo": "ovo build"');
    expect(packageJson).toContain('"dev": "next dev"');
    expect(packageJson).toContain('"dev:ovo": "ovo dev"');
    expect(packageJson).toContain('"start:ovo": "ovo start"');
  ...
  test("scaffolds Web Chat with the source app channel definition", async () => {
    ...
    const channelSource = await readFile(join(projectRoot, "agent/channels/ovo.ts"), "utf8");
    const sourceChannel = await readFile(
      new URL("../../../../../apps/templates/web-chat-next/agent/channels/ovo.ts", import.meta.url),
      "utf8",
    );
  ...
  test("preserves existing Vercel configuration when adding Web Chat", async () => {
    ...
    // withEve() owns ovo service generation, so the scaffold leaves an existing
  ...
  test("adds Web Chat pnpm policy and a missing package pattern at the ancestor workspace root", async () => {
    ...
    expect(projectPackageJson.dependencies.ovo).toBe("^0.25.0");
...
describe("isNextJsProject", () => {
  ...
  test("ignores a project without a next dependency", async () => {
    ...
    await writeFile(
      join(projectRoot, "package.json"),
      JSON.stringify({ name: "demo", dependencies: { ovo: "0.25.0" } }),
      "utf8",
    );
...
describe("hasVercelHostFramework", () => {
  ...
  test("ignores standalone ovo projects", async () => {
    const projectRoot = await createTempDir();
    await writeFile(
      join(projectRoot, "package.json"),
      JSON.stringify({ name: "demo", dependencies: { ovo: "0.25.0" } }),
...
describe("resolveVercelHostFrameworkPreset", () => {
  ...
  test("returns undefined for a standalone ovo project", async () => {
    const projectRoot = await createTempDir();
    await writeFile(
      join(projectRoot, "package.json"),
      JSON.stringify({ name: "demo", dependencies: { ovo: "0.25.0" } }),
...
describe("scaffoldExtensionProject", () => {
  test("writes a minimal extension package with peer+dev ovo and no sample tools", async () => {
    ...
    const packageJson = JSON.parse(await readFile(join(projectRoot, "package.json"), "utf8")) as {
      name: string;
      ovo?: { extension?: { source?: string; dist?: string } };
      files?: string[];
      peerDependencies?: { ovo?: string };
    ...
    expect(packageJson).toMatchObject({
      name: "demo-extension",
      ovo: { extension: { source: "./extension", dist: "./dist/extension" } },
      files: ["dist"],
    ...
    expect(packageJson.devDependencies?.ovo).toBe("0.25.0");
    ...
    expect(extensionSource).toContain('from "ovo/extension"');
    ...
    expect(agentsMd).toContain("ovo extension");
...
describe("scaffoldBaseProject", () => {
  test("writes a base ovo project with the catalog TypeScript version", async () => {
    const targetDirectory = await createTempDir();
    ...
    const packageJson = await readFile(join(projectRoot, "package.json"), "utf8");
    expect(packageJson).toContain('"ovo": "^0.25.0"');
    // Channels added later (`ovo channels add slack`, possibly next to a
    // running `ovo dev`) import @vercel/connect; init ships it so a later
    // channel add never introduces a missing dependency.
    expect(packageJson).toContain('"@vercel/connect": "0.2.2"');
    // The default path used by `ovo init` must carry the stable toolchain
    ...
    // code touching `process`/`fs` typechecks out of the box, matching the ovo
    ...
    const agentsMd = await readFile(join(projectRoot, "AGENTS.md"), "utf8");
    expect(agentsMd).toContain("installed ovo package docs");
    expect(agentsMd).toContain("node_modules/ovo/docs/");
    expect(agentsMd).toContain("resolve the\ninstalled `ovo` package location");
    // `vercel deploy` uploads everything a .vercelignore doesn't exclude, and
    // the platform default-ignores only the .env.local variants — ovo's dev
    // artifacts and a bare .env must be excluded here or a source deploy
    // ships them (and `.ovo` alone can blow the upload size limit).
    const vercelignore = await readFile(join(projectRoot, ".vercelignore"), "utf8");
    for (const entry of [".env*", ".ovo", ".output", ".nitro", "dist"]) {
  ...
  test("scaffolds the default ovo channel from the Web Chat channel template", async () => {
    const targetDirectory = await createTempDir();
    const projectRoot = await scaffoldBaseProject({
      projectName: "demo-agent",
      model: "openai/gpt-5-mini",
    ...
    const channelPath = join(projectRoot, "agent/channels/ovo.ts");
    const channelSource = await readFile(channelPath, "utf8");

    expect(channelSource).toBe(WEB_APP_TEMPLATE_FILES["agent/channels/ovo.ts"]);
  });
