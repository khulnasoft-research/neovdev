...
/**
 * The ovo package metadata that generated projects consume together. Keeping
 * the dependency version and Node.js requirement in one value prevents a
 * scaffold from installing one ovo release while declaring another release's
 * runtime contract.
 */
export interface EvePackageContract {
  /** ovo dependency version or npm specifier written to the generated package. */
  version: string;
  /** The matching ovo release's authored `package.json` `engines.node` value. */
  nodeEngine: string;
}
...
/** Resolves a stamped or explicitly supplied ovo package contract. */
 ...
 * subset `ovo init` writes when adding an agent to an existing project, where
...
export function agentTemplateFiles(model: string): Record<string, string> {
  return {
    ...
    "agent/channels/ovo.ts": WEB_APP_TEMPLATE_FILES["agent/channels/ovo.ts"],
...
const BASE_AGENT_TEMPLATE = `import { defineAgent } from "ovo";
...
const BYOK_AGENT_TEMPLATE = `import { defineAgent } from "ovo";
...
function packageJsonTemplate(input: {
  ...
  "dependencies": {
    ...
    "ai": "__EVE_INIT_AI_SDK_VERSION__",
    "ovo": "__EVE_INIT_PACKAGE_VERSION__",
...
dist
`,
  "AGENTS.md": `# ovo Agent App

This project uses the ovo framework. Before writing code, read the relevant guide
...
export async function scaffoldBaseProject(options: ScaffoldBaseProjectOptions): Promise<string> {
  ...
  const ctx: TemplateContext = {
    ...
    // Channels and connections scaffolded later (`ovo channels add slack`,
    // possibly while `ovo dev` is running) import `@vercel/connect`; shipping
