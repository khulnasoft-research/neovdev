...
const EXTENSION_DECLARATION_TEMPLATE = `import { defineExtension } from "ovo/extension";
import { z } from "zod";
...
const AGENTS_MD_TEMPLATE = `# ovo Extension Package

This package is an ovo extension — a reusable package of tools, connections,
skills, hooks, and instruction fragments that a consuming agent mounts under
\`agent/extensions/\`.

Before writing code, read the Extensions guide from the installed ovo package
...
- Declare the extension in \`extension/extension.ts\` with \`defineExtension\` from
  \`ovo/extension\`. Config is optional; read bound values via the handle's
...
\`ovo extension build\` (wired to \`build\`/\`prepare\`) transforms the complete
...
Keep \`ovo\` as a required wildcard peer so the consumer's ovo is the one that runs;
ovo validates extension compatibility from the generated manifest. Keep the ovo
development dependency pinned exactly so builds remain reproducible. Upgrade it
when the extension intentionally adopts a newer ovo authoring API.
`;
...
/**
 * Scaffolds a standalone ovo extension package: `extension/extension.ts`, package
 * metadata (`ovo.extension`, peer+dev `ovo`, zod), and TypeScript config. Does
