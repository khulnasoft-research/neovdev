...
describe("defaultMessageReducer", () => {
    ...
    expect(data.messages).toEqual([
      {
        ...
        parts: [
          { type: "step-start" },
          {
            ...
            toolName: "ovo:subagent:research",
            type: "dynamic-tool",
          },
        ],
        role: "assistant",
  ...
  it("projects authorization prompts into assistant message parts", () => {
    ...
    const data = reducer.reduce(
      ...
      createAuthorizationRequiredEvent({
        ...
        stepIndex: 0,
        turnId: "turn_1",
        webhookUrl: "https://agent.example.com/ovo/v1/connections/notion/callback/hook",
      }),
    );
  ...
  it("removes streamed text for a null message completion", () => {
    ...
    data = reducer.reduce(
      data,
      createMessageAppendedEvent({
        messageDelta: "<ovo-empty-delivery/>",
        messageSoFar: "<ovo-empty-delivery/>",
        sequence: 1,
        stepIndex: 1,
        turnId: "turn_1",
      }),
