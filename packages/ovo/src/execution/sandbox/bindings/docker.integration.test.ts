...
const TEMPLATE_KEY = "ovo-sbx-tpl-local-abc123";
...
const SESSION_KEY = "ovo-sbx-ses-local-session-1";
...
describe("createDockerSandboxBackend prewarm", () => {
  it("reuses an existing template image without building", async () => {
    const appRoot = await createScratchDirectory("ovo-docker-sandbox-");
  ...
  it("builds, seeds, commits, and cleans up when the template image is missing", async () => {
    ...
    expect(run?.args).toContain(`ovo.sandbox.role=template-build`);
...
describe("createDockerSandboxBackend create", () => {
  ...
  it("creates a session container from the template image with labels and tags", async () => {
    ...
    try {
      ...
      expect(run?.args).toContain("ovo.sandbox.role=session");
      expect(run?.args).toContain("ovo.sandbox.tag.agent=weather");
      expect(run?.args).toContain("ovo.sandbox.tag.devRunId=dev-run-test");
...
describe("docker session primitives", () => {
  ...
  it("spawns commands through a pid-recording bash -lc wrapper with cwd and env", async () => {
    ...
    expect(wrapper).toMatch(/^echo "\$\$" > '\/tmp\/\.ovo-sbx-spawn-[0-9a-f-]+\.pid'; /);
  ...
  it("kill() tree-kills inside the container before killing the docker exec client", async () => {
    ...
    const pidFilePath = /'(\/tmp\/\.ovo-sbx-spawn-[0-9a-f-]+\.pid)'/.exec(wrapper)?.[1];
    expect(pidFilePath).toBeDefined();

    await spawned.kill();

    const treeKill = findCall(calls, (args) => args.includes("ovo-kill-tree"));
  ...
  it("abort tree-kills the spawned process inside the container", async () => {
    ...
    expect(findCall(calls, (args) => args.includes("ovo-kill-tree"))).toBeUndefined();

    controller.abort();
    await new Promise((resolve) => setTimeout(resolve, 0));

    expect(findCall(calls, (args) => args.includes("ovo-kill-tree"))).toBeDefined();
  });
...
describe("pruneDockerSandboxTemplates", () => {
  it("removes images for stale markers and keeps retained or recent ones", async () => {
    ...
    const markersDirectory = join(appRoot, ".ovo", "sandbox-cache", "docker", "templates");
  ...
  it("keeps the marker when the image is still referenced by a container", async () => {
    ...
    const { readdir } = await import("node:fs/promises");
    await expect(
      readdir(join(appRoot, ".ovo", "sandbox-cache", "docker", "templates")),
