...
async function runWithDevelopmentSandboxProgress(
  ...
  logDevelopmentSandboxCommand(`ovo: starting sandbox command: ${formatCommand(command)}`);
  ...
  const timer = setInterval(() => {
    ...
    logDevelopmentSandboxCommand(
      `ovo: waiting for sandbox command (${elapsedSeconds}s elapsed): ${formatCommand(command)}`,
    );
  }, 5_000);
  timer.unref?.();
  ...
  try {
    ...
    logDevelopmentSandboxCommand(
      `ovo: sandbox command finished (exit ${result.exitCode}): ${formatCommand(command)}`,
    );
    return result;
  } catch (error) {
    logDevelopmentSandboxCommand(`ovo: sandbox command failed: ${formatCommand(command)}`);
    throw error;
  } finally {
    clearInterval(timer);
  }
