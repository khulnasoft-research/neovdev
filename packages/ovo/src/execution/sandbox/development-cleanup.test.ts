      ...
      listWith: vi.fn(async () => [
        {
          configJson: JSON.stringify({
            labels: { "ovo.metadataPath": "/tmp/ovo-microsandbox-session/metadata.json" },
          }),
          name: "msb-session",
          status: "running",
        },
...
describe("stopDevelopmentSandboxResources", () => {
  ...
  it("stops Docker containers and snapshots microsandbox sessions tagged with the dev run id", async () => {
    ...
    await stopDevelopmentSandboxResources({ appRoot: "/tmp/ovo-test", devRunId: "run-123" });
    ...
    expect(state.dockerCli.run).toHaveBeenCalledWith([
      ...
      "label=ovo.sandbox=1",
      "--filter",
      "label=ovo.sandbox.tag.devRunId=run-123",
    ]);
    ...
    expect(state.stopAndSnapshotMicrosandboxSandbox).toHaveBeenCalledWith(
      ...
      "msb-session",
      expect.stringMatching(/^ovo-sbx-state-msb-session:/u),
    );
    expect(state.writeSessionMetadata).toHaveBeenCalledWith(
      "/tmp/ovo-microsandbox-session/metadata.json",
      {
        networkPolicy: "allow-all",
        optionsHash: "options-hash",
        sandboxName: "msb-session",
        stateSnapshotName: expect.stringMatching(/^ovo-sbx-state-msb-session:/u),
        version: 2,
      },
    );
  ...
  it("continues microsandbox cleanup when the Docker CLI is unavailable", async () => {
    ...
    expect(state.loadMicrosandboxWithoutInstall).toHaveBeenCalledWith("/tmp/ovo-test");
    expect(state.microsandboxModule.Sandbox.listWith).toHaveBeenCalledWith({
      labels: {
        "ovo.backend": "microsandbox",
        devRunId: "run-123",
