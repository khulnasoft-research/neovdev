...
function dependencies(
  ...
  return {
    ...
    selectInitHandoff: vi.fn(async () => "ovo-dev"),
...
describe("runInitCommand", () => {
  it("creates the base agent with the runtime default model and invoking ovo dependency", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-base-"));
    const output = logger();
    ...
    expect(manifest).toContain('"ovo": "^0.6.0"');
    ...
    expect(output.messages[1]).toContain("Created an ovo agent in ");
    ...
    expect(output.messages[3]).toContain("$ ovo dev");
  });

  it("opens the selected coding-agent REPL instead of starting ovo dev", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-repl-handoff-"));
    const output = logger();
    const deps = dependencies();
    ...
    expect(deps.spawnCodingAgentRepl).toHaveBeenCalledWith(
      expect.objectContaining({
        command: "codex",
        cwd: projectPath,
        prompt: expect.stringContaining("pnpm exec ovo dev --no-ui"),
      }),
    );
    const prompt = deps.spawnCodingAgentRepl.mock.calls[0]?.[0].prompt;
    expect(prompt).toBe(
      initAgentReplPrompt({
        devCommand: "pnpm exec ovo dev",
      }),
    );
  ...
  it("keeps a completed init successful when its optional handoff is cancelled", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-repl-cancelled-"));
    const output = logger();
    const deps = dependencies();
  ...
  it("uses an explicit init package spec for fresh project scaffolds", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-package-spec-"));
    const output = logger();
    const deps = dependencies();
    vi.stubEnv(EVE_INIT_PACKAGE_SPEC_ENV, "file:/tmp/ovo-0.11.5.tgz");
    ...
    expect(packageJson.dependencies.ovo).toBe("file:/tmp/ovo-0.11.5.tgz");
  });

  it("uses an explicit init package spec when adding to an existing project", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-existing-package-spec-"));
  ...
  it.each([undefined, ".", "./"] as const)(
    ...
    async (target) => {
      const projectPath = await mkdtemp(join(tmpdir(), "ovo-init-current-"));
      const output = logger();
      const deps = dependencies();
      ...
      expect(JSON.parse(await readFile(join(projectPath, "package.json"), "utf8"))).toMatchObject({
        name: expect.stringMatching(/^ovo-init-current-/),
      });
      await expect(pathExists(join(projectPath, "pnpm-workspace.yaml"))).resolves.toBe(true);
      expect(deps.runPackageManagerInstall).toHaveBeenCalledWith(
        "pnpm",
  ...
  it.each([
    ["npm", "overrides", ["exec", "--", "ovo", "dev", "--input", "/model"]],
    ["yarn", "resolutions", ["ovo", "dev", "--input", "/model"]],
    ["bun", "overrides", ["x", "ovo", "dev", "--input", "/model"]],
  ] as const)(
    "scaffolds a fresh project owned by the invoking manager %s without pnpm policy",
    async (kind, aiPinField, devArguments) => {
      const parentDirectory = await mkdtemp(join(tmpdir(), `ovo-init-agent-${kind}-`));
      const output = logger();
      const deps = dependencies();
  ...
  it("scaffolds a fresh named project with the ancestor packageManager before npx", async () => {
    const workspaceRoot = await mkdtemp(join(tmpdir(), "ovo-init-bun-workspace-"));
  ...
  it.each([
    ["npm", "package-lock.json", "bun", ["exec", "--", "ovo", "dev", "--input", "/model"]],
    ["yarn", "yarn.lock", "npm", ["ovo", "dev", "--input", "/model"]],
    ["bun", "bun.lock", "npm", ["x", "ovo", "dev", "--input", "/model"]],
    ["pnpm", "pnpm-lock.yaml", "npm", ["exec", "ovo", "dev", "--input", "/model"]],
  ] as const)(
    ...
    async (kind, lockfile, invokingManager, devArguments) => {
      const workspaceRoot = await mkdtemp(join(tmpdir(), `ovo-init-${kind}-workspace-`));
      const appsDirectory = join(workspaceRoot, "apps");
      await mkdir(appsDirectory, { recursive: true });
      await writeFile(
        join(workspaceRoot, "package.json"),
  ...
  it("scaffolds a fresh pnpm workspace member without nested workspace policy", async () => {
    const workspaceRoot = await mkdtemp(join(tmpdir(), "ovo-init-pnpm-workspace-"));
    const appsDirectory = join(workspaceRoot, "apps");
    await mkdir(appsDirectory, { recursive: true });
    await writeFile(
      join(workspaceRoot, "package.json"),
    ...
    expect(projectPackageJson.dependencies.ovo).toBe("^0.6.0");
  ...
  it("scaffolds under an unclaimed pnpm workspace directory by adding a package pattern", async () => {
    const workspaceRoot = await mkdtemp(join(tmpdir(), "ovo-init-pnpm-unclaimed-workspace-"));
  ...
  it("adds Web Chat under an unclaimed pnpm workspace directory without nested workspace policy", async () => {
    const workspaceRoot = await mkdtemp(join(tmpdir(), "ovo-init-web-pnpm-workspace-"));
    const agentsDirectory = join(workspaceRoot, "agents");
    await mkdir(agentsDirectory, { recursive: true });
    await writeFile(
      join(workspaceRoot, "package.json"),
  ...
  it.each([
    ["yarn", "yarn.lock", "resolutions", ["ovo", "dev", "--input", "/model"]],
    ["bun", "bun.lock", "overrides", ["x", "ovo", "dev", "--input", "/model"]],
  ] as const)(
    ...
    async (kind, lockfile, rootAiPinField, devArguments) => {
      const workspaceRoot = await mkdtemp(join(tmpdir(), `ovo-init-${kind}-workspace-member-`));
      const appsDirectory = join(workspaceRoot, "apps");
      await mkdir(appsDirectory, { recursive: true });
      await writeFile(
        join(workspaceRoot, "package.json"),
  ...
  it("adds Web Chat to an npm-owned fresh scaffold without pnpm configuration", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-agent-web-npm-"));
    const output = logger();
    const deps = dependencies();
    deps.detectInvokingPackageManager.mockReturnValue("npm");
  ...
  it("reports a Git initialization failure through the logger without failing", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-git-fail-"));
    const output = logger();
    ...
    expect(deps.spawnPackageManager).toHaveBeenCalledWith(
      ...
      ["exec", "ovo", "dev", "--input", "/model"],
    );
  });

  it("adds Web Chat without Vercel configuration and preserves the invoking ovo dependency", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-web-"));
    const output = logger();
    const deps = dependencies();
    ...
    expect(await readFile(join(projectPath, "package.json"), "utf8")).toContain('"ovo": "^0.6.0"');
  ...
  it("removes the staged project when Web Chat scaffolding fails", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-web-fail-"));
    const output = logger();
    const deps = dependencies();
  ...
  it.each(["../escape", "nested/agent", "My Agent"])(
    ...
    async (name) => {
      const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-name-"));
      const output = logger();
      const deps = dependencies();
  ...
  it("adds an agent to an existing pnpm project directory", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-dir-"));
    const projectRoot = await createHostProject(parentDirectory, {
      name: "host-app",
    ...
    await expect(pathExists(join(projectRoot, "agent/channels/ovo.ts"))).resolves.toBe(true);
    ...
    expect(JSON.parse(await readFile(join(projectRoot, "package.json"), "utf8"))).toMatchObject({
      dependencies: { "@vercel/connect": "0.2.2", ai: "7.0.0", ovo: "^0.6.0", zod: "^3.25.0" },
      engines: { node: "24.x" },
    });
    ...
    expect(output.messages.join("\n")).toContain("Added an ovo agent to ");
    expect(output.messages.join("\n")).not.toContain("Overrode package.json engines.node");
  });

  it("overrides an incompatible existing node engine declaration and warns for ovo init .", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-dir-engine-"));
    const projectRoot = await createHostProject(parentDirectory, {
      name: "host-app",
  ...
  it("replaces an open node engine range with the scaffolded major", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-dir-engine-compatible-"));
    const projectRoot = await createHostProject(parentDirectory, {
      name: "host-app",
  ...
  it("refuses a target directory without package.json before writing anything", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-dir-no-pkg-"));
    const projectRoot = join(parentDirectory, "host-app");
    await mkdir(projectRoot, { recursive: true });
    const output = logger();
    const deps = dependencies();
  ...
  it.each([
    ["npm", "package-lock.json", ["exec", "--", "ovo", "dev"]],
    ["yarn", "yarn.lock", ["ovo", "dev"]],
    ["bun", "bun.lock", ["x", "ovo", "dev"]],
  ] as const)(
    "drives an existing %s project with its own manager and no pnpm policy",
    async (kind, lockfile, devArguments) => {
      const parentDirectory = await mkdtemp(join(tmpdir(), `ovo-init-dir-${kind}-`));
      const projectRoot = join(parentDirectory, "host-app");
      await mkdir(projectRoot, { recursive: true });
  ...
  it("adds an agent to an existing project with the ancestor package manager", async () => {
    const workspaceRoot = await mkdtemp(join(tmpdir(), "ovo-init-existing-workspace-"));
    const appsDirectory = join(workspaceRoot, "apps");
    ...
    expect(deps.tryInitializeGit).not.toHaveBeenCalled();
    expect(deps.spawnPackageManager).toHaveBeenCalledWith("bun", projectRoot, ["x", "ovo", "dev"]);
  });

  it("adds an agent to an existing pnpm workspace member without nested root-only policy", async () => {
    const workspaceRoot = await mkdtemp(join(tmpdir(), "ovo-init-existing-pnpm-workspace-"));
    const appsDirectory = join(workspaceRoot, "apps");
    const projectRoot = join(appsDirectory, "host-app");
    await mkdir(projectRoot, { recursive: true });
    await writeFile(
  ...
  it("reports agent file conflicts before writing anything", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-dir-conflict-"));
    const projectRoot = await createHostProject(parentDirectory);
    ...
    await expect(
      ...
      message: `Cannot add an ovo agent to "${projectRoot}" because it already has: agent/instructions.md.`,
    });
  ...
  it("refuses --channel-web-nextjs when targeting an existing project", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-dir-web-"));
    const projectRoot = await createHostProject(parentDirectory);
    const output = logger();
    const deps = dependencies();
    ...
    ).rejects.toThrow("ovo channels add web");
  ...
  it("hands a coding agent the setup guide when it omits the target", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-agent-bare-"));
    const output = logger();
    const deps = dependencies();
    ...
    // A bare `ovo init` from an agent means it has not chosen what to build, so
    ...
    expect(printed).toContain("Set up an ovo agent");
    expect(printed).toContain("npx ovo@latest init <name>");
  });

  it("scaffolds and initializes Git for a coding agent but does not spawn the dev server", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-agent-named-"));
    const output = logger();
    const deps = dependencies();
    deps.isCodingAgentLaunch.mockResolvedValue(true);
  ...
  it("derives the agent dev handoff command from the existing project's own manager", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-agent-dir-"));
    const projectRoot = join(parentDirectory, "host-app");
    await mkdir(projectRoot, { recursive: true });
    await writeFile(join(projectRoot, "package.json"), '{ "name": "host-app" }\n', "utf8");
    ...
    expect(deps.spawnPackageManager).not.toHaveBeenCalled();
    expect(output.messages.join("\n")).toContain("npm exec -- ovo dev");
  });

  it("stops before Git and dev when dependency installation fails, replaying its output", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-install-fail-"));
    const output = logger();
    const deps = dependencies();
  ...
  it("replays only the actionable npm error, dropping silly/verbose/http/timing noise", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-npm-noise-"));
    const output = logger();
    const deps = dependencies();
    deps.runPackageManagerInstall.mockImplementation(async (_kind, _projectPath, options) => {
      ...
      options?.onOutput?.({
        stream: "stderr",
        text: "npm http fetch GET 200 https://registry.npmjs.org/ovo 41ms",
      });
  ...
  it("replays only the final npm detail lines when filtering leaves no error", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-npm-fallback-"));
    const output = logger();
    const deps = dependencies();
    deps.runPackageManagerInstall.mockImplementation(async (_kind, _projectPath, options) => {
  ...
  it("streams init phases and package-manager output as debug logs", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-debug-"));
    const output = logger();
    const deps = dependencies();
    deps.runPackageManagerInstall.mockImplementation(async (_kind, _projectPath, options) => {
    ...
    try {
      ...
      expect(debugLines).toContain("[ovo:init] creating agent");
      expect(
        debugLines.some((line) => line.startsWith("[ovo:init] installing dependencies with")),
      ).toBe(true);
      expect(debugLines).toContain("[ovo:init] Progress: resolved 62, reused 62, done");
      expect(debugLines).toContain("[ovo:init] initializing git repository");
      expect(deps.runPackageManagerInstall).toHaveBeenCalledWith(
        expect.anything(),
        expect.anything(),
  ...
  it("reports a failed install as failed under debug", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-debug-failure-"));
    const output = logger();
    const deps = dependencies();
  ...
  it("keeps interactive progress on one physical row", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-init-progress-"));
    const output = logger();
    const deps = dependencies();
