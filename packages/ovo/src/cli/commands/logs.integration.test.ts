...
describe("ovo logs", () => {
  ...
  async function appRootWithLogs(files: Record<string, string>): Promise<string> {
    const root = await mkdtemp(join(tmpdir(), "ovo-logs-cmd-"));
    roots.push(root);
    const directory = join(root, ".ovo", "logs");
  ...
  it("reports missing logs without failing, but fails for an explicit reference", async () => {
    ...
    expect(bare.out).toEqual(["No dev diagnostic logs found under .ovo/logs."]);
    ...
    expect(JSON.parse(out.join("\n"))).toEqual([
      {
        ...
        path: join(root, ".ovo", "logs", SECOND),
