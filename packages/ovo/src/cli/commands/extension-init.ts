...
/** Same override env as agent `ovo init` so CI can pin the ovo package specifier. */
...
async function scaffoldExtension(
  ...
  const stagingDirectory = await mkdtemp(join(parentPath, ".ovo-extension-init-"));
...
/**
 * Creates a new ovo extension package. Same install/git flow as agent `ovo init`,
 * but always create-only and prints next steps instead of starting `ovo dev`.
 */
export async function runExtensionInitCommand(
  ...
  try {
    ...
    if (existingDirectory !== undefined) {
      throw new Error(
        "`ovo extension init` creates a new extension package and cannot add to an existing project. " +
          "Pass a new directory name, or run from an empty directory with `ovo extension init .`.",
      );
    }
