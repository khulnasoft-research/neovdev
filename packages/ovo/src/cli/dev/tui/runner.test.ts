  ...
  it("keeps the REPL session when HMR publishes a new runtime revision", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: Parameters<typeof fetch>[0], init?: RequestInit) => {
        ...
        if (url.pathname.startsWith("/ovo/v1/dev/runtime-artifacts")) {
    ...
    expect(
      requests
        .filter(
          (request) =>
            request.method === "POST" && request.url.pathname.startsWith("/ovo/v1/session"),
        )
        .map((request) => request.url.pathname),
    ).toEqual(["/ovo/v1/session", "/ovo/v1/session/session-1"]);
  });
...
describe("EveTUIRunner native continuation state", () => {
  it("continues an input request from ovo-native turn state", async () => {
    ...
    const session = sessionYielding([
      {
        ...
        data: {
          ...
          name: "linear",
          webhookUrl: "https://ovo.test/connections/linear/callback",
        },
      },
...
describe("EveTUIRunner Vercel status line", () => {
  ...
  it("forces a runtime rebuild after /connect adds a connection", async () => {
    ...
    expect(
      requests.some(
        (url) =>
          url.pathname === "/ovo/v1/dev/runtime-artifacts/rebuild" &&
...
describe("EveTUIRunner gateway-auth failure rendering", () => {
  const gatewayFailure = {
    ...
    details: {
      ...
      hint: "Run `ovo link` to populate `VERCEL_OIDC_TOKEN`, or set `AI_GATEWAY_API_KEY`…",
    },
  };
  ...
  it("keeps the harness hint when the TUI has no local project to link", async () => {
    ...
    expect(errors).toHaveLength(1);
    expect(errors[0]!.errorText).toContain("MODEL_CALL_FAILED");
    expect(errors[0]!.hint).toContain("ovo link");
