...
const DEFAULT_APP_ROOT = "/tmp/ovo-dev-hmr";
...
describe("startAuthoredSourceWatcher", () => {
  ...
  it("ignores generated directories while watching authored roots", async () => {
    ...
    try {
      ...
      expect(ignored(join(host.appRoot, ".ovo", "dev-hosts", "candidate"))).toBe(true);
      expect(ignored(join(host.appRoot, "node_modules", "ovo"))).toBe(true);
  ...
  it("watches root config, env, workspace lockfiles, and tsconfig extends", async () => {
    const workspaceRoot = await mkdtemp(join(tmpdir(), "ovo-dev-watch-root-"));
  ...
  it("watches local workspace dependency roots rather than their node_modules links", async () => {
    const workspaceRoot = await mkdtemp(join(tmpdir(), "ovo-dev-watch-linked-"));
    const appRoot = join(workspaceRoot, "apps", "watch-agent");
  ...
  it("watches workspace extension build inputs and ignores its managed output", async () => {
    const appRoot = await mkdtemp(join(tmpdir(), "ovo-dev-watch-extension-"));
  ...
  it("does not watch ancestor lockfiles for an app without a workspace marker", async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), "ovo-dev-watch-standalone-"));
  ...
  it("updates watched tsconfig targets only after a committed rebuild", async () => {
    const appRoot = await mkdtemp(join(tmpdir(), "ovo-dev-watch-extends-"));
    temporaryDirectories.push(appRoot);
    await mkdir(join(appRoot, "agent"), { recursive: true });
    await writeFile(join(appRoot, "package.json"), '{"name":"watch-agent","type":"module"}\n');
...
function createPreparedHost(
  ...
  return {
    ...
    compiledArtifacts: {
      bootstrapPath: join(appRoot, ".ovo", "dev-hosts", "test", "bootstrap.mjs"),
      workflowWorldPluginPath: join(appRoot, ".ovo", "dev-hosts", "test", "world.mjs"),
    },
    generation: {
      fingerprint: "runtime",
      runtimeAppRoot: join(appRoot, ".ovo", "dev-runtime", "snapshots", "test", "source", "app"),
      snapshotRoot: join(appRoot, ".ovo", "dev-runtime", "snapshots", "test"),
      snapshotSourceRoot: join(appRoot, ".ovo", "dev-runtime", "snapshots", "test", "source"),
      sourceRoot: appRoot,
    },
    scheduleRegistrations: [],
    schedules: [],
    workflowBuildDir: join(appRoot, ".ovo", "dev-hosts", "test", "workflow"),
    workspaceExtensions: [],
    workspace: {
      artifactsDir: join(appRoot, ".ovo", "dev-hosts", "test", "artifacts"),
      compilerArtifactsDir: join(appRoot, ".ovo", "dev-hosts", "test", "compiler"),
      nitroBuildDir: join(appRoot, ".ovo", "dev-hosts", "test", "nitro"),
      nitroOutputDir: join(appRoot, ".ovo", "dev-hosts", "test", "output"),
      rootDir: join(appRoot, ".ovo", "dev-hosts", "test"),
      workflowBuildDir: join(appRoot, ".ovo", "dev-hosts", "test", "workflow"),
    },
  };
}
