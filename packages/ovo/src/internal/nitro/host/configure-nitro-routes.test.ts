...
vi.mock("../../application/package.js", () => ({
  ...
  resolvePackageRoot: () =>
    "G:\\projects\\test-ovo\\node_modules\\.pnpm\\ovo@0.3.0\\node_modules\\ovo",
  resolvePackageSourceFilePath: (relativeSourcePath: string) =>
    `G:\\projects\\test-ovo\\node_modules\\.pnpm\\ovo@0.3.0\\node_modules\\ovo\\dist\\${relativeSourcePath
      .replace(/\.[cm]?tsx?$/, ".js")
      .replaceAll("/", "\\")}`,
  resolveWorkflowModulePath: (specifier: string) =>
    `G:\\projects\\test-ovo\\node_modules\\.pnpm\\ovo@0.3.0\\node_modules\\ovo\\dist\\src\\compiled\\${specifier
...
function createNitroStub(
  ...
  const nitro: NitroStub = {
    ...
    options: {
      buildDir: input.buildDir ?? "G:\\projects\\test-ovo\\.ovo\\nitro",
      dev: input.dev ?? false,
      handlers: [],
      rootDir: input.rootDir ?? "G:\\projects\\test-ovo",
      virtual: {},
    },
    routing: {
      sync() {},
...
function createPreparedHost(
  ...
  const appRoot = input.appRoot ?? "G:\\projects\\test-ovo";
  ...
  const preparedHost: PreparedApplicationHostStub = {
    ...
    compiledArtifacts: {
      bootstrapPath: `${appRoot}\\.ovo\\compiled-artifacts-bootstrap.mjs`,
      workflowWorldPluginPath: `${appRoot}${pathSeparator}.ovo${pathSeparator}compiled-artifacts-workflow-world.mjs`,
    },
    scheduleRegistrations: [],
    schedules: [],
    workflowBuildDir: input.workflowBuildDir ?? `${appRoot}\\.ovo\\workflow-cache`,
  };
...
describe("Nitro route configuration", () => {
  ...
  it("registers package-owned route files through file-url virtual handlers", async () => {
    ...
    expect(healthHandler?.handler).toBe(`#ovo-route-handler/GET ${EVE_HEALTH_ROUTE_PATH}`);

    const virtualSource = nitro.options.virtual[healthHandler?.handler ?? ""];
    expect(virtualSource).toContain(
      'import handler from "file:///G:/projects/test-ovo/node_modules/.pnpm/ovo@0.3.0/node_modules/ovo/dist/src/internal/nitro/routes/health.js";',
    );
  ...
  it("bakes the agent name into the home page route", async () => {
    ...
    expect(homeHandler?.handler).toBe("#ovo-route/");
  ...
  it("registers the health route for HEAD so load balancers probing with HEAD see 200", async () => {
    ...
    expect(headHandler?.handler).toBe(`#ovo-route-handler/HEAD ${EVE_HEALTH_ROUTE_PATH}`);
  ...
  it("registers workflow routes through physical handlers with relative bundle imports", async () => {
    const root = "/tmp/ovo-nitro-routes";
    ...
    expect(nitro.options.virtual["#ovo-workflow/workflows"]).toBeUndefined();
  });
  ...
  it("leaves development queue dispatch to the stable parent", async () => {
    const root = "/tmp/ovo-nitro-direct-handlers";
    const buildDir = `${root}/nitro`;
    const nitro = createNitroStub({ buildDir, dev: true, rootDir: root });
  ...
  it("keeps configured development World queue dispatch inside the worker", async () => {
    const root = "/tmp/ovo-nitro-configured-world-handlers";
  ...
  it("bakes the module map loader into the dev schedule handler", async () => {
    ...
    const source = nitro.options.virtual[`#ovo-route${EVE_DEV_DISPATCH_SCHEDULE_ROUTE_PATTERN}`];
  ...
  it("registers the dev runtime artifact revision route only in dev mode", async () => {
    ...
    expect(devNitro.options.handlers).toContainEqual({
      handler: "#ovo-route/ovo/v1/dev/runtime-artifacts",
      method: "GET",
      route: "/ovo/v1/dev/runtime-artifacts",
    });
    expect(devNitro.options.handlers).not.toContainEqual(
      expect.objectContaining({
        route: "/ovo/v1/dev/runtime-artifacts/rebuild",
      }),
    );
  ...
  it("registers the agent info route for dev and production app builds", async () => {
    ...
    expect(devNitro.options.handlers).toContainEqual({
      handler: `#nitro/virtual/ovo-channel/GET ${EVE_INFO_ROUTE_PATH}`,
      method: "GET",
    ...
    expect(
      devNitro.options.virtual[`#nitro/virtual/ovo-channel/GET ${EVE_INFO_ROUTE_PATH}`],
    ).toContain('"kind":"development"');
    expect(
      prodNitro.options.virtual[`#nitro/virtual/ovo-channel/GET ${EVE_INFO_ROUTE_PATH}`],
    ...
    expect(devNitro.options.virtual[`#ovo-route${EVE_INFO_ROUTE_PATH}`]).toBeUndefined();
    expect(prodNitro.options.virtual[`#ovo-route${EVE_INFO_ROUTE_PATH}`]).toBeUndefined();
  });

  it("does not register direct workflow queue handlers for Vercel production builds", async () => {
    vi.stubEnv("VERCEL", "1");

    const root = "/tmp/ovo-nitro-direct-handlers-vercel";
    const buildDir = `${root}/nitro`;
    const workflowBuildDir = `${root}/workflow-cache`;
  ...
  it("registers direct workflow queue handlers for self-hosted production builds with a configured world", async () => {
    const root = "/tmp/ovo-nitro-direct-handlers-self-hosted";
    const buildDir = `${root}/nitro`;
    const workflowBuildDir = `${root}/workflow-cache`;
    const nitro = createNitroStub({ buildDir, dev: false, rootDir: root });
  ...
  it("does not register direct workflow queue handlers for self-hosted production builds without a configured world", async () => {
    const root = "/tmp/ovo-nitro-direct-handlers-self-hosted-no-world";
    const buildDir = `${root}/nitro`;
    const workflowBuildDir = `${root}/workflow-cache`;
    const nitro = createNitroStub({ buildDir, dev: false, rootDir: root });
