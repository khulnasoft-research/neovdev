...
describe("localDev", () => {
  ...
  it("authenticates requests addressed to `localhost`", () => {
    const result = localDev()(requestFor("http://localhost:3000/ovo/v1/info"));
  ...
  it("authenticates requests addressed to `127.0.0.1`", () => {
    expect(localDev()(requestFor("http://127.0.0.1:3000/ovo/v1/info"))).toMatchObject({
      principalType: "local-dev",
    });
  });
  ...
  it("rejects requests addressed to a public hostname (returns null)", () => {
    ...
    expect(localDev()(requestFor("https://example.com/ovo/v1/info"))).toBeNull();
    expect(localDev()(requestFor("https://myapp.fly.dev/ovo/v1/info"))).toBeNull();
    expect(localDev()(requestFor("https://myapp.vercel.app/ovo/v1/info"))).toBeNull();
  });
...
describe("routeAuth", () => {
  ...
  it("advertises a Basic challenge when the route is protected only by httpBasic", async () => {
    ...
    if (result instanceof Response) {
      expect(result.status).toBe(401);
      expect(result.headers.get("www-authenticate")).toBe('Basic realm="ovo", charset="UTF-8"');
    }
  });
  ...
  it("advertises both Basic and Bearer for a mixed httpBasic + jwtHmac route", async () => {
    ...
    if (result instanceof Response) {
      ...
      expect(header).toContain('Basic realm="ovo", charset="UTF-8"');
  ...
  it("routes the production placeholder as a structured 401", async () => {
    ...
    if (result instanceof Response) {
      ...
      await expect(result.json()).resolves.toEqual({
        ...
        error:
          "Production auth is not configured. Replace placeholderAuth() in agent/channels/ovo.ts with your app's auth provider.",
        ok: false,
      });
    }
  });
  ...
  it("emits debug logs on the auth.vercel-oidc namespace for rejection paths", async () => {
    ...
    try {
      ...
      expect(lines).toContain(
        "[ovo:auth.vercel-oidc] Rejected token that failed to decode as a JWT.",
      );
    } finally {
  ...
  it("rejects a Vercel-issued token whose project_id does not match VERCEL_PROJECT_ID", async () => {
    ...
    try {
      const token = await issuer.signToken({
        ...
        // Different project ID than the deployed ovo app.
...
describe("vercelOidc strategy helper", () => {
  ...
  it("uses the local host's linked project binding only for bearer requests", async () => {
    ...
    try {
      ...
      const authFn = vercelOidc();
      const unauthenticatedRequest = new Request("http://localhost/ovo/v1/session");
      await expect(
      ...
      const request = new Request("http://localhost/ovo/v1/session", {
        headers: { authorization: `Bearer ${token}` },
      });
