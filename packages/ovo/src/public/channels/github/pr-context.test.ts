...
describe("SHA pinning", () => {
  it("keeps webhook ABC when live PR head is XYZ at context build time", async () => {
            ...
            files: [
              {
                additions: 1,
                deletions: 0,
                filename: "packages/ovo/src/at-abc.ts",
    ...
    expect(text(messages)).toContain("- packages/ovo/src/at-abc.ts");
...
describe("GitHub pull-request context", () => {
  it("builds metadata and the changed-file diff by default", async () => {
        ...
        jsonResponse([
          {
            additions: 4,
            deletions: 1,
            filename: "packages/ovo/src/github.ts",
            patch: "@@ real patch",
            status: "modified",
          },
        ]),
    ...
    expect(text(messages)).toContain("- packages/ovo/src/github.ts (modified, +4, -1)");
