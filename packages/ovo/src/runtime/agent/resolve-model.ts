...
async function loadSourceBackedRuntimeModelReference(
  ...
  const normalizedDefinition = normalizeAgentDefinition(
    definition,
    `Expected the authored agent config export "${reference.source.exportName ?? "default"}" from "${reference.source.logicalPath}" to match the public ovo shape.`,
  );
...
export async function loadDynamicRuntimeModelDefinition(input: {
  ...
  const normalizedDefinition = normalizeAgentDefinition(
    definition,
    `Expected the authored agent config export "${input.dynamicModel.exportName ?? "default"}" from "${input.dynamicModel.logicalPath}" to match the public ovo shape.`,
  );
