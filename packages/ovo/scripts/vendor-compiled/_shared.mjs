 ...
 * the monorepo can adopt the same vendoring pipeline without ovo-specific
 ...
 *
 * Use this when a package's surface is part of ovo's public type contract
 ...
 *     already part of ovo's public dependency surface, such as `ai`.
 *   `vendored` — rewrites `from '<moduleName>'` to a `#compiled/<...>`
 *     specifier so the dependency resolves against ovo's own vendored copy.
 *     This works even when one vendored package's `.d.ts` references another
 *     (e.g. `@workflow/core` → `@workflow/world` → `zod`) because vendored
 *     directories intentionally carry no `package.json` to shadow ovo's
 ...
 * don't have to install the @types package just to typecheck against ovo
 * (e.g. `mdast`).
 */
export function buildOpaqueTypesStub(names, moduleName) {
  const lines = [
    ...
    `// upstream @types just to typecheck against ovo. If real types are`,
...
export function createOptionalNativeStubPlugin(packageNames) {
  ...
  return {
    name: "ovo-optional-native-stub",
    resolveId(source) {
      if (!packageNameSet.has(source)) {
        return undefined;
      }
      return `ovo-optional-native-stub:${source}`;
    },
    load(id) {
      if (!id.startsWith("ovo-optional-native-stub:")) {
        return undefined;
      }
      const packageName = id.slice("ovo-optional-native-stub:".length);
      return {
...
async function prepareCompiledModule({ module, compiledRoot, packageRoot }) {
  ...
  // under ovo (which is `"type": "module"`), so the bundled ESM `.js` inherits
  // the right module type without one. More importantly, writing a package.json
  // here would create a package scope that shadows ovo's `#compiled/*` imports
  ...
  // shadowing file, resolution falls through to ovo's root imports map and the
