...
} from "#compiled/eventsource-parser/stream/index.js";
...
declare function getWebSocketConstructor(
 ...
 */
declare function waitForWebSocketBufferDrain(
 ...
 */
declare function connectToWebSocket({
  url,
  protocols,
  headers,
  webSocket,
 ...
 * ```
 */
declare function convertToFormData<T extends Record<string, unknown>>(
  input: T,
  options?: {
 ...
 */
declare function createToolNameMapping({
  tools,
  providerToolNames,
}: {
  /**
 ...
 */
declare function detectMediaType({
  data,
  topLevelType,
}: {
 ...
 */
declare function fetchWithValidatedRedirects({
  url,
  headers,
  abortSignal,
  maxRedirects,
...
declare function zodSchema<OBJECT>(
 ...
 */
declare function parseJSON(options: { text: string; schema?: undefined }): Promise<JSONValue>;
/**
 ...
 */
declare function parseJSON<T>(options: { text: string; schema: FlexibleSchema<T> }): Promise<T>;
type ParseResult<T> =
  | {
      success: true;
      value: T;
...
declare function injectJsonInstructionIntoMessages({
  messages,
  schema,
  schemaPrefix,
  schemaSuffix,
 ...
 */
declare function isUrlSupported({
  mediaType,
  url,
  supportedUrls,
}: {
...
>;
declare function isCustomReasoning(
 ...
 *   supported.
 */
declare function mapReasoningToProviderEffort<T extends string>({
  reasoning,
  effortMap,
  warnings,
}: {
 ...
 *   supported.
 */
declare function mapReasoningToProviderBudget({
  reasoning,
  maxOutputTokens,
  maxReasoningBudget,
  minReasoningBudget,
 ...
 */
declare function parseJsonEventStream<T>({
  stream,
  schema,
}: {
...
declare function parseProviderOptions<OPTIONS>({
  provider,
  providerOptions,
  schema,
}: {
 ...
 */
declare function isExecutableTool<TOOL extends Tool>(
  tool: TOOL | undefined,
 ...
 */
type ToolExecuteFunction<INPUT, OUTPUT, CONTEXT extends Context | unknown | never> = (
  input: INPUT,
 ...
 */
type ToolNeedsApprovalFunction<INPUT, CONTEXT extends Context | unknown | never> = (
  input: INPUT,
  options: {
    /**
     * The ID of the tool call. You can use it e.g. when sending tool-call related information with stream data.
...
> = {
  ...
  needsApproval?:
    | boolean
    | ToolNeedsApprovalFunction<[INPUT] extends [never] ? unknown : INPUT, NoInfer<CONTEXT>>;
  /**
...
> = BaseTool<INPUT, OUTPUT, CONTEXT> & {
  ...
  description?:
    | string
    | ((options: { context: NoInfer<CONTEXT>; experimental_sandbox?: SandboxSession }) => string);
  /**
 ...
 */
type ProviderDefinedToolFactory<INPUT, ARGS extends object, CONTEXT extends Context = {}> = <
  OUTPUT,
>(
  options: ARGS & {
    execute?: ToolExecuteFunction<INPUT, OUTPUT, CONTEXT>;
...
declare function createProviderDefinedToolFactory<
  INPUT,
  ARGS extends object,
...
declare function createProviderDefinedToolFactoryWithOutputSchema<
  INPUT,
  OUTPUT,
  ARGS extends object,
  CONTEXT extends Context = {},
...
declare function createProviderExecutedToolFactory<
  INPUT,
  OUTPUT,
  ARGS extends object,
  CONTEXT extends Context = {},
 ...
 */
declare function readResponseWithSizeLimit({
  response,
  url,
  maxBytes,
}: {
 ...
 */
declare function resolveFullMediaType({ part }: { part: LanguageModelV4FilePart }): string;
 ...
 */
declare function resolveProviderReference({
  reference,
  provider,
}: {
 ...
 */
declare const retryWithExponentialBackoff: ({
  maxRetries,
  initialDelayInMs,
  backoffFactor,
  abortSignal,
 ...
 * ```
 */
declare function serializeModelOptions<
  CONFIG extends {
 ...
 */
declare function serializeTranscriptionStreamPart(
  part: Experimental_TranscriptionModelV4StreamPart,
): string | undefined;
/**
 ...
 */
declare function parseTranscriptionStreamPart(
  text: string,
): Experimental_TranscriptionModelV4StreamPart | undefined;
 ...
 */
declare function validateTypes<OBJECT>({
  value,
  schema,
  context,
}: {
 ...
 * @returns An object with either a `success` flag and the parsed and typed data, or a `success` flag and an error object.
 */
declare function safeValidateTypes<OBJECT>({
  value,
  schema,
  context,
}: {
 ...
 */
declare function withUserAgentSuffix(
 ...
 */
type InferToolInput<TOOL extends Tool<any, any, any>> =
  TOOL extends Tool<infer INPUT, any, any> ? INPUT : never;

/**
 * Infer the output type of a tool.
 */
type InferToolOutput<TOOL extends Tool<any, any, any>> =
 ...
 */
declare function executeTool<TOOL extends Tool>({
  tool,
  input,
  options,
}: {
 ...
 */
type InferToolSetContext<TOOLS extends ToolSet> = Normalize<
...
export {
  ...
  type ExecutableTool,
  type SandboxProcess as Experimental_SandboxProcess,
  type SandboxSession as Experimental_SandboxSession,
