...
async function createAgentProject(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "ovo-link-command-"));
...
describe("runLinkCommand", () => {
  test("refuses a directory without an ovo agent", async () => {
    const projectRoot = await mkdtemp(join(tmpdir(), "ovo-link-empty-"));
  ...
  test("creates a project through the shared flow and reports success", async () => {
    ...
    expect(fake.prompter.intro).toHaveBeenCalledWith("Link your ovo agent to Vercel");
