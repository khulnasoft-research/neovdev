...
const log = createLogger("ovo.channel");

/**
 * Event-handler channel context exposed by `eveChannel({ events })`. The default ovo HTTP channel
...
/**
 * Higher-level CORS policy for the default ovo HTTP channel. Pass `true` for
...
/** Low-level ovo HTTP handle exposed to `eveChannel({ onMessage })`. */
...
export interface EveChannelInput {
  ...
  /**
   * Browser CORS policy for the ovo HTTP routes. Omit or pass `false` to leave
  ...
  /**
   * Pre-dispatch hook for inbound ovo HTTP messages. Runs after route auth and body
  ...
  /**
   * Runtime stream-event handlers for the default ovo HTTP channel. Handlers receive
...
/**
 * Builds the default ovo HTTP channel: a {@link defineChannel} instance serving the
 * built-in `/ovo/v1` routes (GET inspects the agent, POST creates a session, POST
 ...
 * Default-export the result as your `agent/channels/ovo.ts` channel; reach for
...
export function eveChannel(input: EveChannelInput): EveChannel {
  ...
  return defineChannel<undefined, EveEventContext>({
    ...
    routes: [
      ...
      POST("/ovo/v1/session", async (req, { send }) => {
        const authResult = await routeAuth(req, input.auth);
        if (authResult instanceof Response) return authResult;
        ...
        const token = `ovo:${crypto.randomUUID()}`;
      ...
      POST("/ovo/v1/session/:sessionId", async (req, { send, getSession, params }) => {
        const authResult = await routeAuth(req, input.auth);
        if (authResult instanceof Response) return authResult;
        const sessionAuth = authResult;
      ...
      GET("/ovo/v1/session/:sessionId/stream", async (req, { getSession, params }) => {
        const authResult = await routeAuth(req, input.auth);
        if (authResult instanceof Response) return authResult;
...
async function resolveOnMessage(input: {
  ...
  try {
    ...
    const ctx: EveMessageContext = { ovo };
...
function parseMessagePart(raw: unknown): TextPart | FilePart | Response {
  ...
  if (part.type === "file") {
    ...
    // Callers must never supply framework-internal refs (`ovo-url:`,
    // `ovo-sandbox:`, `ovo-attachment:`): the staging pipeline trusts the
