...
/** A coding-agent REPL that can take over the terminal after `ovo init`. */
...
export type InitHandoff = "ovo-dev" | CodingAgentRepl;
 ...
 * existing `ovo dev` handoff. Non-interactive sessions, and systems with none
...
export async function selectInitHandoff(input: {
  ...
  if (!dependencies.hasInteractiveTerminal()) return "ovo-dev";
