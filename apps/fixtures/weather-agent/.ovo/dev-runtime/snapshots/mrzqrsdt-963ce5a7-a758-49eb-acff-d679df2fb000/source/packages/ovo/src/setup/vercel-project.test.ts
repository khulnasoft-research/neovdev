...
describe("getVercelAuthStatus", () => {
  it("reports authenticated when whoami succeeds", async () => {
    ...
    await expect(getVercelAuthStatus("/tmp/ovo-agent")).resolves.toBe("authenticated");
  });
  ...
  it("reports logged-out when whoami ran but exited non-zero", async () => {
    ...
    await expect(getVercelAuthStatus("/tmp/ovo-agent")).resolves.toBe("logged-out");
  });
  ...
  it("reports cli-missing — not logged-out — when the binary is absent (ENOENT)", async () => {
    ...
    await expect(getVercelAuthStatus("/tmp/ovo-agent")).resolves.toBe("cli-missing");
  });
  ...
  it("reports unavailable — not logged-out — on a transient fault (DNS/network)", async () => {
    ...
    await expect(getVercelAuthStatus("/tmp/ovo-agent")).resolves.toBe("unavailable");
  });
});
...
describe("requireAuth", () => {
  it("throws a CLI-missing action (not a login action) on ENOENT", async () => {
    ...
    await expect(requireAuth("/tmp/ovo-agent")).rejects.toMatchObject({
  ...
  it("throws a plain error (not a login action) on a transient fault", async () => {
    ...
    const error = await requireAuth("/tmp/ovo-agent").catch((e: unknown) => e);
...
describe("pickTeam", () => {
  it("shows a spinner around the team pull and stops it before selection", async () => {
    ...
    await expect(pickTeam(prompter, "/tmp/ovo-agent", undefined)).resolves.toBe("team-b");
  ...
  it("stops the spinner even when the team pull throws", async () => {
    ...
    await expect(pickTeam(prompter, "/tmp/ovo-agent", undefined)).rejects.toThrow("network down");
    expect(stop).toHaveBeenCalledTimes(1);
  });
});
...
describe("pickProject", () => {
  it("labels the spinner with the team and stops it before selection", async () => {
    ...
    await expect(pickProject(prompter, "/tmp/ovo-agent", "team-a")).resolves.toEqual({
      kind: "existing",
...
describe("pickNewProjectName", () => {
  it("prompts for a replacement when the default project name already exists", async () => {
    ...
    await expect(
      pickNewProjectName(prompter, "/tmp/ovo-agent", "team-a", "my-agent"),
...
describe("assertNewProjectNameAvailable", () => {
  it("uses an exact project lookup instead of a paginated list", async () => {
    ...
    await expect(
      assertNewProjectNameAvailable("/tmp/ovo-agent", "team-a", "my-agent"),
    ).rejects.toThrow(
    ...
    expect(mockedCaptureVercel).toHaveBeenCalledWith(
      ["api", "/v9/projects/my-agent", "--scope", "team-a", "--raw"],
      { cwd: "/tmp/ovo-agent", signal: undefined, timeoutMs: 15_000 },
    );
  });
...
describe("resolveProjectByNameOrId", () => {
  it("maps Vercel API fields to the stable project identity", async () => {
    ...
    await expect(resolveProjectByNameOrId("/tmp/ovo-agent", "team-a", "my-agent")).resolves.toEqual(
  ...
  it("bounds the direct lookup with the project request deadline", async () => {
    ...
    await resolveProjectByNameOrId("/tmp/ovo-agent", "team-a", "my-agent");
...
describe("linkProject", () => {
  it("links a resolved existing project through `vercel link`", async () => {
    ...
    expect(mockedRunVercel).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ cwd: "/tmp/ovo-agent", nonInteractive: true }),
    );
  });
  ...
  it("creates and links an available new project", async () => {
    mockedCaptureVercel
      ...
      .mockResolvedValueOnce(captured({ framework: "ovo" }));
  ...
  it("keeps a detected host framework when the framework-specific ovo import is present", async () => {
    mockedCaptureVercel
      .mockResolvedValueOnce(
        failedCapture(
          JSON.stringify({ error: { code: "not_found", message: "Project not found" } }),
    ...
    expect(detectFrameworkIntegrationImport).toHaveBeenCalledWith("/tmp/ovo-agent", "ovo/next");
    expect(mockedCaptureVercel).toHaveBeenCalledTimes(2);
  });

  it.each([
    { framework: "nuxtjs", importSpecifier: "ovo/nuxt" },
    { framework: "sveltekit", importSpecifier: "ovo/sveltekit" },
  ...
  it("keeps a detected host framework when the user confirms the framework-specific ovo import", async () => {
    mockedCaptureVercel
      .mockResolvedValueOnce(
        failedCapture(
          JSON.stringify({ error: { code: "not_found", message: "Project not found" } }),
    ...
    expect(single).toHaveBeenCalledWith(
      expect.objectContaining({
        message: "Vercel detected Next.js. Is this project using ovo/next?",
      }),
    );
    expect(mockedCaptureVercel).toHaveBeenCalledTimes(2);
  });

  it("sets the project framework to ovo when the user rejects a detected host framework", async () => {
    mockedCaptureVercel
      .mockResolvedValueOnce(
        failedCapture(
          JSON.stringify({ error: { code: "not_found", message: "Project not found" } }),
  ...
  it("sets the project framework to ovo when Vercel returns no framework", async () => {
    mockedCaptureVercel
      .mockResolvedValueOnce(
        failedCapture(
          JSON.stringify({ error: { code: "not_found", message: "Project not found" } }),
  ...
  it("sets the project framework to ovo in headless mode when a detected host framework is ambiguous", async () => {
    mockedCaptureVercel
      .mockResolvedValueOnce(
        failedCapture(
          JSON.stringify({ error: { code: "not_found", message: "Project not found" } }),
  ...
  it("sets the project framework to ovo when the detected framework has no ovo integration", async () => {
    mockedCaptureVercel
      .mockResolvedValueOnce(
        failedCapture(
          JSON.stringify({ error: { code: "not_found", message: "Project not found" } }),
