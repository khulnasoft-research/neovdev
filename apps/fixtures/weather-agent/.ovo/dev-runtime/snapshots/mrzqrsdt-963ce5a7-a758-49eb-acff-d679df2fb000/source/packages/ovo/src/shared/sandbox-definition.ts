...
/**
 * Resolves a build-time revalidation key for a bootstrap snapshot. ovo
...
interface SandboxDefinitionBase<BO = Record<string, never>, SO = Record<string, never>> {
   ...
   * ```
   *
   * When this field is omitted, ovo substitutes `defaultSandbox()` at
...
> extends SandboxDefinitionBase<BO, SO> {
   ...
   * snapshot produced by {@link bootstrap}. ovo evaluates this
