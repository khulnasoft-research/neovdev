...
const COMPILED_RUNTIME_MODEL_CATALOG_CACHE_KIND = "ovo-model-catalog-cache";
...
/**
 * Stable runtime model limits that ovo can embed in compiled artifacts without
...
export function resolveCompiledRuntimeModelCatalogCachePath(appRoot: string): string {
  return join(appRoot, ".ovo", "cache", "model-catalog.json");
}
...
async function fetchAndPersistModelCatalog(
  ...
  try {
    ...
    await mkdir(join(appRoot, ".ovo", "cache"), { recursive: true });
