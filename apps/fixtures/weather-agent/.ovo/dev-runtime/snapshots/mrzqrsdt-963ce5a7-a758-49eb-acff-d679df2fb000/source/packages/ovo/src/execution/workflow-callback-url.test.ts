...
describe("resolveVercelProductionCallbackBaseUrl", () => {
  ...
  it("adds the Vercel automation bypass query param when configured", () => {
    ...
    expect(
      createWorkflowCallbackUrl("https://agent.example.com", "/ovo/v1/callback/ovo%3Aparent-token"),
    ).toBe(
      "https://agent.example.com/ovo/v1/callback/ovo%3Aparent-token?x-vercel-protection-bypass=secret+value",
    );
  });
  ...
  it("preserves existing callback query params when adding the Vercel bypass query param", () => {
    ...
    expect(
      createWorkflowCallbackUrl(
        ...
        "/ovo/v1/connections/linear/callback/tok123?code=abc",
      ),
    ).toBe(
      "https://agent.example.com/ovo/v1/connections/linear/callback/tok123?code=abc&x-vercel-protection-bypass=secret",
    );
  });
});
