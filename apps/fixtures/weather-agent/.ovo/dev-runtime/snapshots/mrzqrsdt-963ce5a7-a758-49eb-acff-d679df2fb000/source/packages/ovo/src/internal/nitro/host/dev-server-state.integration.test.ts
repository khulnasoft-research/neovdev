...
async function createState(): Promise<DevelopmentServerState> {
  const appRoot = await mkdtemp(join(tmpdir(), "ovo-dev-server-state-"));
...
describe("DevelopmentServerState", () => {
  ...
  it("writes and reads the ready server URL", async () => {
    ...
    await expect(readFile(join(state.appRoot, ".ovo", STATE_FILE_NAME), "utf8")).resolves.toBe(
  ...
  it("treats malformed state as stale", async () => {
    const state = await createState();
    await mkdir(join(state.appRoot, ".ovo"), { recursive: true });
    await writeFile(join(state.appRoot, ".ovo", STATE_FILE_NAME), "{ not json", "utf8");
