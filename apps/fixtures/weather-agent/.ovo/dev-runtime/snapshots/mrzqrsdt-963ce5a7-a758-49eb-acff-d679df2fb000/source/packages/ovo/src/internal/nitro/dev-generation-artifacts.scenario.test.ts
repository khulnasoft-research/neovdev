...
describe("development generation artifacts", () => {
  ...
  it("materializes one shared module graph for every authored entry", async () => {
    const sharedMarker = "ovo-shared-generation-module-marker";
    ...
    const compileRoot = join(snapshot.runtimeAppRoot, ".ovo", "compile");
  ...
  it("preserves extension scope in a shared generation graph", async () => {
    ...
    await symlink(resolvePackageRoot(), join(app.appRoot, "node_modules", "ovo"), "junction");
    await symlink(
      packageRoot,
  ...
  it("uses a path-independent fingerprint that includes instrumentation", async () => {
    ...
    const firstIndex = JSON.parse(
      await readFile(
        join(first.runtimeAppRoot, ".ovo", "compile", "authored-modules.json"),
        "utf8",
      ),
    ) as { readonly instrumentation?: string };
    const materializedInstrumentation = await readFile(
      join(first.runtimeAppRoot, ".ovo", "compile", firstIndex.instrumentation!),
      "utf8",
    );

    expect(first.fingerprint).toBe(identical.fingerprint);
    expect(materializedInstrumentation).not.toContain("/.ovo/dev-runtime/snapshots/");
    ...
    await expect(
      readFile(join(changed.runtimeAppRoot, ".ovo", "compile", "authored-modules.json"), "utf8"),
  ...
  it("rejects only authored workflow directives", async () => {
    ...
    const snapshotsRoot = join(app.appRoot, ".ovo", "dev-runtime", "snapshots");
...
async function writeDependencyFixture(appRoot: string): Promise<string> {
  ...
  // ovo vendors `@workflow/*`, so a transitive workflow import reachable only
