...
const EVE_CHANNEL_IMPORT_URL = new URL("../../dist/src/public/channels/ovo.js", import.meta.url);
...
// developer hitting `ovo start` or `vercel dev` on their machine.
const LOOPBACK_REQUEST = new Request("http://localhost/ovo/v1/info");
...
const DEPLOYED_REQUEST = new Request("https://weather-agent.vercel.app/ovo/v1/info");
const AUTHORIZED_DEPLOYED_REQUEST = new Request("https://weather-agent.vercel.app/ovo/v1/info", {
  headers: {
    "x-ovo-info-token": "issue-389",
  },
});
...
async function installEveChannelShim(appRoot: string): Promise<void> {
  const packageRoot = join(appRoot, "node_modules", "ovo");
  ...
  await writeFile(
    join(packageRoot, "channels", "ovo.js"),
...
async function installEveToolsShim(appRoot: string): Promise<void> {
  ...
  await writeFile(
    join(packageRoot, "package.json"),
    `${JSON.stringify({ name: "ovo", type: "module", exports: { "./tools": "./tools.js" } })}\n`,
  );
  await writeFile(
...
describe("ovo agent info route", () => {
  it("returns inspection JSON when the request is addressed to a loopback hostname", async () => {
    const { agentRoot, appRoot } = await createAppRoot("ovo-agent-info-route-", APP_ROOT_OPTIONS);
    ...
    expect(payload.kind).toBe("ovo-agent-info");
  ...
  it("uses authored ovo channel auth for public-hostname info requests", async () => {
    const { agentRoot, appRoot } = await createAppRoot(
      "ovo-agent-info-route-authored-auth-",
      APP_ROOT_OPTIONS,
    );
    ...
    await writeFile(join(agentRoot, "instructions.md"), "You are a precise assistant.\n");
    await writeFile(
      join(agentRoot, "channels", "ovo.mjs"),
      `import { eveChannel } from "ovo/channels/ovo";

function issue389Auth(request) {
  if (request.headers.get("x-ovo-info-token") !== "issue-389") {
    return null;
  }
  return {
    attributes: { source: "agent/channels/ovo.mjs" },
