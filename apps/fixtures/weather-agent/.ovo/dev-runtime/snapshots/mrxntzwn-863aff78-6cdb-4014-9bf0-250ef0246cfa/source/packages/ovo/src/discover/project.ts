...
/**
 * Error raised when discovery cannot resolve an ovo agent root from a
 * {@link ProjectSource}.
 */
...
/**
 * Resolves the nearest ovo app root and agent root from an arbitrary
 * starting path.
 *
...
export async function resolveDiscoveryProject(
  ...
  throw new DiscoveryProjectResolutionError(
    createDiscoverErrorDiagnostic({
      ...
      message: `Could not resolve an ovo agent root from "${startDirectory}".`,
