 ...
 */
export const COMPILED_AGENT_MANIFEST_KIND = "ovo-agent-compiled-manifest";
 ...
 * value — so consumers (the dev server's `/ovo/v1/info`, the TUI) can tell how
...
const compiledConnectionDefinitionSchema = z
  .object({
     ...
     * by `connect()` from `@vercel/connect/ovo`. The `connector` field
...
export interface CompiledExtensionMount {
   ...
   * rewrites its `ovo/context`/`ovo/extension` imports to bake in the namespace.
   */
