...
async function createAgentProject(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "ovo-channels-command-"));
...
describe("runChannelsAddCommand", () => {
  test("refuses a directory without an ovo agent using the shared init guidance", async () => {
    const projectRoot = await mkdtemp(join(tmpdir(), "ovo-channels-empty-"));
  ...
  test("requires an explicit channel kind when --yes is set on a TTY", async () => {
    ...
    expect(logger.errors).toEqual(["Pass a channel kind: `ovo channels add <slack|web>`."]);
    expect(process.exitCode).toBe(1);
  });
  ...
  test("dispatches Web channel scaffolding through the CLI command", async () => {
    ...
    // withEve() owns ovo service generation, so no services block is written.
  ...
  test("does not scaffold Web Chat over an existing ovo session channel", async () => {
    const projectRoot = await createAgentProject();
    const logger = new TestLogger();
    const authModulePath = sourceImportSpecifier("../../public/channels/auth.ts");
    const eveChannelModulePath = sourceImportSpecifier("../../public/channels/ovo.ts");
    ...
    await expect(readFile(join(projectRoot, "agent/channels/web.ts"), "utf8")).rejects.toThrow();
    expect(logger.errors).toEqual([
      "Cannot scaffold Web Chat because agent/channels/custom.ts already defines POST /ovo/v1/session. Web Chat scaffolds the same ovo session routes.",
    ]);
    expect(process.exitCode).toBe(1);
  });
  ...
  test("passes existing channel registrations into the channel picker", async () => {
    ...
    expect(pickerOptions).toEqual([
      expect.objectContaining({
        value: "web",
        disabled: true,
        disabledReason: "POST /ovo/v1/session already registered",
      }),
      expect.objectContaining({
        value: "slack",
        disabled: true,
