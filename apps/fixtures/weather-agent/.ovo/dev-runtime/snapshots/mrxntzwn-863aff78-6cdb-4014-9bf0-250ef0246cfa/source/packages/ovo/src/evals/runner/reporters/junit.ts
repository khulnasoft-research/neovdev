...
function renderJUnit(summary: EveEvalRunSummary, input: { readonly suiteName?: string }): string {
  ...
  return [
    ...
    `<testsuite name="${escapeXml(input.suiteName ?? "ovo evals")}" tests="${summary.results.length}" failures="${failures}" skipped="${summary.skipped}" time="${formatSeconds(durationSeconds(summary))}">`,
    ...cases,
    "</testsuite>",
    "",
  ].join("\n");
...
function renderTestCase(result: EveEvalResult): string {
  const attrs = `classname="ovo.eval" name="${escapeXml(result.id)}" time="${formatSeconds(durationSeconds(result))}"`;
 ...
 * `.ovo/evals/` artifacts; CI annotations only need the verdict and why.
 */
