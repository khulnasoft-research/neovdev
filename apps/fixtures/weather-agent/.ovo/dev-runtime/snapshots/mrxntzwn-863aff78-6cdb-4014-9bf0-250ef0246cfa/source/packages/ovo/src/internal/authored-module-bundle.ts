...
export function createAuthoredModuleBundleError(modulePath: string, error: unknown): Error {
  ...
  if (
    ...
    hints.push(
      "If this comes from an asset import, ovo only supports assets that Rolldown can emit for a Node ESM bundle; otherwise load the file through fs/import.meta.url or externalize the package.",
    );
  }
