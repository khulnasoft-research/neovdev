...
describe("schedule task routes", () => {
  it("registers virtual task handlers and cron entries for compiled schedules", () => {
    ...
    expect(nitro.options.tasks).toEqual({
      "ovo.schedule.c2NoZWR1bGVzL2RhaWx5LWRpZ2VzdC5tanM": {
        description: 'Run ovo schedule "daily-digest" from "schedules/daily-digest.mjs".',
        handler: "#ovo-schedule-task/ovo.schedule.c2NoZWR1bGVzL2RhaWx5LWRpZ2VzdC5tanM",
      },
      "ovo.schedule.c2NoZWR1bGVzL3dlZWtseS1jbGVhbnVwLm1qcw": {
        description: 'Run ovo schedule "weekly-cleanup" from "schedules/weekly-cleanup.mjs".',
        handler: "#ovo-schedule-task/ovo.schedule.c2NoZWR1bGVzL3dlZWtseS1jbGVhbnVwLm1qcw",
      },
    });
    ...
    const virtualSource =
      nitro.options.virtual["#ovo-schedule-task/ovo.schedule.c2NoZWR1bGVzL2RhaWx5LWRpZ2VzdC5tanM"];
  ...
  it("removes only ovo-owned task entries, virtual modules, and cron mappings", () => {
    const nitro = createNitroStub({
      scheduledTasks: {
        "0 8 * * *": ["ovo.schedule.example", "user-task"],
        "0 0 * * *": "ovo.schedule.alone",
      },
      tasks: {
        "ovo.schedule.example": { description: "ovo", handler: "#ovo-schedule-task/example" },
        "ovo.schedule.alone": { description: "ovo", handler: "#ovo-schedule-task/alone" },
        "user-task": { description: "user", handler: "/user/task.ts" },
      },
      virtual: {
        "#ovo-schedule-task/example": "...",
        "#ovo-schedule-task/alone": "...",
        "#user/virtual": "...",
      },
    });
