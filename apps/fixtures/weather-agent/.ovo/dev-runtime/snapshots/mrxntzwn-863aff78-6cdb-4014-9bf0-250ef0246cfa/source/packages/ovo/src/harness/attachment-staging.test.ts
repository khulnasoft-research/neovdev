...
describe("stageAttachmentsForAdapter", () => {
  ...
  it("stages a FilePart into the sandbox, rewrites data to an ovo-sandbox: ref, and preserves mediaType", async () => {
    const sandbox = mockSandbox();
    ...
    // data is an ovo-sandbox: ref URL (NOT raw bytes) — the key
  ...
  it("passes already-staged ovo-sandbox: ref parts through unchanged (idempotent)", async () => {
    const sandbox = mockSandbox();
    const existingRef = new URL(
      "ovo-sandbox:?path=%2Fworkspace%2Fattachments%2Fdeadbeef01234567%2Ffoo.png&size=42&type=image%2Fpng",
    );
    const content: UserContent = [
      {
        data: existingRef,
