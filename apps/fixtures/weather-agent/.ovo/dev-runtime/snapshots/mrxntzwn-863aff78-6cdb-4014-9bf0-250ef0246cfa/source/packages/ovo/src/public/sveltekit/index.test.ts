...
vi.mock("./vercel-json.js", () => ({
  ensureEveVercelJson: vi.fn(async () => ({ servicePrefix: "/_eve_internal/ovo" })),
}));
...
describe("eveSvelteKit", () => {
  it("configures Vite dev server proxy to a shared ovo server", async () => {
  ...
  it("configures Vite preview proxy and starts ovo for local production preview", async () => {
    const plugin = eveSvelteKit();
    const result = (await getConfigHook(plugin)(
      {
        preview: {
  ...
  it("configures vercel.json during production builds", async () => {
    const plugin = eveSvelteKit({ eveBuildCommand: "pnpm build:ovo", eveRoot: "agent" });
    ...
    expect(ensureEveVercelJsonMock).toHaveBeenCalledWith({
      ...
      servicePrefix: "/_eve_internal/ovo",
