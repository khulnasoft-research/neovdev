...
describe("eval artifacts", () => {
  it("persists skipped counts and reasons in every JSON artifact", async () => {
    await writeArtifacts("/tmp/ovo-evals", skippedSummary());

    const summary = writtenJson("/tmp/ovo-evals/summary.json");
    expect(summary).toMatchObject({
      skipped: 1,
      evals: [{ id: "runtime/skipped", skipReason: "dev routes unavailable" }],
    });

    const result = writtenJson("/tmp/ovo-evals/results.jsonl");
    expect(result).toMatchObject({
      id: "runtime/skipped",
      skipReason: "dev routes unavailable",
    });

    const detail = writtenJson("/tmp/ovo-evals/evals/runtime/skipped.json");
