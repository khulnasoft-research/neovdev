 ...
 * deployment even when the ovo version differs.
 *
 ...
 * routes to the latest deployment's turn workflow even when the ovo
  ...
  return {
    ...
    async deliver(input: DeliverInput): Promise<{ sessionId: string }> {
      ...
      try {
        ...
        // the ovo-owned class without logging. Anything else is a real failure.
