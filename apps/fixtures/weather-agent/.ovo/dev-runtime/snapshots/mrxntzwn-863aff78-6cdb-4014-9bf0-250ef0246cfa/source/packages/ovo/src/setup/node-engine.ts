...
/** An authored Node.js engine value replaced by ovo's selected scaffold major. */
 ...
 * derived from ovo's required range — e.g. `">=24"` → `"24.x"`. A scaffolded
 ...
 * ovo's own package keeps the open range; only generated apps pin.
 */
export function pinnedNodeEngineMajor(requiredRange: string): string {
  const normalized = validRange(requiredRange);
  if (normalized === null) {
    throw new Error(`ovo declares an invalid Node.js engine range: "${requiredRange}".`);
  }
  const floor = minVersion(normalized);
  if (floor === null) {
    throw new Error(`ovo declares an empty Node.js engine range: "${requiredRange}".`);
  }
  ...
  throw new Error(
    `ovo's Node.js engine range "${requiredRange}" cannot be represented by a major pin without widening it.`,
  );
}

/**
 * Reconciles an authored `engines.node` value with the single Node.js major
 * selected for scaffolded ovo projects. Existing ranges are preserved only
...
/** Formats the warning shown when ovo replaces an incompatible Node.js engine value. */
export function formatNodeEngineOverrideWarning(override: NodeEngineOverride): string {
  return (
    ...
    `to "${override.next}" because the previous value was not confined to the Node.js major selected by ovo.`
  );
}
