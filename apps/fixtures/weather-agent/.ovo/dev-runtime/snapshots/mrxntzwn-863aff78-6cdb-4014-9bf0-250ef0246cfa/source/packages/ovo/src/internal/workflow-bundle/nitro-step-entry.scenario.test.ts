...
describe("writeNitroStepEntrypoint", () => {
  it("serializes Windows absolute imports as file URLs", async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), "ovo-nitro-step-entry-windows-"));
    const outfile = join(tempRoot, "build", "steps.mjs");

    try {
      await writeNitroStepEntrypoint({
        builtinsPath: "G:\\projects\\ovo\\node_modules\\ovo\\dist\\builtins.js",
        discoveredEntries: {
        ...
        projectRoot: "G:\\projects\\ovo",
        sideEffectFiles: [],
        workingDir: "G:\\projects\\ovo",
      });

      await expect(readFile(outfile, "utf8")).resolves.toBe(
        [
          "// Generated by ovo. Do not edit by hand.",
          'import "file:///G:/projects/ovo/node_modules/ovo/dist/builtins.js";',
  ...
  it("does not rewrite the Nitro step entry when the content is unchanged", async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), "ovo-nitro-step-entry-"));
