...
description: "Trace an agent with OpenTelemetry in instrumentation.ts, read the workflow run tags ovo emits, and debug discovery with ovo info and the common-failures table."
---

`instrumentation.ts` is where you configure how an ovo agent is observed. The framework auto-discovers `agent/instrumentation.ts` and runs it at server startup before any agent code. Its presence implicitly enables telemetry, so there is no separate `isEnabled` toggle.
...
ovo observes an agent through three distinct surfaces. They do not all live in this file, and they write to different places:

| Surface                          | Configured in `instrumentation.ts`?                         | What it is                                                                                                                                                    |
| -------------------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Workflow run tags** (`$ovo.*`) | No (automatic)                                              | Framework-owned attributes on each Vercel Workflow run. Let dashboards stitch session, turn, and subagent runs into a tree and surface model and token usage. |
...
ovo creates the `ai.ovo.turn` parent span per turn and passes enriched telemetry to the AI SDK so model calls and tool executions are traced automatically. Session, turn, step, and channel context is injected as the framework half of the runtime context (`ovo.version`, `ovo.session.id`, `ovo.environment`, `ovo.turn.id`, `ovo.turn.sequence`, `ovo.step.index`, `ovo.channel.kind`) and rides onto the spans alongside any values your `events["step.started"]` callback returns under `runtimeContext`.

## Workflow run tags

Separately from OpenTelemetry, ovo tags every workflow run with reserved `$ovo.*` attributes. These live on the Vercel Workflow run, queryable in the Workflow dashboard, not on OTel spans, and you do not configure them: they are framework-owned and emitted automatically on every session, turn, and subagent run, whether or not an `instrumentation.ts` file is present. Authored code cannot set or override the `$ovo.` namespace.
...
- `$ovo.type`: `"session"`, `"turn"`, or `"subagent"`
- `$ovo.parent`: session id of the immediate parent
- `$ovo.root`: session id of the root session in the chain (group a whole tree with `$ovo.root=<id>`)
- `$ovo.subagent`: compiled graph node id (subagent runs only)
- `$ovo.trigger`: the channel kind that started the run
- `$ovo.title`: truncated title derived from the first user message

Per-turn usage tags are written on each step of a turn, accumulating cumulative totals (last write wins):

- `$ovo.model`: model id for the turn
- `$ovo.input_tokens`, `$ovo.output_tokens`, `$ovo.cache_read_tokens`: running token counts
- `$ovo.tool_count`: number of tools available to the turn
...
`ovo info` is the fastest way to see what ovo actually picked up: the active tools, skills, subagents, schedules, routes, and discovery diagnostics. ovo also writes inspectable artifacts under `.ovo/`, kept even when discovery hits errors:

| Artifact                        | Tells you                                   |
| ------------------------------- | ------------------------------------------- |
| `agent-discovery-manifest.json` | what ovo found on disk                      |
| `diagnostics.json`              | authored-shape errors and warnings          |
| `compiled-agent-manifest.json`  | the serialized surface ovo loads at runtime |
| `module-map.mjs`                | compiled module entrypoints ovo imports     |

When `ovo build` fails on discovery errors, the CLI prints the full diagnostics report (severity, message, source path) and the path to the diagnostics artifact.

### Common failures

| Symptom                                       | Likely cause and fix                                                                                                                                                                                                                                                        |
| --------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Tool not discovered (the model never sees it) | Run `ovo info`. Confirm the file is in the right slot (`agent/tools/<name>.ts`) and default-exports `defineTool(...)`, and check `.ovo/diagnostics.json` for shape errors. `schedules/` are root-only.                                                                      |
| Model won't call a tool it should             | Tighten the tool `description` and `inputSchema`; put procedural guidance in a [skill](../skills), not the description. Confirm it's in the active set with `ovo info`.                                                                                                     |
...
| Build fails with discovery errors             | Read the printed diagnostics and `.ovo/diagnostics.json`; confirm the root-vs-subagent boundary is valid and secrets come from env vars.                                                                                                                                    |
