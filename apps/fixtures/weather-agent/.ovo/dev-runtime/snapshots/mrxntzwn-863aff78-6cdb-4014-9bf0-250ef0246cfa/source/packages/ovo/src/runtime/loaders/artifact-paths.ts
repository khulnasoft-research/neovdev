...
/**
 * Resolves the stable ovo artifact paths for one application root without
...
export function resolveRuntimeCompilerArtifactPaths(appRoot: string): RuntimeCompilerArtifactPaths {
  ...
  const discoveryDirectoryPath = `${normalizedAppRoot}/.ovo/discovery`;
  const compileDirectoryPath = `${normalizedAppRoot}/.ovo/compile`;
