...
describe("executeBashOnSandbox", () => {
  ...
  it("logs sandbox command progress in dev without adding to stderr", async () => {
    ...
    expect(log).toHaveBeenCalledWith("ovo: starting sandbox command: ls -la /workspace");
    expect(log).toHaveBeenCalledWith("ovo: sandbox command finished (exit 0): ls -la /workspace");
  });
});
