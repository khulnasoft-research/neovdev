...
async function loadMaterializedCompiledModuleMap(input: {
  ...
  const moduleMapPath = join(input.runtimeAppRoot, ".ovo", "compile", input.moduleMapPath);
  ...
  if (!parsed.success) {
    throw new Error(
      `Expected materialized authored module map "${moduleMapPath}" to export a valid compiled ovo module map. ${formatValidationError(parsed.error)}`,
    );
  }
