...
async function writeRuntimeLoaderFixture(agentRoot: string): Promise<void> {
  ...
  await writeFile(
    ...
    [
      ...
      // to depend on `ovo` resolving from the temp
      // package root.
      "export default {",
      '  __kind: "ovo:channel",',
...
describe("runtime compiled artifact loaders", () => {
  it("loads resolved compaction config from compiled authored artifacts", async () => {
    const { agentRoot, appRoot } = await createAppRoot(
      "ovo-runtime-loaders-compaction-",
      APP_ROOT_OPTIONS,
    );
  ...
  it("loads the compiled manifest and module map from disk-backed compiler artifacts", async () => {
    const { agentRoot, appRoot } = await createAppRoot("ovo-runtime-loaders-", APP_ROOT_OPTIONS);
  ...
  it("loads local subagent tools and sandboxes without indexing lib in the module map", async () => {
    const { agentRoot, appRoot } = await createAppRoot(
      "ovo-runtime-loaders-subagent-lib-",
      APP_ROOT_OPTIONS,
    );
  ...
  it("rejects invalid compiled manifests", async () => {
    const { appRoot } = await createAppRoot("ovo-runtime-loader-bad-manifest-", APP_ROOT_OPTIONS);
    const manifestPath = join(appRoot, ".ovo", "compile", "compiled-agent-manifest.json");

    await mkdir(join(appRoot, ".ovo", "compile"), {
      recursive: true,
    });
    await writeFile(
      manifestPath,
      JSON.stringify({
        kind: "not-a-ovo-manifest",
      }),
    );
  ...
  it("rejects discovery manifests written to the compiled manifest path", async () => {
    const { agentRoot, appRoot } = await createAppRoot(
      "ovo-runtime-loader-wrong-manifest-",
      APP_ROOT_OPTIONS,
    );
  ...
  it("rejects invalid compiled module maps", async () => {
    const { appRoot } = await createAppRoot("ovo-runtime-loader-bad-module-map-", APP_ROOT_OPTIONS);
    const moduleMapPath = join(appRoot, ".ovo", "compile", "module-map.mjs");
