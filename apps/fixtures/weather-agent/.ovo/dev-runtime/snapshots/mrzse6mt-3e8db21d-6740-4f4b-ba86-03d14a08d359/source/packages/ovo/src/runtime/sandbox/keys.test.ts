...
function createMetadataFixture(generatorVersion: string): CompileMetadata {
  return {
    compile: {
      moduleMap: { path: ".ovo/compile/module-map.mjs", sha256: "b".repeat(64) },
    },
    discovery: {
      diagnostics: { path: ".ovo/discovery/diagnostics.json", sha256: "c".repeat(64) },
      manifest: { path: ".ovo/discovery/agent-discovery-manifest.json", sha256: "d".repeat(64) },
      sourceGraphHash: "e".repeat(64),
      summary: { errors: 0, warnings: 0 },
    },
    generator: { name: "ovo", version: generatorVersion },
...
function expectedTemplateKey(input: { scopeSource: string; version: string }): string {
  ...
  const versionHash = sha256(`workspace-content:${CONTENT_HASH}:__root__:ovo:default-sandbox`);
  const templateHash = sha256(
    `${input.version}:${RUNTIME_SANDBOX_CONTRACT_VERSION}:${versionHash}`,
  ).slice(0, 20);
  return `ovo-sbx-tpl-local-${scope}-${templateHash}`;
}
...
describe("createRuntimeSandboxKeys", () => {
  ...
  it("keeps the session key stable across unrelated source and ovo version changes", async () => {
