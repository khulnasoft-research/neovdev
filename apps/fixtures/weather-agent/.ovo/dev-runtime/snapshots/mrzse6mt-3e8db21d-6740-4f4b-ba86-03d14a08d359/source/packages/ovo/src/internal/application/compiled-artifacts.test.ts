...
describe("createWorkflowWorldPluginSource", () => {
  it("imports a configured world package and delegates its construction to Workflow", () => {
    const source = createWorkflowWorldPluginSource({
      compiledArtifactsBootstrapPath: "/app/.ovo/compile/compiled-artifacts-bootstrap.mjs",
      configuredWorld: "@acme/ovo-world",
      defaultWorld: "vercel",
    });

    expect(source).toContain('import "/app/.ovo/compile/compiled-artifacts-bootstrap.mjs";');
    expect(source).toContain('import * as workflowWorldModule from "@acme/ovo-world";');
    ...
    expect(source).toContain(
      'validateWorkflowWorld({ packageName: "@acme/ovo-world", world: workflowWorld });',
    );
  ...
  it("configures the vendored local World with ovo's app-local data resolver", () => {
    const source = createWorkflowWorldPluginSource({
      compiledArtifactsBootstrapPath: "/app/.ovo/compile/bootstrap.mjs",
  ...
  it("selects the vendored Vercel World with Workflow's selector", () => {
    ...
    expect(source).toMatch(/headers: \{ "User-Agent": "ovo\/.+" \}/);
  });
});
...
describe("createDevelopmentWorkflowWorldPluginSource", () => {
  it("installs the parent-backed World without starting a local World in the worker", () => {
    const source = createDevelopmentWorkflowWorldPluginSource({
      compiledArtifactsBootstrapPath: "/app/.ovo/host/bootstrap.mjs",
      configuredWorld: undefined,
    });
