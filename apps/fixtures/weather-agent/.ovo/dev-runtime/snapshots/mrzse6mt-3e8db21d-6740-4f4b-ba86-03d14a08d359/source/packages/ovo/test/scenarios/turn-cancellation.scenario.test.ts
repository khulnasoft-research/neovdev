...
const REMOTE_DESCRIPTOR: ScenarioAppDescriptor = {
  ...
  files: {
    "agent/agent.ts": `import { defineAgent } from "ovo";

export default defineAgent({ model: "openai/gpt-5.4-mini" });
`,
    "agent/channels/ovo.ts": `import { eveChannel } from "ovo/channels/ovo";
...
`,
    ...
    "agent/tools/wait-for-cancel.ts": `import { defineTool } from "ovo/tools";
import { z } from "zod";
...
`,
      ...
      "agent/subagents/local-sleeper/agent.ts": `import { defineAgent } from "ovo";
...
`,
      ...
      "agent/subagents/local-sleeper/tools/wait-for-cancel.ts": `import { defineTool } from "ovo/tools";
import { z } from "zod";
...
`,
      "agent/subagents/remote-sleeper.ts": `import { defineRemoteAgent } from "ovo";
import { bearer } from "ovo/agents/auth";
