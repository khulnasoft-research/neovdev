...
/**
 * Point-in-time projected state for an ovo agent session (`data`, `error`,
...
/**
 * Configuration for creating or binding a Vue ovo agent session.
 *
...
export interface UseEveAgentOptions<TData> extends EveAgentStoreCallbacks<TData> {
  ...
  /**
   * Base URL used for ovo client requests. Do not combine with `agent`.
   *
   * By default, requests target same-origin ovo routes such as `/ovo/v1/...`.
   * Pass a same-origin prefix such as `/api` to use an app-owned proxy, or an
   * absolute origin to talk to an ovo server directly.
   *
   * @default ""
   */
...
/**
 * Vue composable that drives one ovo session and projects its event stream into
 * reactive UI state.
 *
