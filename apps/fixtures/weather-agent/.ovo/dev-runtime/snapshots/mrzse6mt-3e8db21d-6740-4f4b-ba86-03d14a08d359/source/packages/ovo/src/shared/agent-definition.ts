...
/**
 * Optional overrides that ovo forwards to the AI SDK model runtime call for
 * this model.
 */
...
export interface InternalAgentCompactionDefinition {
   ...
   *
   * When omitted, ovo uses the active turn model for the summary call.
   */
  model?: InternalAgentModelDefinition;
  /**
   * Fraction of the primary model context window that triggers compaction.
   *
   * ovo defaults to `0.9` when this is omitted.
   */
 ...
 * `thresholdPercent`, ovo summarizes earlier turns to reclaim space. Every
 * field is optional; omit the block to use ovo's defaults.
 */
export interface PublicAgentCompactionDefinition {
   ...
   * ovo uses this value verbatim and skips the AI Gateway lookup for the
...
export interface AgentLimitsDefinition {
   ...
   *
   * ovo checks this before starting each model call. The model call that crosses
...
export interface AgentBuildDefinition {
  /**
   * Additional imported package names that ovo should keep external and trace
   * into hosted build output. ovo also keeps matching imports external while
...
/**
 * Advanced durable-runtime configuration for ovo's Workflow SDK integration.
 */
...
export type PublicAgentDefinition = {
   ...
   *
   * Escape hatch for cases where ovo cannot resolve the model's metadata via
   * the AI Gateway model catalog (e.g. a custom or unlisted model id). When
   * set, ovo uses this value verbatim and skips the AI Gateway lookup. Prefer
   * leaving this unset so ovo can stay in sync with provider metadata.
   */
  readonly modelContextWindowTokens?: number;
  readonly modelOptions?: AgentModelOptionsDefinition;
  /**
