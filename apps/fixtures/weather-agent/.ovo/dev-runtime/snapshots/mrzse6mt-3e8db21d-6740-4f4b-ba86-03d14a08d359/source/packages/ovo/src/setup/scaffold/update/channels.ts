...
const WEB_CHANNEL_PATH = "agent/channels/ovo.ts";
 ...
 * under (so the framework owns the top-level build and ovo runs as a sibling).
 ...
 * top-level deployment while ovo runs as a sibling service. These match Ovo's
...
function buildSlackTemplate(connectorUid: string): string {
  ...
  return `import { connectSlackCredentials } from "@vercel/connect/ovo";
 ...
 *
 * The ovo service and its `/ovo/v1/*` routes are generated by `withEve()` into
 ...
 * ovo service, so the scaffold has nothing to add to a file the user manages.
 */
