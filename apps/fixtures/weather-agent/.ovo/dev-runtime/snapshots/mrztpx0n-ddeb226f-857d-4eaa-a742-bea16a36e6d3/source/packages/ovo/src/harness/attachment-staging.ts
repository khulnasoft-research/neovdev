 ...
 * staged part to a compact `ovo-sandbox:` ref.
 *
 * Remote HTTP URLs pass through for provider-side fetches; existing
 * `ovo-sandbox:` refs pass through so staging is idempotent.
 */
...
/**
 * Hydrates `ovo-sandbox:` file refs for a single model call.
 *
...
export async function hydrateSandboxAttachments(
  ...
  if (sandboxAccess === undefined) {
    throw new Error(
      "Cannot hydrate sandbox-ref FilePart: no SandboxKey is bound on the active ovo context. " +
 ...
 * {@link FilePart} whose `data` is an `ovo-sandbox:` ref URL.
 *
...
/**
 * Reconstitutes `URL` objects from `ovo-url:` serialized strings in
 * `FilePart.data`. Before the queue boundary, `send()` serializes
 * `URL` objects as `ovo-url:{href}` strings. This pass restores them
