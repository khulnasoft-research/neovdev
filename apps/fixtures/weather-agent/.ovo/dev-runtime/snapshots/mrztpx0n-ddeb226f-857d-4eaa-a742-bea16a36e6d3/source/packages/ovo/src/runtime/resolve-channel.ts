 ...
 * (the connection callback route, the `ovo` session channel) build
...
export async function resolveChannelDefinition(
  ...
  try {
    ...
    const channelDefinition = normalizeChannelDefinition(
      resolvedExportValue,
      `Expected the channel export "${definition.exportName ?? "default"}" from "${definition.logicalPath}" to match the public ovo shape.`,
    );
