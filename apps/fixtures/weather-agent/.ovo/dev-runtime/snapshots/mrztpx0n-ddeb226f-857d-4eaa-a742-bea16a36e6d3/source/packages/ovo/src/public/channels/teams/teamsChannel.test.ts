...
async function firePost(
  ...
  const response = await post.handler(
    new Request("https://ovo.test/ovo/v1/teams", {
...
function messageActivity(input: { readonly conversationType: string }): Record<string, unknown> {
  return {
    ...
    entities: [
      {
        mentioned: { id: "BOT", name: "ovo Bot" },
        text: "<at>ovo Bot</at>",
        type: "mention",
      },
    ],
    text: input.conversationType === "personal" ? "hello" : "<at>ovo Bot</at> hello",
    textFormat: "xml",
    type: "message",
  };
}
