  ...
  it("starts turn workflows on the latest deployment in Vercel production", async () => {
    ...
    expect(startMock).toHaveBeenCalledWith(
      ...
      [createTurnWorkflowInput(input)],
      {
        ...
        attributes: {
          "$ovo.channel_request_id": "req_turn",
          "$ovo.parent": "sess-test",
          "$ovo.root": "sess-test",
          "$ovo.type": "turn",
        },
        deploymentId: "latest",
...
describe("dispatchRuntimeActionsStep", () => {
    ...
    expect(startMock).toHaveBeenCalledWith(
      ...
      [
        expect.objectContaining({
          ...
          serializedContext: expect.objectContaining({
            "ovo.channel": expect.objectContaining({
              kind: "subagent",
      ...
      ],
      {
        ...
        attributes: expect.objectContaining({
          "$ovo.parent": "parent-session",
          "$ovo.root": "root-session",
          "$ovo.type": "subagent",
        }),
        deploymentId: "latest",
      },
      ...
      vi
        ...
        .mockResolvedValue(
          Response.json(
            ...
            { headers: { "x-ovo-session-id": "remote-child" }, status: 202 },
          ),
        ),
    );
  ...
  it("blocks a stale recursive agent call from a delegated session", async () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      "[ovo:execution.dispatch-runtime-actions] recursive agent call blocked outside the root session",
      expect.objectContaining({
        callId: "call-1",
        currentDepth: 1,
        subagentName: "agent",
...
describe("turnStep", () => {
  ...
  it("clears pending authorization after a matching callback resumes the turn", async () => {
    const challenge = {
      ...
      hookUrl: "https://app.example/ovo/v1/connections/statuspage/callback/sess-test:auth",
      name: "statuspage",
      resume: { nonce: "n1" },
    };
    const session = createStubSession({
...
describe("emitTerminalSessionFailureStep", () => {
  function buildSerializedContextWithAdapter(
    ...
    serialized["ovo.sessionId"] = sessionId;
    return serialized;
  }
