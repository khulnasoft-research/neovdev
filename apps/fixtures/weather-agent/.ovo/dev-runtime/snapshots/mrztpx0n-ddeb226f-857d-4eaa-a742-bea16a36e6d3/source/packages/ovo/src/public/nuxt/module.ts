...
/**
 * Options for the ovo Nuxt module.
 */
export interface EveNuxtModuleOptions {
  /**
   * Path to the ovo application root, resolved relative to the Nuxt project
   * root unless absolute. Defaults to the Nuxt project root. The dev server is
   * spawned here, and on Vercel it is the root the generated ovo service
   * builds from.
   */
  eveRoot?: string;
  /**
   * Command that builds the ovo app inside the generated Vercel ovo service.
   * Defaults to running the installed ovo binary from the Nuxt app's
   * dependencies (`node <path-to>/ovo/bin/ovo.js build`).
   */
 ...
 * ovo service into. The full `nitro` typing lives behind the nitropack/Nuxt
...
/**
 * Resolve the destination ovo routes proxy to. In dev this is an explicit
...
/**
 * Nuxt module that wires an ovo agent into a Nuxt app. Register under `modules`
 * (configured via the `ovo` config key). It auto-imports the `useEveAgent()`
 * composable and routes ovo transport requests (`/ovo/v1/**`) to the ovo
...
export default defineNuxtModule<EveNuxtModuleOptions>({
  ...
  async setup(options, nuxt) {
    ...
    addImports({ name: "useEveAgent", from: "ovo/vue" });

    // On Vercel the ovo app deploys as a sibling service. A Nitro runtime
    // `proxy` rule can't reach it — the proxied request loops back into the
    // Nuxt function and 404s — so declare the service and route ovo transport
    ...
    if (!nuxt.options.dev && process.env.VERCEL) {
      ...
      // booting the shared ovo dev server can take a while, so defer it out of
