 ...
 * against, in order of preference. ovo compares the world's declared dependency
...
export interface AssertWorkflowWorldCompatibilityInput {
  ...
  /** The `@workflow/core` version this ovo release bundles. */
 ...
 *
 * This is intentionally tiny: ovo only needs the major number and any
 ...
 * prerelease line that is incompatible with the line this ovo release bundles.
 */
export function assertWorkflowWorldCompatibility(
  ...
  throw new Error(
    `Configured Workflow world "${input.worldPackageName}" targets ${declared.packageName} ${worldLineLabel}, ` +
      `but this ovo release requires the ${declared.packageName} ${formatExpectedLine(expectedLine)}. ` +
