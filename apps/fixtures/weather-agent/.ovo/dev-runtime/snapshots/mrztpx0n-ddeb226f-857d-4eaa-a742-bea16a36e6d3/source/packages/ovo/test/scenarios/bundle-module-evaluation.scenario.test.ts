...
/**
 * Bundles one entry against the local ovo dist into a single concatenated
 * chunk, mirroring the shape of Nitro's `_libs/ovo.mjs`.
 * Externalizes non-ovo specifiers so the test exercises ovo's evaluation
...
describe("ovo dist single-chunk module evaluation", () => {
  it("concatenates the ovo dist for a Nitro-style step entry without leaving any imported binding in TDZ", async () => {
    ...
    const scratch = await createScratchDirectory("ovo-step-entry-eval-");
  ...
  it("every framework tool definition is defined in the concatenated chunk (no silent `undefined` slots)", async () => {
    ...
    const scratch = await createScratchDirectory("ovo-framework-tools-eval-");
    const outDir = join(scratch, "out");
    await mkdir(outDir, { recursive: true });
