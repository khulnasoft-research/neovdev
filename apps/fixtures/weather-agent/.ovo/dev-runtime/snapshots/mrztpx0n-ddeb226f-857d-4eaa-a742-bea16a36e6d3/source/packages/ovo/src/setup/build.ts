...
//
// Version stamping is NOT handled here: ovo's scripts/stamp-version-tokens.mjs
...
const WEB_CHANNEL_SOURCE_PATH = "agent/channels/ovo.ts";

const FILE_TRANSFORMS: Record<string, ReadonlyArray<readonly [string, string]>> = {
  "app/_components/agent-chat.tsx": [
    ['const AGENT_NAME = "ovo-agent";', 'const AGENT_NAME = "__EVE_INIT_APP_NAME__";'],
  ],
  "app/layout.tsx": [['  title: "ovo Next.js Starter",', '  title: "__EVE_INIT_APP_NAME__",']],
  "next.config.ts": [
    [
...
async function renderGeneratedModule(): Promise<string> {
  ...
  return [
    "// Generated from apps/templates/web-chat-next by ovo's setup build (src/setup/build.ts).",
...
if (mode === "--write") {
  ...
  if (current !== generated) {
    process.stderr.write(
      "packages/ovo/src/setup/scaffold/create/web-template.ts is stale. Run `pnpm --filter ovo generate:web-template`.\n",
    );
    process.exitCode = 1;
  }
}
