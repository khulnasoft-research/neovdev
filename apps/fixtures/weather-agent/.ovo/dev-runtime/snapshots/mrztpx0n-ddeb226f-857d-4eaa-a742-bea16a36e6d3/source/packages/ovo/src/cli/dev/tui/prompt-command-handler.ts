  ...
  return {
    async handle(
      ...
      if (target.kind === "remote" && !isPromptCommandAvailableFor(command.name, "remote")) {
        return {
          message: `/${command.name} needs ovo dev running the local server (it is not available with --url).`,
        };
      }
      ...
      if (command.name === "model" && command.argument.length > 0) {
        if (target.kind !== "local") {
          return {
            message:
              "/model needs ovo dev running the local server (it is not available with --url).",
          };
        }
        ...
        try {
          ...
          // A source-backed model (an SDK model call) isn't a string literal ovo
