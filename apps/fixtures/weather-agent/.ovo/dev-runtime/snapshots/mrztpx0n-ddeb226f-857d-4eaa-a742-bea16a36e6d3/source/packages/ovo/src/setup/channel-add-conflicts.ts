...
const SCAFFOLDED_WEB_CHANNEL_LOGICAL_PATH = "channels/ovo.ts";
...
export interface ExistingChannelRegistrations {
   ...
   * imply the app: every scaffolded agent has `agent/channels/ovo.ts` for the
...
export function assertCanAddSelectedChannels(
  ...
  if (selectedChannels.includes("web")) {
    ...
    if (conflictingOwner !== undefined) {
      throw new Error(
        `Cannot scaffold Web Chat because agent/${conflictingOwner} already defines POST ${EVE_CREATE_SESSION_ROUTE_PATH}. Web Chat scaffolds the same ovo session routes.`,
      );
    }
  }
