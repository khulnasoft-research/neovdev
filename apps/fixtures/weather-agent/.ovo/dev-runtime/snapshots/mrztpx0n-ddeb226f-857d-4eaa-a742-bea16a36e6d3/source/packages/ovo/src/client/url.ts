/**
 * Builds a fetchable URL from a caller-provided host and an ovo route path.
 *
 ...
 * talk to an app-owned proxy instead of the ovo deployment directly.
 */
export function createClientUrl(
  ...
  const url = new URL(host, "http://ovo.local");
  const basePath = trimTrailingSlash(url.pathname);
  mergeSearchParams(url.searchParams, searchParams);
