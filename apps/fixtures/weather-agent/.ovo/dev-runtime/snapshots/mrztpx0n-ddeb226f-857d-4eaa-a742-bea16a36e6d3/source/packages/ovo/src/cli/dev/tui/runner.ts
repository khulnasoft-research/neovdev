...
export type AgentTUISessionOptions = {
   ...
   * Set by the runner for the first prompt when `ovo dev --input` is used.
   */
  initialDraft?: string;
...
export type EveTUIRunnerOptions = TuiDisplayOptions & {
   ...
   * know about transport-specific challenges (the `ovo dev` glue) inject
   * a richer formatter here.
   */
  formatTransportError?: (error: unknown) => string;
  /**
   * Local `ovo dev` server URL. When present, normal prompts refresh the
...
export class EveTUIRunner {
   ...
   * ovo `InputResponse[]` payloads on the next `send()`.
   */
   ...
   * header. Never throws: a missing or unauthorized `/ovo/v1/info` simply
  ...
  /**
   * Fresh `ovo init` launches the TUI with `/model` prefilled. Project-backed
...
/**
 * Reduces one ovo session-stream turn into renderer-native TUI events.
 * ovo events name assistant/reasoning sections by `turnId` + `stepIndex`;
 ...
 *
 * ovo names parts by `turnId:stepIndex`, but the harness reuses `stepIndex`
