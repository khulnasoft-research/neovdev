...
function createHost(
  ...
  return {
    ...
    compileResult: {
      ...
      project: { agentRoot: "/tmp/ovo-test/agent" },
    } as CompileAgentResult,
    compiledArtifacts: {
      bootstrapPath: `/tmp/ovo-test/.ovo/dev-hosts/${id}/bootstrap.mjs`,
      workflowWorldPluginPath: `/tmp/ovo-test/.ovo/dev-hosts/${id}/workflow-world.mjs`,
    },
    generation: {
      fingerprint: runtimeFingerprint,
      runtimeAppRoot: `/tmp/ovo-test/.ovo/dev-runtime/snapshots/${id}/source/app`,
      snapshotRoot: `/tmp/ovo-test/.ovo/dev-runtime/snapshots/${id}`,
      snapshotSourceRoot: `/tmp/ovo-test/.ovo/dev-runtime/snapshots/${id}/source`,
      sourceRoot: "/tmp/ovo-test",
    },
    scheduleRegistrations: [],
    schedules: [],
    workflowBuildDir: `/tmp/ovo-test/.ovo/dev-hosts/${id}/workflow`,
    workspaceExtensions: [],
    workspace: {
      artifactsDir: `/tmp/ovo-test/.ovo/dev-hosts/${id}/artifacts`,
      compilerArtifactsDir: `/tmp/ovo-test/.ovo/dev-hosts/${id}/compiler`,
      nitroBuildDir: `/tmp/ovo-test/.ovo/dev-hosts/${id}/nitro`,
      nitroOutputDir: `/tmp/ovo-test/.ovo/dev-hosts/${id}/output`,
      rootDir: `/tmp/ovo-test/.ovo/dev-hosts/${id}`,
      workflowBuildDir: `/tmp/ovo-test/.ovo/dev-hosts/${id}/workflow`,
    },
  };
}
...
describe("transactional authored rebuild coordinator", () => {
  it("passes changed paths into extension preparation and retains its committed watch plan", async () => {
    ...
    const extension = {
      packageRoot: "/tmp/ovo-test/packages/crm",
      buildConfigPaths: ["/tmp/ovo-test/packages/crm/tsconfig.json"],
      config: {
        sourceRoot: "/tmp/ovo-test/packages/crm/extension",
        distRoot: "/tmp/ovo-test/packages/crm/dist/extension",
        outDir: "/tmp/ovo-test/packages/crm/dist",
        packageName: "@acme/crm",
        runtimeDependencies: ["ovo"],
        shortName: "crm",
      },
    } as const;
    candidate.workspaceExtensions = [extension];
    mocks.prepareDevelopmentApplicationHost.mockResolvedValueOnce(candidate);
    const changedPaths = ["/tmp/ovo-test/packages/crm/extension/tools/search.ts"];
  ...
  it("discards the candidate and keeps prior state when the swap itself fails", async () => {
    const failingFactory: DevelopmentRunnerFactory = () => {
      const runner = createRunner({ entry: "/tmp/ovo-test/entry.mjs", name: "t", workerData: {} });
      return {
        ...runner,
        waitForReady: async () => {
