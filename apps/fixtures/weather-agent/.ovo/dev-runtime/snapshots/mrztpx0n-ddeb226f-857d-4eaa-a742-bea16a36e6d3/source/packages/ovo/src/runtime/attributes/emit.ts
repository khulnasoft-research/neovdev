...
/**
 * Writes a batch of ovo-owned attributes to the active workflow run.
 *
 * Reserved-namespace contract:
 * - All keys must use the `$ovo.` prefix (the workflow runtime would
 ...
 *   maps with optional fields (`$ovo.subagent` is only present on
 ...
 *   {@link truncateForTag} so a long free-form value (e.g. `$ovo.title`)
 ...
 * so the ovo session it tagged is unaffected.
 *
...
export async function setEveAttributes(attrs: Record<string, EveAttributeValue>): Promise<void> {
  ...
  try {
    ...
    if (!WARNED_ABOUT_TAG_FAILURE) {
      ...
      console.warn("[ovo] setEveAttributes failed; suppressing further warnings this process.", {
