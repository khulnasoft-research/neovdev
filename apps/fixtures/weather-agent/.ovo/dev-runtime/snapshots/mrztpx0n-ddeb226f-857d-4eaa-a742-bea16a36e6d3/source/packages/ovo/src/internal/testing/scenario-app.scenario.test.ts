...
describe("materializeScenarioApp", () => {
  ...
  it("writes a package.json wiring the packed ovo tarball", async () => {
    const app = await scenarioApp({
      files: {},
    ...
    expect(manifest.dependencies["ovo"]).toMatch(/^file:\.\/ovo-.*\.tgz$/);
  });
  ...
  it("installs dependencies into node_modules when requested", async () => {
    ...
    const eveManifestPath = join(app.appRoot, "node_modules", "ovo", "package.json");
