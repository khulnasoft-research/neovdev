...
async function transformAndEval(
  ...
  const defineTool = (entry: Record<string, unknown>) =>
    Object.assign(entry, { [Symbol.for("ovo:tool-brand")]: true });
...
function defineTool(entry) {
  return Object.assign(entry, { [Symbol.for("ovo:tool-brand")]: true });
}
...
`;
    ...
    expect(result).not.toBeNull();
    expect(result!.code).toContain('import { defineDynamic, defineTool } from "ovo/tools"');
    ...
    "session.started": async () => {
      ...
      return {
        tool: defineTool({
          description: "T",
          inputSchema: { type: "object" },
          execute(_input: Record<string, unknown>, ctx: import("ovo/tools").ToolContext) {
