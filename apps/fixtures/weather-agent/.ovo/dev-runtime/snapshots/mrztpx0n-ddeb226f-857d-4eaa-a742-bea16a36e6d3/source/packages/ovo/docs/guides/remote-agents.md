---
title: "Remote Agents"
description: "Call another ovo deployment as a subagent with defineRemoteAgent: same lowered tool shape, outbound auth, durable callback dispatch."
---

`defineRemoteAgent` calls a separately deployed ovo agent as if it were a local subagent. Reach for it when the specialist you delegate to is a separately owned agent behind its own URL rather than a directory in your repo.
...
import { defineRemoteAgent } from "ovo";
...
| -------------- | --------------------------------------------- | -------- | ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `url`          | `string \| (() => string \| Promise<string>)` | Yes      | n/a               | Base URL of the remote ovo deployment to call. A string is baked at compile time; a function is resolved at runtime (see [Runtime URLs](#runtime-urls)). |
| `description`  | `string`                                      | Yes      | n/a               | Model-visible delegation description.                                                                                                                    |
| `auth`         | `OutboundAuthFn`                              | No       | none              | Outbound auth hook from `ovo/agents/auth`.                                                                                                               |
| `headers`      | `HeadersValue`                                | No       | none              | Static or lazily resolved request headers.                                                                                                               |
| `path`         | `string`                                      | No       | `/ovo/v1/session` | Route appended to `url` for the create-session request.                                                                                                  |
...
A string `url` is read at compile time and frozen into the build. When the target comes from a runtime env var — known only once the deployment runs — pass a function instead. ovo calls it when it resolves the agent graph at runtime, so it can read `process.env`:
...
`auth` is an `OutboundAuthFn` from `ovo/agents/auth` that attaches request headers to the outbound dispatch:
...
If you are calling another Vercel-deployed ovo agent, reach for `vercelOidc()`. The remote verifies the OIDC token to authorize the caller. See [Auth & route protection](./auth-and-route-protection) for the receiving side.
...
1. The parent starts a task-mode session on the remote's `POST /ovo/v1/session`, passing a framework callback URL.
...
Cancelling the parent while a remote call is active sends an authenticated `POST /ovo/v1/session/:childSessionId/cancel` to the remote and waits for that request to be accepted before the parent settles. ovo resolves the remote's `headers` and `auth` again for every cancellation attempt, so rotating credentials work the same way as they do for session creation. Cancellation always uses the standard ovo cancel path on `url`, even when `path` customizes only the create-session endpoint. The remote child reports `turn.cancelled` → `session.waiting` on its own stream; an older or unreachable remote is logged but cannot turn the parent's cancellation into a failure.
