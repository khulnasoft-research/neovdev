...
export async function emitExtensionDistribution(input: {
  ...
  try {
    ...
    throw new Error(
      `TypeScript emitted no declarations for "${sourceRelativePath}". Ensure the package tsconfig.json \`include\` covers every module under \`ovo.extension.source\`.`,
      { cause: error },
    );
  }
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      ...
      if (logicalPath === EXTENSION_COMPATIBILITY_MANIFEST_FILENAME) {
        throw new Error(
          `The extension source cannot contain "${EXTENSION_COMPATIBILITY_MANIFEST_FILENAME}"; ovo reserves it for generated compatibility metadata.`,
        );
      }
...
async function writeDeclarationBarrel(input: {
  ...
  await writeFile(
    input.path,
    ["// Generated by ovo. Do not edit by hand.", "", ...lines, ""].join("\n"),
    "utf8",
  );
}
