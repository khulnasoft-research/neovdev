...
describe("chatSdkChannel", () => {
  it("mounts GET and POST webhook routes per Chat SDK adapter", () => {
    ...
    expect(
      ...
      { method: "GET", path: "/ovo/v1/test" },
      { method: "POST", path: "/ovo/v1/test" },
    ]);
  });
  ...
  it("routes GET verification challenges to the adapter webhook", async () => {
    ...
    const get = compiled.routes.find(
      (route) => route.method === "GET" && route.path === "/ovo/v1/test",
    );
    if (!get || !isHttpRouteDefinition(get)) {
      throw new Error("Expected GET /ovo/v1/test.");
    }

    const response = await get.handler(
      new Request("https://example.com/ovo/v1/test?crc_token=abc123", { method: "GET" }),
      {
        getSession: vi.fn() as any,
        resolveActiveSession: async () => undefined,
        cancel: vi.fn(),
  ...
  it("hands Chat SDK mentions to Ovo through bridge.send", async () => {
    ...
    const { response, send } = await firePost(bridge.channel, "/ovo/v1/test", {
      text: "@bot hello",
    });
  ...
  it("posts completed Ovo messages as markdown through the stored Chat SDK thread", async () => {
    const adapter = testAdapter();
    const bridge = chatSdkChannel({
      adapters: { test: adapter },
      state: memoryState(),
  ...
  it("renders input requests as Chat SDK cards and resumes on button actions", async () => {
    ...
    const { send } = await firePost(bridge.channel, "/ovo/v1/test", {
