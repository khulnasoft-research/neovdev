...
/**
 * Emitted when a resolved package is not a valid ovo extension.
 */
...
/**
 * Emitted when this ovo cannot consume one of the extension capabilities the
 ...
 * `package.json#ovo.extension.dist` for the agent-shaped distribution root.
 */
    ...
    return {
      diagnostics: unsupportedCapabilities.map((unsupported) =>
        createDiscoverErrorDiagnostic({
          ...
          message: `Extension "${location.packageName}" requires ${unsupported.capability} contract v${unsupported.requiredVersion}, but this ovo supports ${unsupported.capability} contract ${formatSupportedVersions(unsupported.supportedVersions)}. Upgrade ovo or install an extension release that requires a supported ${unsupported.capability} contract.`,
          sourcePath: compatibilityPath,
        }),
      ),
    };
...
export async function locateExtensionMountPackage(input: {
  ...
  let pkg: { name?: unknown; ovo?: { extension?: unknown } };
  try {
  ...
  if (extension === null) {
    return {
      diagnostics: [
        createDiscoverErrorDiagnostic({
          code: DISCOVER_EXTENSION_PACKAGE_INVALID,
          message: `Package "${specifier}" is not an ovo extension: its package.json must declare \`ovo.extension.dist\`.`,
          sourcePath: manifestPath,
        }),
      ],
    };
