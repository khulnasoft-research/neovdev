...
describe("addChannels box", () => {
  ...
  it("honors the configureVercelServices override over the Vercel-project gate", async () => {
    const deps = createDeps();
    // `ovo channels add` pins the services config on even when unlinked, the
  ...
  it("warns for overwritten files, an overridden node engine, and competing Next.js configs", async () => {
    ...
    expect(prompter.log.warning).toHaveBeenCalledWith(
      'Overrode package.json engines.node from "22.x" to "24.x" because the previous value was not confined to the Node.js major selected by ovo.',
    );
    expect(prompter.log.warning).toHaveBeenCalledWith(
      "Found competing Next.js config at /tmp/project/next.config.mjs; merge any needed settings into next.config.ts and remove it before starting the preview, or Next.js may ignore the generated ovo rewrite.",
    );
  });
  ...
  it("continues without Slack when creation fails under warn-and-continue", async () => {
    ...
    expect(prompter.log.warning).toHaveBeenCalledWith(
      "Slackbot creation failed. Continuing without Slack — add it later with `ovo channels add slack`.",
    );
  ...
  it("continues without Slack when attachment fails under warn-and-continue", async () => {
    ...
    expect(result.state.channels).toEqual([]);
    expect(result.state.slackbotCreated).toBe(false);
    // Not "ovo channels add slack": re-creating would orphan the connector
  ...
  it("continues without Slack when the workspace install is missing under warn-and-continue", async () => {
    ...
    expect(result.state.slackScaffolded).toBe(false);
    expect(prompter.log.warning).toHaveBeenCalledWith(
      "Slackbot is not connected to a Slack workspace. Slack channel was not added. Continuing without Slack — the install timed out and was cleaned up; re-run `ovo channels add slack` to try again.",
    );
  });
