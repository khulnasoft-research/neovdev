...
/** Injected for tests; defaults to the real ovo-scaffold helpers. */
...
export interface ScaffoldOptions {
  ...
  /** Allow the in-place scaffold to replace ovo scaffold files that already exist. */
  overwriteExisting?: boolean;
  /**
   * Headless mode: a headless re-run over an existing ovo project skips
...
/**
 * Whether the run is headless. A headless re-run over an existing ovo project
 ...
 * ran, because a headless re-run on an already-scaffolded ovo project skips the
...
export function scaffold(options: ScaffoldOptions): SetupBox<SetupState, ScaffoldInput, string> {
  ...
  return {
    ...
    async gather(): Promise<ScaffoldInput> {
      ...
      // an existing ovo project skips the write, a composition-time fact.
    ...
    async perform({ state, input }): Promise<string> {
      ...
      if (input.headless && !options.overwriteExisting && (await deps.isEveProject(projectPath))) {
        prompter.log.message("Existing ovo project detected; continuing setup...");
        return projectPath;
      }
