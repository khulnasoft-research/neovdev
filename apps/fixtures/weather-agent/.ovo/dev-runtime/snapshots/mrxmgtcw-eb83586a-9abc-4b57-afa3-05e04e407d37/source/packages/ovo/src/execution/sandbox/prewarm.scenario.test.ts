...
describe("prewarmAppSandboxes", () => {
  ...
  it("loads workspace seeds from an invocation-owned compiler directory", async () => {
    ...
    const compilerAppRoot = join(appRoot, ".ovo", "builds", "isolated", "compiler");
    await compileAgentInWorkspace({
      artifactLocations: {
        publishedRoot: join(compilerAppRoot, ".ovo"),
        writeRoot: join(compilerAppRoot, ".ovo"),
      },
      startPath: appRoot,
    });
  ...
  it("authored default override produces a single prewarm target, not two", async () => {
    ...
    expect(log.mock.calls.map(([message]) => message)).toEqual([
      "ovo: initializing 1 sandbox template...",
      "ovo: initialized 1 sandbox template (0 reused, 1 built).",
    ]);
...
async function createScenarioAppRoot(): Promise<string> {
  const appRoot = await createScratchDirectory("ovo-prewarm-");
...
async function createDefaultGraphAppRoot(
  ...
  const appRoot = await createScratchDirectory("ovo-prewarm-graph-");
  const agentRoot = join(appRoot, "agent");
...
async function createAuthoredOverrideAppRoot(
  ...
  const appRoot = await createScratchDirectory("ovo-prewarm-override-");
  const agentRoot = join(appRoot, "agent");
...
async function createSkillOnlyAppRoot(input: { readonly skillBody: string }): Promise<string> {
  const appRoot = await createScratchDirectory("ovo-prewarm-skill-only-");
  const agentRoot = join(appRoot, "agent");
...
async function createBootstrapRevalidationKeyAppRoot(input: {
  ...
  const appRoot = await createScratchDirectory("ovo-prewarm-bootstrap-revalidation-key-");
  const agentRoot = join(appRoot, "agent");
