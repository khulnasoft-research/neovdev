...
async function createTemporaryAppRoot(options?: { sourceGraphHash?: string }): Promise<string> {
  const appRoot = await createScratchDirectory("ovo-sbx-keys-");
  await mkdir(join(appRoot, ".ovo", "compile"), { recursive: true });
  await writeFile(
    join(appRoot, ".ovo", "compile", "compile-metadata.json"),
    `${JSON.stringify({
      compile: {
        moduleMap: { path: ".ovo/compile/module-map.mjs", sha256: "deadbeef" },
      },
      discovery: {
        diagnostics: { path: ".ovo/discovery/diagnostics.json", sha256: "deadbeef" },
        manifest: { path: ".ovo/discovery/agent-discovery-manifest.json", sha256: "deadbeef" },
...
describe("createRuntimeSandboxTemplateKey", () => {
  it("falls back to the realpath of the disk app root for the Vercel adapter when no project id is resolvable", async () => {
    ...
    expect(firstKey).toMatch(/^ovo-sbx-tpl-vercel-/);
  ...
  it("falls back to the bundled cache key for the Vercel adapter when no app root and no project id are available", async () => {
    ...
    expect(key).toMatch(/^ovo-sbx-tpl-vercel-/);
  });
