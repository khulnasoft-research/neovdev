...
describe("buildHomePageResponse", () => {
  ...
  it("links out to the public docs site", async () => {
    ...
    expect(body).toContain("https://khulnasoft.com/docs/ovo");
  });

  it("renders the ovo wordmark as an inline SVG", async () => {
    const body = await buildResponseForRequest("https://my-agent.example.com/").text();

    expect(body).toContain('<div class="brand" aria-label="ovo">');
    expect(body).toContain('viewBox="0 0 169 53"');
    expect(body).not.toContain('<h1 class="mono">ovo</h1>');
  });
  ...
  it("renders dollar-containing values literally", async () => {
    ...
    expect(body).toContain("ovo dev https://agent-$&amp;.example");
    expect(body).not.toContain("{{AGENT_NAME}}");
    expect(body).not.toContain("{{DEPLOYMENT_URL}}");
  });

  it("echoes the deployment origin into the `ovo dev` hint", async () => {
    const body = await buildResponseForRequest("https://my-agent.example.com/").text();

    expect(body).toContain("ovo dev https://my-agent.example.com");
    expect(body).not.toContain("ovo dev {{DEPLOYMENT_URL}}");
  });
  ...
  it("prefers x-forwarded-host / x-forwarded-proto over the raw request URL", async () => {
    ...
    expect(body).toContain("ovo dev https://agent.production.example");
  ...
  it("uses the leftmost hop from a comma-separated x-forwarded-host", async () => {
    ...
    expect(body).toContain("ovo dev https://public.example");
