...
/**
 * Base URL of a remote ovo deployment, either a static string or a function
...
/**
 * Public definition for a remote ovo agent. The compiler lowers it to a
 * subagent tool.
 */
export interface RemoteAgentDefinition {
  ...
  /**
   * Route ovo appends to `url` for the create-session request. Defaults to the
   * framework create-session route (`/ovo/v1/session`).
   */
  readonly path: string;
  /**
   * Base URL of the remote ovo deployment to call. Accepts a static string
...
/**
 * Authored input that {@link defineRemoteAgent} accepts. ovo derives identity
...
/**
 * Defines a remote ovo agent that the parent can call as a subagent tool. The
 ...
 * framework create-session route (`/ovo/v1/session`) on the target `url`.
 */
