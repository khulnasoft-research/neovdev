...
async function createTemporaryAppRoot(input: {
  ...
  await writeFile(
    ...
    `${JSON.stringify(
      {
        name: "ovo-bin-build-output-test",
        private: true,
        type: "module",
      },
      null,
...
async function runEveBuild(appRoot: string): Promise<ProcessResult> {
  return await new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      ...
      reject(
        new Error(
          `Timed out waiting for ovo build output.\nstdout:\n${stdout}\nstderr:\n${stderr}`,
        ),
      );
    }, 60_000);
...
describe("ovo build process output", () => {
  ...
  it("prints discovery diagnostics to stderr when build fails", async () => {
    const appRoot = await createTemporaryAppRoot({
      ...
      prefix: "ovo-bin-build-output-failure-",
    });
    ...
    expect(result.stderr).not.toContain(`${join(resolvedAppRoot, ".ovo", "builds")}/`);
  ...
  it("prints bundled missing-import errors to stderr when build fails", async () => {
    const appRoot = await createTemporaryAppRoot({
      prefix: "ovo-bin-build-output-missing-import-",
    });
  ...
  it("prints bundled syntax errors to stderr when build fails", async () => {
    const appRoot = await createTemporaryAppRoot({
      prefix: "ovo-bin-build-output-syntax-error-",
    });
  ...
  it("prints authored-definition shape errors to stderr when build fails", async () => {
    const appRoot = await createTemporaryAppRoot({
      prefix: "ovo-bin-build-output-definition-shape-",
    });
    ...
    expect(result.stdout).toBe("");
    expect(result.stderr).toContain(
      'Expected the tool export "default" from "tools/bad.ts" to match the public ovo shape.',
    );
    expect(result.stderr).not.toContain("Diagnostics artifact:");
  }, 120_000);
});
