...
/**
 * The one diagnostics recorder an `ovo dev` process owns: the per-process
 * JSONL log (sink), its environment dump, session-stats accumulation, and
 * ownership of ovo's structured log records. The TUI holds a single
...
export interface DevDiagnostics {
  ...
  reportStats(): void;
  /**
   * Takes ownership of ovo's structured log records: each record is
...
/**
 * Creates the diagnostics recorder for one `ovo dev` process: the
