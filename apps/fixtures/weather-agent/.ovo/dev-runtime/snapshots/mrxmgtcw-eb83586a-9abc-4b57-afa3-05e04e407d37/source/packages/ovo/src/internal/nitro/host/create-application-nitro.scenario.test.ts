...
function createPreparedHost(): PreparedDevelopmentApplicationHost {
  ...
  return {
    ...
    compiledArtifacts: {
      bootstrapPath: `${appRoot}/.ovo/bootstrap.mjs`,
      workflowWorldPluginPath: `${appRoot}/.ovo/workflow-world.mjs`,
    } as PreparedApplicationHost["compiledArtifacts"],
    scheduleRegistrations: [],
    schedules: [],
    generation: {
      fingerprint: "runtime-fingerprint",
      runtimeAppRoot: `${appRoot}/.ovo/dev-runtime/snapshots/test/source/app`,
      snapshotRoot: `${appRoot}/.ovo/dev-runtime/snapshots/test`,
      snapshotSourceRoot: `${appRoot}/.ovo/dev-runtime/snapshots/test/source`,
      sourceRoot: appRoot,
    },
    workflowBuildDir: `${appRoot}/.ovo/dev-hosts/test/workflow`,
    workspaceExtensions: [],
    workspace: {
      artifactsDir: `${appRoot}/.ovo/dev-hosts/test/artifacts`,
      compilerArtifactsDir: `${appRoot}/.ovo/dev-hosts/test/compiler`,
      nitroBuildDir: `${appRoot}/.ovo/dev-hosts/test/nitro`,
      nitroOutputDir: `${appRoot}/.ovo/dev-hosts/test/output`,
      rootDir: `${appRoot}/.ovo/dev-hosts/test`,
      workflowBuildDir: `${appRoot}/.ovo/dev-hosts/test/workflow`,
    },
  };
}
...
describe("application Nitro creation", () => {
  ...
  it("preserves workflow bundle side effects and skips workflow transform for cached bundles", async () => {
    ...
    ).find((plugin) => plugin.name === "ovo:workflow-module-side-effects");
    ...
    const cachedStepPath =
      "/Users/jj/dev/ovo/packages/ovo/.ovo/workflow-cache/hash1234567890/steps.mjs";
  ...
  it("sets the ovo framework version on Vercel app-surface build output config", async () => {
    vi.stubEnv("VERCEL", "1");
    const nitroStub = createNitroStub();
    createNitroMock.mockResolvedValueOnce(nitroStub.nitro);
  ...
  it("clears Nitro build cache output from a different ovo version", async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), "ovo-nitro-version-cache-"));
    ...
    try {
      ...
      await Promise.all([
        writeFile(
          join(nitroBuildDir, "ovo-cache.json"),
      ...
      await expect(readFile(join(nitroBuildDir, "ovo-cache.json"), "utf8")).resolves.toBe(
        `${JSON.stringify(
          {
  ...
  it("rewrites Windows paths in Nitro generated routing imports", async () => {
    ...
    ).find((plugin) => plugin.name === "ovo:nitro-routing-import-specifiers");
    ...
    expect(
      routingImportPlugin.transform(
        'import handler from "G:\\projects\\test-ovo\\dist\\route.js";',
        "#nitro/virtual/routing",
      ),
    ).toEqual({
      code: 'import handler from "file:///G:/projects/test-ovo/dist/route.js";',
      map: null,
    });
    expect(
      routingImportPlugin.transform(
        'import meta from "G:\\projects\\test-ovo\\dist\\route.js?meta";',
        "#nitro/virtual/routing-meta",
      ),
    ).toEqual({
      code: 'import meta from "file:///G:/projects/test-ovo/dist/route.js?meta";',
      map: null,
    });
    expect(
      routingImportPlugin.transform(
  ...
  it("merges framework and configured hosted dependencies", async () => {
    ...
    preparedHost.compileResult.manifest.config = {
      ...preparedHost.compileResult.manifest.config,
      build: {
        externalDependencies: ["fixture-external", "sharp", "ovo"],
      },
    ...
    for (const call of createNitroMock.mock.calls.slice(0, 3)) {
      ...
      expect(traceDeps).not.toContain("ovo");
    }
  });
  ...
  it("transforms the modules imported by the Nitro step entry", async () => {
    const nitroBuildDir = await mkdtemp(join(tmpdir(), "ovo-nitro-build-"));
    const nitroStub = createNitroStub({
      buildDir: nitroBuildDir,
    });
    createNitroMock.mockResolvedValueOnce(nitroStub.nitro);
    const workflowBuildDir = await mkdtemp(join(tmpdir(), "ovo-step-transform-"));
    ...
    try {
      ...
      ).find((plugin) => plugin.name === "ovo:workflow-step-transform");
      ...
      ).find((plugin) => plugin.name === "ovo:workflow-step-module-side-effects");
