  ...
  while (true) {
    try {
      ...
      if (Date.now() > deadline) {
        throw new Error(
          `Timed out after ${DEV_SERVER_REGISTRY_TIMEOUT_MS}ms waiting for another Nuxt process to start ovo.`,
        );
      }
 ...
 * cross-process lock so concurrent Nuxt processes don't each boot ovo.
 */
