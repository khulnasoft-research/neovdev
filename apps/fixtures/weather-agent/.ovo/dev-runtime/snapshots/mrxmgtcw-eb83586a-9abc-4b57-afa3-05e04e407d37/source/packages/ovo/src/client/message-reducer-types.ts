...
/**
 * UIMessage-compatible ovo message projection for chat and agent UIs.
 */
...
/**
 * ovo-owned message shape that follows the AI SDK UIMessage convention.
 */
 ...
 * `toolName` and `toolMetadata.ovo` ({@link EveMessageToolMetadata}) record call identity.
 */
...
/**
 * ovo-specific metadata attached to an {@link EveDynamicToolPart}. `ovo.kind`
 * classifies the action (`"tool-call"`, `"subagent-call"`, `"load-skill"`, or
 * `"unknown"`), `ovo.name` is the resolved action name, and `ovo.inputRequest`
 * and `ovo.inputResponse` store the HITL prompt and submitted response when the
