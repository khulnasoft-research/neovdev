...
/**
 * End-to-end proof that the *packed* ovo artifact can open onboarding `/model`
 * after a consumer-shaped install.
 *
 * Every other smoke test resolves ovo's modules inside the workspace, where
 ...
 *
 * Regression: ovo 0.6.x–0.7.0 imported `oxc-parser` from the `/model` flow
 * while declaring it only as a devDependency. In a scaffolded project the
 * import threw `ERR_MODULE_NOT_FOUND`, which crashed `ovo dev` with a silent
 ...
 * `npm install` resolves ovo's declared dependencies from the registry.
 */
...
void (async () => {
  const consumerRoot = await mkdtemp(join(tmpdir(), "ovo-packed-install-"));
  try {
    const tarballPath = join(consumerRoot, "ovo.tgz");
    ...
    console.log(theme.muted("[tui-packed-install] packed ovo tarball"));

    await writeFile(
      join(consumerRoot, "package.json"),
      `${JSON.stringify(
        {
          name: "ovo-packed-install-consumer",
          private: true,
          type: "module",
          dependencies: { ovo: `file:${tarballPath}` },
        },
        null,
        2,
      )}\n`,
    ...
    // `--min-release-age=0` matches ovo's own scaffold install (the packed
    ...
    const harnessPath = join(consumerRoot, "node_modules/ovo/dist/src/cli/dev/tui/test/index.js");
