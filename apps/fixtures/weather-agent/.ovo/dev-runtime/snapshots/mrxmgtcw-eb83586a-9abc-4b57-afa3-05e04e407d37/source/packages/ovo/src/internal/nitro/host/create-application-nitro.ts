...
const WORKFLOW_TRANSFORM_PATCHED = Symbol("ovo.workflow-transform-patched");
const WORKFLOW_CACHE_PATH_FRAGMENT = "/.ovo/workflow-cache/";

/**
 * Packages ovo itself pulls into hosted application output that must stay
...
function collectHostedTraceDependencies(
  ...
  // its nf3 database. traceDeps is only for ovo-owned or author-configured
...
function createDevelopmentWatchOptions(appRoot: string): { ignored: string[] } {
  return {
    // ovo's authored-source watcher owns app code rebuilds. If Nitro/Rollup
...
function addWorkflowModuleSideEffectsPlugin(nitro: Nitro, workflowBuildDir: string): void {
  ...
  nitro.hooks.hook("rollup:before", (_nitro, config) => {
    ...
    config.plugins.unshift({
      name: "ovo:workflow-module-side-effects",
...
function addNitroStepModuleSideEffectsPlugin(
  ...
  nitro.hooks.hook("rollup:before", (_nitro, config) => {
    ...
    config.plugins.unshift({
      name: "ovo:workflow-step-module-side-effects",
...
function addNitroStepTransformPlugin(
  ...
  nitro.hooks.hook("rollup:before", (_nitro, config) => {
    ...
    config.plugins.unshift({
      ...
      name: "ovo:workflow-step-transform",
    });
  });
...
function addDynamicToolTransformPlugin(nitro: Nitro): void {
  nitro.hooks.hook("rollup:before", (_nitro, config) => {
    ...
    config.plugins.unshift({
      ...
      name: "ovo:dynamic-tool-transform",
    });
  });
}
...
function addInstrumentationModuleSideEffectsPlugin(
  ...
  nitro.hooks.hook("rollup:before", (_nitro, config) => {
    ...
    config.plugins.unshift({
      name: "ovo:instrumentation-module-side-effects",
...
/**
 * Extends the Workflow Nitro transform exclusion list to include ovo's
...
/**
 * Creates one isolated Nitro host candidate for `ovo dev`.
 */
