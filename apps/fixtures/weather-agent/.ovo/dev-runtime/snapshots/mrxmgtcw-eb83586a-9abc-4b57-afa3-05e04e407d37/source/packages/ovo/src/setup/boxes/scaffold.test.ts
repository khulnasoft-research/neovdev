...
describe("scaffold box", () => {
  ...
  it("headless re-run on an existing ovo project skips scaffolding and continues", async () => {
    ...
    expect(prompter.log.message).toHaveBeenCalledWith(
      "Existing ovo project detected; continuing setup...",
    );
    expect(next.projectPath).toEqual({ kind: "resolved", inPlace: false, path: "/tmp/my-agent" });
  });

  it("interactive run scaffolds even when the directory is already an ovo project", async () => {
    const deps = fakeDeps({ isEveProject: vi.fn(async () => true) });
  ...
  it("headless overwriteExisting re-scaffolds an existing ovo project and warns per overwrite", async () => {
    const deps = fakeDeps({ isEveProject: vi.fn(async () => true) });
    const prompter = createPrompter();
    const box = scaffold({
      prompter,
