 ...
 * (model id, instructions, tools, skills, etc.) lives behind the resolved ovo
 * channel auth policy at `/ovo/v1/info`.
 *
 ...
 * echoes the visitor's own request origin back into the `$ ovo dev …`
...
<body>
<main>
  <div class="brand" aria-label="ovo">${EVE_LOGO_SVG}</div>
  <section class="panel" aria-label="Agent status">
    ...
    <div class="terminal mono" role="group" aria-label="Send a message from your terminal">
      ...
      <span class="terminal-cmd">ovo dev ${DEPLOYMENT_URL_PLACEHOLDER}</span>
    </div>
  </section>
</main>
</body>
 ...
 * `host` header, then to `request.url` for local `ovo dev` runs that
...
export default function handleStaticHomePageRequest(event: H3Event): Response {
  return buildHomePageResponse({ agentName: "ovo" }, event.req);
}
