 ...
 *
 * Authors as inline `__kind: "ovo:channel"` objects in `.mjs` so the
 * test compiles without an ovo install. The compiler's
...
describe("cross-channel receive end-to-end", () => {
  ...
  it("throws when the passed channel reference is not registered in this agent", async () => {
    const { agentRoot, appRoot } = await createAppRoot(
      "ovo-cross-channel-receive-mismatch-",
      APP_ROOT_OPTIONS,
    );
    ...
    await withRuntimeSession(createRuntimeSession("cross-channel-receive-mismatch"), async () => {
      ...
      const stranger = { __kind: "ovo:channel", routes: [], adapter: { kind: "x" } };
