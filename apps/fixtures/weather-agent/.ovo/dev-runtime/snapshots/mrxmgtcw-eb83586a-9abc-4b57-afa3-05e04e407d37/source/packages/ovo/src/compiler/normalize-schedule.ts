  ...
  const definition: ScheduleDefinition =
    source.sourceKind === "markdown"
      ? normalizeScheduleDefinition(
          source.definition,
          `Expected the compiled schedule definition at "${source.logicalPath}" to match the public ovo shape.`,
        )
      : normalizeScheduleDefinition(
          ...
          `Expected the schedule export "${source.exportName ?? "default"}" from "${source.logicalPath}" to match the public ovo shape.`,
        );
