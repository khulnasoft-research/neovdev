...
import { ensureBuiltCli, runEveCli } from "../bin/ovo.js";

const bootstrapOptions = {
  cliEntrypointPath: "/workspace/packages/ovo/dist/src/cli/run.js",
  packageRoot: "/workspace/packages/ovo",
  postBuildScriptPaths: [
    "/workspace/packages/ovo/scripts/copy-compiled-assets.mjs",
    "/workspace/packages/ovo/scripts/copy-docs.mjs",
    "/workspace/packages/ovo/scripts/stamp-version-tokens.mjs",
  ],
...
describe("ovo CLI bootstrap", () => {
  it("fails before bootstrapping when Node.js is older than 24", async () => {
    ...
    await expect(
      ...
      "ovo requires Node.js >=24. You are running v23.11.0. " +
  ...
  it.each(["v24.0.0-rc.1", "v24garbage"])(
    "rejects %s when it does not satisfy ovo's package engine",
    async (nodeVersion) => {
      const exists = vi.fn(async () => true);
      const importModule = vi.fn(async () => ({
        runCli: vi.fn(async () => {}),
  ...
  it("enforces upper bounds and gaps in the package engine range", async () => {
    ...
    ).rejects.toThrow("ovo requires Node.js >=24 <26. You are running v26.0.0.");
  ...
  it("imports the compiled CLI when build output already exists", async () => {
    ...
    expect(runCommand).not.toHaveBeenCalled();
    expect(importModule).toHaveBeenCalledWith("file:///workspace/packages/ovo/dist/src/cli/run.js");
  ...
  it("fails when the CLI is missing from a packaged install", async () => {
    ...
    await expect(
      ...
      `ovo package at ${bootstrapOptions.packageRoot} does not include the sources required to rebuild the CLI.`,
    );
  ...
  it("fails when a bootstrap build does not create the CLI entrypoint", async () => {
    ...
    ).rejects.toThrow(`Building ovo did not produce ${bootstrapOptions.cliEntrypointPath}.`);
  });
});
