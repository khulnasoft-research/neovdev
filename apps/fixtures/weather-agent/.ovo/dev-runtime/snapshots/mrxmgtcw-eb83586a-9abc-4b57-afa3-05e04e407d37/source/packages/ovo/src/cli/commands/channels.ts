...
/**
 * `ovo channels add` composes the channel picker, scaffold, and deploy boxes.
...
async function runAddChannelsFlow(
  ...
  if (kind === undefined && (options.yes || !process.stdin.isTTY || !process.stdout.isTTY)) {
    throw new Error(
      `Pass a channel kind: \`ovo channels add <${KNOWN_CHANNEL_KINDS.join("|")}>\`.`,
    );
  }

  const prompter = dependencies.createPrompter?.() ?? createPrompter();
  prompter.intro("Add channels to your ovo agent");
...
export async function runChannelsListCommand(
  ...
  if (channels.length === 0) {
    logger.log("No channels defined. Run `ovo channels add` to add one.");
    return;
  }
