...
/**
 * ovo-owned schema contract for tool input and output schemas: a Standard
...
function rehydrateJsonSchema(jsonSchema: JsonObject): ToolSchema {
  ...
  try {
    ...
    console.warn(
      "[ovo] Tool schema uses JSON Schema features outside local validation support; " +
 ...
 * version key is always stripped so every ovo boundary carries one canonical
 * wire form.
 */
