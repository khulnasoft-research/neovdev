...
describe("encodeAttachmentRef", () => {
  it("builds an ovo-attachment: URL with a v=1 version and a base64url JSON payload", () => {
...
describe("parseAttachmentRef", () => {
  ...
  it("rejects non-ovo-attachment schemes", () => {
    expect(() => parseAttachmentRef(new URL("https://files.slack.com/x"))).toThrow(
      /ovo-attachment:/u,
    );
    expect(() => parseAttachmentRef(new URL("data:,hello"))).toThrow(/ovo-attachment:/u);
  });

  it("rejects a missing or unrecognized wire version", () => {
    expect(() => parseAttachmentRef(new URL("ovo-attachment:?p=eyJwYXJhbXMiOnt9fQ"))).toThrow(
      /wire format version/u,
    );
    expect(() => parseAttachmentRef(new URL("ovo-attachment:?v=2&p=eyJwYXJhbXMiOnt9fQ"))).toThrow(
      /wire format version/u,
    );
  });

  it("rejects a missing payload", () => {
    expect(() => parseAttachmentRef(new URL("ovo-attachment:?v=1"))).toThrow(/"p" payload/u);
  });

  it("rejects a payload that is not base64url-encoded JSON", () => {
    expect(() => parseAttachmentRef(new URL("ovo-attachment:?v=1&p=!!!notjson!!!"))).toThrow(
  ...
  it("rejects a payload that decodes to a non-object", () => {
    ...
    expect(() => parseAttachmentRef(new URL(`ovo-attachment:?v=1&p=${encoded}`))).toThrow(
      /JSON object/u,
    );
  });
...
describe("isAttachmentRefUrl", () => {
  it("returns true only for URL instances with the ovo-attachment scheme", () => {
  ...
  it("rejects string inputs even when they look like attachment refs", () => {
    expect(isAttachmentRefUrl("ovo-attachment:?v=1&p=eyJwYXJhbXMiOnt9fQ")).toBe(false);
  });
