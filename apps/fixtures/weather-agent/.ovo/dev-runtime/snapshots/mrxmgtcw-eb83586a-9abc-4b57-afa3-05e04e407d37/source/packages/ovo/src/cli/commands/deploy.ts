...
/**
 * `ovo deploy`: deploy the agent to Vercel production. An already-linked
 ...
 * refuses with `ovo link` guidance otherwise. The flow itself is
...
export async function runDeployCommand(
  ...
  prompter.intro("Deploy your ovo agent to Vercel");
  try {
    ...
    if (result.kind === "needs-link") {
      logger.error(
        "This directory is not linked to a Vercel project. Run `ovo link` first (or `vercel link --project <name> --yes --non-interactive` in CI), then re-run `ovo deploy`.",
      );
      process.exitCode = 1;
      return;
    }
