...
describe("workflowEntryReference", () => {
  it("uses the installed ovo package identity for the runtime workflow id", () => {
    ...
    // deployment even when ovo itself has been upgraded.
...
describe("createWorkflowRuntime#run", () => {
      ...
      [
        {
          ...
          serializedContext: expect.objectContaining({
            "ovo.bundle": { source: compiledArtifactsSource },
            "ovo.channel": { kind: "http", state: {} },
            "ovo.mode": "task",
          }),
        },
      ],
  ...
  it("uses an explicit title without changing the workflow model input", async () => {
    ...
    expect(startOptions.attributes["$ovo.title"]).toBe("ship it");
  });
  ...
  it("serializes the channel request id into workflow context", async () => {
    ...
    expect(startMock).toHaveBeenCalledWith(
      ...
      ],
      {
        allowReservedAttributes: true,
        attributes: {
          "$ovo.channel_request_id": "req_run",
          "$ovo.title": "hello",
          "$ovo.trigger": "http",
          "$ovo.type": "session",
        },
  ...
  it("seeds subagent lineage attributes when starting a delegated session", async () => {
    ...
    expect(startMock).toHaveBeenCalledWith(workflowEntryReference, expect.any(Array), {
      ...
      attributes: {
        ...
        "$ovo.parent_call": "call-1",
        "$ovo.parent_turn": "turn-1",
        "$ovo.root": "root-session",
        "$ovo.subagent": "researcher",
        "$ovo.trigger": "subagent",
        "$ovo.type": "subagent",
      },
    });
  });
