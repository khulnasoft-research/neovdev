/**
 * Local access point for the `ovo dev` terminal-UI test harness used by the
 * `tui-*.ts` smoke tests. The harness is test infrastructure, not part of
 * ovo's public API, so it is intentionally absent from the package's
 * `exports` map; TUI smoke tests reach the built file directly by path instead.
 *
 * Requires `pnpm run build:js` to have produced `packages/ovo/dist` first.
 */
export {
