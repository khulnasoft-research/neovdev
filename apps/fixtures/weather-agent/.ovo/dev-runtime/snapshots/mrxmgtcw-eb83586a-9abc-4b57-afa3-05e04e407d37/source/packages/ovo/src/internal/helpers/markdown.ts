...
export function lowerInstructionsMarkdown(markdown: string): InstructionsDefinition {
  return normalizeInstructionsDefinition(
    { markdown },
    "Expected authored instructions markdown to match the public ovo shape.",
  );
}
...
export function lowerScheduleMarkdown(source: string): ScheduleDefinition {
  ...
  return defineSchedule(
    normalizeScheduleDefinition(
      rawDefinition,
      "Expected authored schedule markdown to match the public ovo shape.",
    ),
  );
}
...
export function lowerSkillMarkdown(
  ...
  return defineSkill(
    normalizeSkillDefinition(
      rawDefinition,
      "Expected authored skill markdown to match the public ovo shape.",
    ),
  );
}
 ...
 * ovo deliberately does not consume. Skill identity is path-derived, so an
 * authored `name` is meaningless to ovo; we drop it silently rather than
