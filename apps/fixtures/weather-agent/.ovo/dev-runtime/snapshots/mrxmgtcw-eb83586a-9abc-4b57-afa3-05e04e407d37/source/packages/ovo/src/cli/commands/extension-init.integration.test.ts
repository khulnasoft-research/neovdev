...
describe("runExtensionInitCommand", () => {
  it("scaffolds an extension package and prints next steps without starting ovo dev", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-extension-init-"));
    const output = logger();
    ...
    const packageJson = JSON.parse(await readFile(join(projectPath, "package.json"), "utf8")) as {
      ...
      devDependencies?: { ovo?: string; typescript?: string };
      dependencies?: { zod?: string; ai?: string };
      scripts?: Record<string, string>;
    };
    expect(packageJson.ovo?.extension).toEqual({
      source: "./extension",
      dist: "./dist/extension",
    });
    expect(packageJson.files).toEqual(["dist"]);
    expect(packageJson.peerDependencies?.ovo).toBe("*");
    expect(packageJson.peerDependenciesMeta).toBeUndefined();
    expect(packageJson.devDependencies?.ovo).toBe("0.6.0");
    expect(packageJson.dependencies?.zod).toBe("4.0.0");
    expect(packageJson.dependencies?.ai).toBeUndefined();
    expect(packageJson.scripts?.build).toBe("ovo extension build");
    expect(packageJson.scripts?.prepare).toBe("ovo extension build");
    ...
    expect(printed).toContain("Created an ovo extension in ");
  ...
  it("scaffolds an extension for a coding agent with a named target", async () => {
    ...
    expect(printed).not.toContain("Set up an ovo agent");
  });

  it("hands a coding agent the extension setup guide when the target is omitted", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-extension-init-bare-"));
    const output = logger();
    const deps = dependencies();
    deps.isCodingAgentLaunch.mockResolvedValue(true);
    ...
    const printed = output.messages.join("\n");
    expect(printed).toContain("npx ovo@latest extension init <name>");
    expect(printed).toContain("does not start ovo dev");
    expect(printed).toContain("ovo extension build");
  });

  it("rejects an existing project with package.json", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-extension-init-existing-"));
  ...
  it("honors EVE_INIT_PACKAGE_SPEC for the dev dependency while keeping a wildcard peer", async () => {
    const parentDirectory = await mkdtemp(join(tmpdir(), "ovo-extension-init-spec-"));
    const output = logger();
    const deps = dependencies();
    vi.stubEnv(EVE_INIT_PACKAGE_SPEC_ENV, "file:/tmp/ovo-local.tgz");
    ...
    const packageJson = JSON.parse(
      ...
      peerDependencies?: { ovo?: string };
      devDependencies?: { ovo?: string };
    };
    expect(packageJson.peerDependencies?.ovo).toBe("*");
    expect(packageJson.devDependencies?.ovo).toBe("file:/tmp/ovo-local.tgz");
  });
});
