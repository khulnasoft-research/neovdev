...
/**
 * Starts the authored-source watcher used by `ovo dev`.
 *
    ...
    queue = queue
      .then(async () => {
        ...
        try {
          ...
          console.error(`[ovo:dev] rebuild failed: ${toErrorMessage(error)}`);
        }
      })
      .catch((error) => {
        console.error(`[ovo:dev] rebuild queue error: ${toErrorMessage(error)}`);
      });
    await queue;
  };
  const flush = async () => {
