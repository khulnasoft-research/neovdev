...
/**
 * Handle to a spawned `ovo dev --no-ui` process serving a scenario app.
 */
...
/**
 * Spawns `ovo dev --no-ui` for the app and resolves once the server URL is
...
export async function startEveDev(
  ...
  try {
    ...
    await waitForPath(join(appRoot, ".ovo", "dev-server-state.v1.json"));
  } catch (error) {
...
/**
 * Spawns `ovo dev` and resolves with the process exit for flows that expect
...
export async function runEveDevToExit(
  ...
  try {
    return await withinDeadline(
      ...
      `Timed out waiting for ovo dev to exit.\n\noutput:\n${output}`,
      120_000,
    );
  } finally {
    if (child.exitCode === null && child.signalCode === null) {
...
function spawnEveDev(
  ...
  const eveBinPath = join(appRoot, "node_modules", "ovo", "bin", "ovo.js");
  ...
  return await new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      settleReject(
        new Error(
          [
            "Timed out waiting for ovo dev to print its server URL.",
    ...
    function handleOutput() {
      ...
      if (hasKnownDevServerFailure(combinedOutput)) {
        settleReject(
          new Error(
            [
              "ovo dev emitted a known reload or generated-bundle failure.",
              `stdout:\n${output.stdout}`,
              `stderr:\n${output.stderr}`,
            ].join("\n\n"),
          ),
    ...
    function handleExit(code: number | null, signal: NodeJS.Signals | null) {
      ...
      settleReject(
        new Error(
          [
            `ovo dev exited before printing its server URL (code ${String(code)}, signal ${String(signal)}).`,
            `stdout:\n${output.stdout}`,
            `stderr:\n${output.stderr}`,
          ].join("\n\n"),
        ),
