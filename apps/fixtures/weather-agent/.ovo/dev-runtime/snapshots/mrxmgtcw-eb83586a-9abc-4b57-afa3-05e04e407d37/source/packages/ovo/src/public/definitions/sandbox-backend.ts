 ...
 * provisioned. Run `ovo build` or call `prewarmAppSandboxes()` before
...
export class SandboxTemplateNotProvisionedError extends Error {
  ...
  constructor(input: { readonly backendName: string; readonly templateKey: string }) {
    super(
      `Sandbox template "${input.templateKey}" is not provisioned for backend "${input.backendName}". Run \`ovo build\` or invoke \`prewarmAppSandboxes()\` before serving traffic.`,
    );
