...
const buildNitroMock = vi.fn(async (nitro: Nitro) => {
    ...
    `${JSON.stringify(
      {
        routes: [
          { handle: "filesystem" },
          { dest: "/ovo/v1/health", src: "/ovo/v1/health" },
          {
            dest: "/ovo/v1/session/[sessionId]/stream",
            src: "^/ovo/v1/session/(?<sessionId>[^/]+)/stream$",
          },
          { dest: "/index", src: "/" },
          { dest: "/__server", src: "/(.*)" },
        ],
  ...
  await mkdir(join(outputDir, "functions", "ovo", "v1"), { recursive: true });
  await symlink("./__server.func", join(outputDir, "functions", "index.func"), "dir");
  await symlink(
    "./../../__server.func",
    join(outputDir, "functions", "ovo", "v1", "health.func"),
    "dir",
  );
});
...
function createPreparedHost(appRoot: string): PreparedApplicationHost {
  ...
  return {
    ...
    compiledArtifacts: {
      bootstrapPath: join(appRoot, ".ovo", "compile", "compiled-artifacts-bootstrap.mjs"),
    ...
    schedules: [],
    workflowBuildDir: join(appRoot, ".ovo", "workflow-cache"),
  };
}
...
describe("buildApplication", () => {
  ...
  it("builds without publishing stable runtime compiler artifacts", async () => {
    vi.stubEnv("VERCEL", "");
    const appRoot = await createScratchDirectory("ovo-build-application-single-");
    ...
    await Promise.all([
      writeFile(join(outputDir, "ovo-cache.json"), `${JSON.stringify({ eveVersion: "old" })}\n`),
    ...
    expect(createProductionApplicationNitroMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        buildDir: expect.stringContaining(join(appRoot, ".ovo", "builds")),
        outputDir: expect.stringContaining(join(appRoot, ".ovo", "builds")),
      }),
    );
    await expect(readFile(staleOutputPath, "utf8")).rejects.toThrow();
    await expect(readFile(join(outputDir, "ovo-cache.json"), "utf8")).resolves.toBe(
      `${JSON.stringify(
        {
    ...
    await expect(
      readFile(join(appRoot, ".ovo", "compile", "compiled-agent-manifest.json"), "utf8"),
    ).rejects.toThrow();
  ...
  it("writes a versioned timing and output-size profile outside the published output", async () => {
    vi.stubEnv("VERCEL", "");
    const appRoot = await createScratchDirectory("ovo-build-application-profile-");
    const profilePath = join(appRoot, ".ovo", "profiles", "build.json");
  ...
  it("keeps a profile path inside the published output from failing or changing a build", async () => {
    vi.stubEnv("VERCEL", "");
    const appRoot = await createScratchDirectory("ovo-build-application-profile-output-");
  ...
  it("keeps profile write errors from failing a completed build", async () => {
    vi.stubEnv("VERCEL", "");
    const appRoot = await createScratchDirectory("ovo-build-application-profile-write-error-");
    ...
    try {
      ...
      expect(outputDir).toBe(join(appRoot, ".output"));
      await expect(readFile(join(outputDir, "ovo-cache.json"), "utf8")).resolves.toContain(
        "eveVersion",
      );
      await expect(readFile(profilePath, "utf8")).rejects.toThrow();
      expect(warn).toHaveBeenCalledWith(
  ...
  it("keeps the last-good output when Nitro mutates its target before failing", async () => {
    vi.stubEnv("VERCEL", "");
    const appRoot = await createScratchDirectory("ovo-build-application-last-good-");
    const outputDir = join(appRoot, ".output");
  ...
  it("builds isolated Vercel Nitro surfaces and stitches workflow functions", async () => {
    vi.stubEnv("VERCEL", "1");
    const appRoot = await createScratchDirectory("ovo-build-application-vercel-");
    const profilePath = join(appRoot, ".ovo", "profiles", "vercel-build.json");
    const stableFlowOutputDir = join(appRoot, ".ovo", "nitro-output", "flow");
    ...
    await Promise.all([
      writeFile(
        join(stableFlowOutputDir, "ovo-cache.json"),
    ...
    expect(flowOutputDir).toEqual(expect.stringContaining(join(appRoot, ".ovo", "builds")));
    ...
    const nestedFunctionStats = await lstat(
      join(appRoot, ".vercel", "output", "functions", "ovo", "v1", "health.func"),
    );
    const sharedFunctionStats = await lstat(
      join(appRoot, ".vercel", "output", "functions", "ovo", "__server.func"),
    );
    ...
    await expect(
      realpath(join(appRoot, ".vercel", "output", "functions", "ovo", "v1", "health.func")),
    ).resolves.toBe(
      await realpath(join(appRoot, ".vercel", "output", "functions", "ovo", "__server.func")),
    );
    await expect(
      readFile(
        join(appRoot, ".vercel", "output", "functions", "ovo", "v1", "health.func", "_runtime.mjs"),
        "utf8",
      ),
    ).resolves.toContain("export default");
    expect(vercelConfig.routes).toEqual([
      { handle: "filesystem" },
      { dest: "/ovo/__server", src: "/ovo/v1/health" },
      {
        dest: "/ovo/__server",
        src: "^/ovo/v1/session/(?<sessionId>[^/]+)/stream$",
      },
  ...
  it("skips Vercel sandbox prewarm only when the build opts out", async () => {
    vi.stubEnv("VERCEL", "1");
    const appRoot = await createScratchDirectory("ovo-build-application-skip-prewarm-");
  ...
  it("normalizes ovo function output behind a non-Next host service", async () => {
    vi.stubEnv("VERCEL", "1");
    const appRoot = await createScratchDirectory("ovo-build-application-vercel-nuxt-");
    const flowOutputDir = join(appRoot, ".ovo", "nitro-output", "flow");
  ...
  it("normalizes ovo function output behind a host service from a service array", async () => {
    vi.stubEnv("VERCEL", "1");
    const appRoot = await createScratchDirectory("ovo-build-application-vercel-service-array-");
  ...
  it("resolves service roots relative to a linked Vercel root directory", async () => {
    vi.stubEnv("VERCEL", "1");
    const projectRoot = await createScratchDirectory("ovo-build-application-vercel-root-dir-");
  ...
  it("builds isolated Vercel Nitro surfaces from legacy root service config", async () => {
    vi.stubEnv("VERCEL", "1");
    const appRoot = await createScratchDirectory("ovo-build-application-vercel-root-config-");
    const flowOutputDir = join(appRoot, ".ovo", "nitro-output", "flow");
  ...
  it("leaves standalone Vercel Nitro output routable at the root", async () => {
    vi.stubEnv("VERCEL", "1");
    const appRoot = await createScratchDirectory("ovo-build-application-vercel-standalone-");
