...
describe("vercelGatewayFetch", () => {
  it("sends the ovo product token as the user-agent", async () => {
    ...
    expect(new Headers(init?.headers).get("user-agent")).toMatch(/^ovo\/.+/);
  });
});

describe("resolveProviderHeaders", () => {
  it("returns the ovo user-agent for bare model ids", () => {
    expect(resolveProviderHeaders("anthropic/claude-sonnet-4-5")).toEqual({
      "user-agent": expect.stringMatching(/^ovo\/.+/),
    });
  });

  it("returns the ovo user-agent for gateway model instances", () => {
