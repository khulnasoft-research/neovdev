...
const EVE_SERVICE_ROUTE_SRC = "^/ovo/v1/(.*)$";
const EVE_SERVICE_ROUTE_PATH = "/ovo/v1/$1";
...
function findConfiguredEveServiceEntry(
  ...
  if (agent.name !== undefined) {
    ...
    if (namedService?.service.framework === "ovo") {
      return namedService;
    }
  }

  return agent.name === undefined ? findServiceEntryByFramework(services, "ovo") : undefined;
}
...
function assertRootServicesIncludeEve(input: {
  ...
  for (const agent of input.agents) {
    ...
    if (configuredEveService === undefined) {
      throw new Error(
        `${VERCEL_JSON_FILE_NAME} already defines services, so withEve cannot add generated ovo services through ${VERCEL_OUTPUT_CONFIG_FILE_NAME}. Add the ovo service for ${agent.name ?? "the default agent"} to ${VERCEL_JSON_FILE_NAME}, or remove services from ${VERCEL_JSON_FILE_NAME}.`,
      );
    }
...
function createEveServiceRouteSrc(publicRoutePrefix: string): string {
  ...
  const normalizedPrefix = publicRoutePrefix.startsWith("/")
    ? publicRoutePrefix
    : `/${publicRoutePrefix}`;
  return `^${escapeRegExp(normalizedPrefix)}/ovo/v1/(.*)$`;
}
