...
describe("resolveEveAgentHost", () => {
  it("defaults to same-origin root ovo routes", () => {
  ...
  it("maps a named agent to the same-origin named route prefix", () => {
    expect(resolveEveAgentHost({ agent: "support" })).toBe("/ovo/agents/support");
  });
  ...
  it("rejects names that are not safe route segments", () => {
    expect(() => resolveEveAgentHost({ agent: "Support" })).toThrow("ovo agent name");
  });
});
