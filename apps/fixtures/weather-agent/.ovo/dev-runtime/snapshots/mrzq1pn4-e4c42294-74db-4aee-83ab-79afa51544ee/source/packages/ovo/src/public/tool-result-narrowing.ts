 ...
 */
const DEFINITION_KEY = Symbol.for("ovo.definition-source-key");
 ...
 */
const REGISTRY_SYMBOL = Symbol.for("ovo.definition-source-registry");
...
export function registerDefinitionSource(key: string, entry: DefinitionSourceEntry): void {
  ...
  if (existing !== undefined && !sameDefinitionSourceEntry(existing, entry)) {
    if (existing.kind !== "ambiguous") {
      console.warn(
        [
          `ovo could not assign a unique toolResultFrom identity for ${JSON.stringify(key)}.`,
          ...
          "Use the original definition object loaded by ovo so source-derived identity can be used instead.",
        ].join(" "),
      );
    }
