...
describe("application host preparation", () => {
  ...
  it("keeps production compiler and host writes inside one invocation workspace", async () => {
    const { agentRoot, appRoot } = await createAppRoot("ovo-production-host-workspace-", {
      files: {
    ...
    try {
      ...
      expect(existsSync(join(appRoot, ".ovo", "compile"))).toBe(false);
      expect(existsSync(join(appRoot, ".ovo", "host"))).toBe(false);
    } finally {
  ...
  it("selects the Vercel Workflow world for a prebuilt production host", async () => {
    ...
    const { agentRoot, appRoot } = await createAppRoot("ovo-vercel-production-world-", {
      files: {
        "agent/instructions.md": "Use the configured model.",
      },
  ...
  it("keeps Nitro host inputs outside retained runtime generations", async () => {
    const { agentRoot, appRoot } = await createAppRoot("ovo-stable-dev-host-artifacts-", {
      files: {
        "agent/instructions.md": "Use the configured model.",
      },
    ...
    expect(firstHost.compiledArtifacts.bootstrapPath).not.toContain("/.ovo/dev-runtime/snapshots/");
