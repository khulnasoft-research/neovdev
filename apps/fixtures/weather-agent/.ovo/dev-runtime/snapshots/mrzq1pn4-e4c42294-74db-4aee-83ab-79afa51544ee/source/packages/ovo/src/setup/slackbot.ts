 ...
 * ovo attaches this project's trigger route.
...
async function runAttempt(input: {
  ...
  if (input.source.state === "existing") {
    ...
    if (created.state === "unresolved") {
      log.warning(
        "Vercel did not return an exact Slack connector UID for this request, so ovo cannot attach or remove it safely.",
      );
...
/**
 * Creates or reuses a Slack connector and points its event destination at ovo.
...
export async function provisionSlackbot(
  ...
  if (existing.state === "failed") {
    log.warning(
      `Could not inspect existing Slack connectors, so ovo did not create another one. ${existing.message}`,
    );
    return { state: "connector-lookup-failed" };
  }
  if (projectId === undefined && existing.connectorUids.size > 0) {
    log.warning(
      "Could not verify which Slack connectors belong to this Vercel project, so ovo did not create another one. Restore `.vercel/project.json`, then try again.",
    );
    return { state: "connector-lookup-failed" };
  }
  ...
  async function runExistingConnector(
    ...
    const notInstalled = (): ProvisionSlackbotResult => {
      log.warning(
        `The existing Slack connector \`${attempt.ref.uid}\` is not connected to a Slack workspace. ovo did not remove it because this run did not create it. If its original browser request is still open, complete it; otherwise run \`vercel connect remove ${attempt.ref.uid} --disconnect-all --yes\` before trying again.`,
      );
      return {
  ...
  async function runInteractiveAttempt(
    ...
    const prompt = awaitChoice({
      ...
      context: "Complete setup in the browser, then wait while ovo verifies the connection",
      actions: [
    ...
    if (race.choice === "retry") {
      ...
      if (inventory.state === "failed") {
        log.warning(
          `Could not inspect existing Slack connectors, so ovo did not create another one. ${inventory.message}`,
        );
