...
import { isEveServerHealthy } from "#shared/ovo-server-health.js";
...
async function closeDevelopmentServerResources(input: {
  ...
  await attempt(() =>
    stopDevelopmentSandboxResources({
      ...
      log: (message) => console.warn(`[ovo:dev] ${message}`),
    }),
  );
...
async function startNitroDevelopmentServer(
  ...
  // Marks this process tree as an `ovo dev` session so runtime features
...
/**
 * Creates a development server for an ovo application. Call `start()` to boot an
 ...
 * `/ovo/v1/dev/schedules/:scheduleId` route — the handler returns
