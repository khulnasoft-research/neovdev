...
import { eveChannel, defaultEveAuth, type EveChannelInput } from "#public/channels/ovo.js";
...
const OVERRIDE_AUTH: SessionAuthContext = {
  ...
  authenticator: "ovo-on-message",
  principalId: "user-2",
  principalType: "user",
};

/**
 * Creates a POST handler test harness for the create route (POST /ovo/v1/session).
...
function createEveContinueHandler(input: EveChannelInput) {
  ...
  const continueRoute = channel.routes.find(
    (r) => r.method === "POST" && r.path === "/ovo/v1/session/:sessionId",
  );
 ...
 * (POST /ovo/v1/session/:sessionId/cancel).
 */
function createEveCancelHandler(input: EveChannelInput) {
  const channel = eveChannel(input);
  const cancelRoute = channel.routes.find(
    (r) => r.method === "POST" && r.path === "/ovo/v1/session/:sessionId/cancel",
  );
...
function cancelRequest(body?: unknown): Request {
  return new Request("https://example.com/ovo/v1/session/test-session-id/cancel", {
    ...(body === undefined
      ? {}
      : {
...
function createEveStreamHandler(input: EveChannelInput) {
  ...
  const streamRoute = channel.routes.find(
    (route) => route.method === "GET" && route.path === "/ovo/v1/session/:sessionId/stream",
  );
...
describe("eveChannel — events", () => {
  ...
  it("normalizes higher-level CORS options", () => {
    const channel = eveChannel({
      ...
      cors: {
        ...
        credentials: true,
        exposedHeaders: ["x-ovo-session-id"],
        maxAge: 300,
        methods: ["POST", "GET"],
    ...
    expect(channel.cors).toEqual({
      ...
      credentials: true,
      exposeHeaders: ["x-ovo-session-id"],
      maxAge: "300",
      methods: ["POST", "GET"],
      origin: ["https://app.example.com"],
  ...
  it("passes configured event handlers through with session context", async () => {
    ...
    const session: RuntimeSession = {
      ...
      sessionId: "sess-ovo-event",
      turn: { id: "turn-1", sequence: 1 },
    };
    const ctx = new ContextContainer();
    ctx.set(ContinuationTokenKey, "ovo:continuation");
    ctx.set(SessionIdKey, "sess-ovo-event");
    ...
    expect(observed).toEqual(["done", "ovo:continuation", "sess-ovo-event"]);
  });
});
...
describe("eveChannel — stream cursor", () => {
  it("establishes the NDJSON body before the first durable event", async () => {
    ...
    const response = await handler.fetch("https://ovo.test/ovo/v1/session/test-session-id/stream");
  ...
  it("forwards negative tail-relative start indices", async () => {
    ...
    const response = await handler.fetch(
      "https://ovo.test/ovo/v1/session/test-session-id/stream?startIndex=-1",
    );
  ...
  it.each(["1.5", "1junk", "0x10", "1e2", ""])(
    ...
    async (startIndex) => {
      const handler = createEveStreamHandler({ auth: none() });
      const response = await handler.fetch(
        `https://ovo.test/ovo/v1/session/test-session-id/stream?startIndex=${encodeURIComponent(startIndex)}`,
      );
...
describe("eveChannel — onMessage", () => {
  it("runs after auth on create requests and appends returned context", async () => {
    const onMessage = vi.fn((ctx, message) => {
      expect(ctx.ovo.caller).toEqual(ACCEPTED_AUTH);
      expect(defaultEveAuth(ctx)).toEqual(ACCEPTED_AUTH);
      expect(ctx.ovo.sessionId).toBeUndefined();
      expect(ctx.ovo.request.url).toBe("https://example.com/ovo/v1/session");
  ...
  it("passes session context to onMessage on continue requests", async () => {
    const onMessage = vi.fn((ctx, message) => {
      expect(ctx.ovo.caller).toEqual(ACCEPTED_AUTH);
      expect(defaultEveAuth(ctx)).toEqual(ACCEPTED_AUTH);
      expect(ctx.ovo.sessionId).toBe("test-session-id");
...
describe("eveChannel — create session (text)", () => {
    ...
    const response = await handler.fetch(
      createJsonMessageRequest({
        callback: {
          ...
          token: "tok123",
          url: "https://caller.example.com/ovo/v1/callback/tok123",
        },
        message: "hi",
        mode: "task",
      }),
    ...
    const response = await handler.fetch(
      createJsonMessageRequest({
        callback: {
          ...
          subagentName: "research",
          token: "tok123",
          url: "https://caller.example.com/ovo/v1/callback/other-token",
        },
        message: "hi",
        mode: "task",
      }),
  ...
  it("rejects invalid JSON bodies with 400", async () => {
    ...
    const response = await handler.fetch(
      new Request("https://example.com/ovo/v1/session", {
        body: "not-json",
        headers: { "content-type": "application/json" },
        method: "POST",
      }),
...
describe("eveChannel — create session (UserContent array)", () => {
  ...
  it("rejects a file part whose data carries a framework-internal ref scheme", async () => {
    ...
    // The `ovo-url:ovo-sandbox:` gadget would otherwise be reconstituted by the
    // staging pipeline into an arbitrary sandbox file read before the model call.
    const response = await handler.fetch(
      createJsonMessageRequest({
        message: [
          filePartBody({
            data: "ovo-url:ovo-sandbox:?path=/etc/passwd&size=1&type=image/png",
            mediaType: "image/png",
          }),
        ],
      }),
  ...
  it("rejects a file part whose data is a bare internal sandbox ref", async () => {
    ...
    const response = await handler.fetch(
      createJsonMessageRequest({
        message: [filePartBody({ data: "ovo-sandbox:?path=/etc/passwd", mediaType: "image/png" })],
      }),
    );
...
describe("eveChannel — cancel turn", () => {
  it("cancels the current turn with no body and reports 'accepted'", async () => {
    ...
    expect(response.headers.get("x-ovo-session-id")).toBe("test-session-id");
