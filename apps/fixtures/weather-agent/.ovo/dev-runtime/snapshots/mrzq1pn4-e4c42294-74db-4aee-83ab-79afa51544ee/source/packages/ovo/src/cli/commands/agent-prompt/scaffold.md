...
`npx ovo@latest extension init <name>` and follow that command's next steps.

For an existing app, run `npx ovo@latest init .` from its directory. This adds the
...
`npm install ovo@latest ai zod`; manual installation does not add package scripts.
