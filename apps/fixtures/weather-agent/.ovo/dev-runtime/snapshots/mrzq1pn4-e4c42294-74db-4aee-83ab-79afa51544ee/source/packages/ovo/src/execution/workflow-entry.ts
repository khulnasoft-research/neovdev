...
import { readChannelRequestId, readRootSessionId } from "#execution/ovo-workflow-attributes.js";
...
export async function workflowEntry(input: WorkflowEntryInput): Promise<WorkflowEntryResult> {
  ...
  const continuationToken = (input.serializedContext["ovo.continuationToken"] as string) || "";
  const mode = input.serializedContext["ovo.mode"] as RunMode;
  const capabilities = input.serializedContext["ovo.capabilities"] as
    | SessionCapabilities
    | undefined;
  const serializedBundle = input.serializedContext["ovo.bundle"] as {
    source: DurableCompiledArtifactsSource;
    nodeId?: string;
  };

  // Seed `ovo.sessionId` so the terminal failure emitter can stamp it
  // onto `session.failed` even if `createSessionStep` itself throws.
  input.serializedContext["ovo.sessionId"] = sessionId;
