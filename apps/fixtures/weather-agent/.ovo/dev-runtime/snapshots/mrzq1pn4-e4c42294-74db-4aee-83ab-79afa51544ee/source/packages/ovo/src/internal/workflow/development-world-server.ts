...
class LocalParentDevelopmentWorkflowWorld implements ParentDevelopmentWorkflowWorld {
  ...
  async start(): Promise<void> {
    ...
    if (missingGenerationIds.size > 0) {
      ...
      console.error(
        `[ovo:dev] ${String(missingGenerationIds.size)} active local Workflow run(s) reference development generations that no longer exist ` +
  ...
  async #call(call: DevelopmentWorldCall): Promise<unknown> {
    ...
    // Deployment identity and enqueueing carry ovo semantics (generation
  ...
  #generationExists(generationId: string): boolean {
    ...
    return existsSync(
      join(this.#appRoot, ".ovo", "dev-runtime", "snapshots", generationId, "generation.json"),
    );
  }
}
