...
/**
 * Builds the current package as an ovo extension. Requires
 * `package.json#ovo.extension` declaring distinct source and dist roots.
 */
export async function runExtensionBuildCommand(
  ...
  if (config === null) {
    throw new Error(
      'This package is not an ovo extension. Add `"ovo": { "extension": { "source": "./extension", "dist": "./dist/extension" } }` to package.json, ' +
        "or run `ovo build` for an agent app.",
    );
  }
