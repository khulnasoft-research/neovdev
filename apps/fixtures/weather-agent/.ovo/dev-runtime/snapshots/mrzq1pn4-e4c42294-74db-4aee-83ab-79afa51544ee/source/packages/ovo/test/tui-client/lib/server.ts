...
/**
 * Builds an agent app, starts the built output with `ovo start`, and waits for
 * `/ovo/v1/health` plus a matching `/ovo/v1/info` agent identity before
 ...
 * dev-only HTTP route (e.g. `POST /ovo/v1/dev/schedules/:scheduleId`) so
...
function spawnServerProcess(input: {
  ...
  return spawn("pnpm", input.args, {
    ...
    // (pnpm + the `ovo` Node grandchild it spawns) with one
    // `process.kill(-pid, ...)`. Without it, signalling only the
    // immediate child reaps `pnpm` but orphans `ovo`, and the orphan
...
async function waitForAgentServer(input: {
  ...
  while (Date.now() < deadline) {
    ...
    const healthResponse = await fetch(`${input.baseUrl}/ovo/v1/health`, {
      signal: AbortSignal.timeout(2_000),
    }).catch(() => undefined);
    if (healthResponse?.ok) {
      const infoResponse = await fetch(`${input.baseUrl}/ovo/v1/info`, {
        signal: AbortSignal.timeout(2_000),
      }).catch(() => undefined);
      if (infoResponse?.ok) {
        ...
        throw new Error(
          `Agent server ${JSON.stringify(input.appName)} returned an unrecognized identity payload from ${input.baseUrl}/ovo/v1/info.`,
        );
      }
    }
...
/** Reads the agent name from a versioned `/ovo/v1/info` response. */
export function getAgentNameFromInfoPayload(payload: unknown): string | undefined {
  if (!isRecord(payload) || payload.kind !== "ovo-agent-info" || payload.version !== 1) {
    return undefined;
  }
