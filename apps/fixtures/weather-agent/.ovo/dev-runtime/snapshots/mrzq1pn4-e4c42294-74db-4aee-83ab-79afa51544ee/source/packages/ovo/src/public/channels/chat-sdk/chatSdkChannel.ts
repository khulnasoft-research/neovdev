 ...
 * thread for the Ovo session so event handlers can post replies without
...
/**
 * Channel-owned metadata exposed to Ovo instrumentation.
 */
...
/** Event-handler context for `chatSdkChannel`, including Ovo session helpers. */
 ...
 * determines the Ovo continuation token and the persisted channel state.
 */
 ...
 * `ChatConfig` fields, plus Ovo route and event settings.
 */
...
> extends Omit<ChatConfig<TAdapters>, "adapters"> {
  ...
  /**
   * Base route for generated adapter webhooks. Defaults to `/ovo/v1`, so an
   * adapter named `slack` mounts at `/ovo/v1/slack`.
   */
  ...
  /** Extra Chat SDK webhook options. Ovo owns `waitUntil`. */
  readonly webhook?: Omit<WebhookOptions, "waitUntil">;
  /** Optional Ovo event handlers. Supplied handlers replace built-in defaults. */
  readonly events?: ChatSdkChannelEvents<TAdapters>;
  /**
   * Prefix for default Ovo HITL button action ids. Change this if your Chat SDK
...
/**
 * Creates an Ovo channel backed by one Chat SDK runtime and its adapters.
 *
 * @example
 * ```ts
...
function renderInputRequest(request: InputRequest, inputActionPrefix: string) {
  ...
  children.push(
    CardText("This request needs a freeform answer. Continue from the Ovo session UI."),
  );
  return children;
}
