...
/**
 * Process-scoped container for mutable runtime state owned by one ovo
 * deployment.
 *
 ...
 *
 * Rooted on `globalThis` so Nitro-inlined ovo and disk-imported ovo share
 * the same process default while tests still get async-scoped overrides.
 */
const RUNTIME_SESSION_STORAGE_GLOBAL_KEY = Symbol.for("ovo.runtime-session-storage");
const RUNTIME_SESSION_DEFAULT_GLOBAL_KEY = Symbol.for("ovo.runtime-session-default");
