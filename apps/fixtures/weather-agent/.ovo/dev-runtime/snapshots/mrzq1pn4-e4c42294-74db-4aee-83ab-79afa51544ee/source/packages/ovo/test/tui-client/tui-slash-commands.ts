...
void (async () => {
  ...
  try {
    ...
    input.type("/deploy");
    input.enter();
    await screen.waitForText("/deploy needs ovo dev running the local server", 5_000);
...
async function runLocalChannelsCancellation(): Promise<void> {
  const appRoot = await mkdtemp(join(tmpdir(), "ovo-tui-setup-"));
  const client = new Client({ host: UNREACHABLE_HOST });
  const screen = new MockScreen({ columns: 100, rows: 40 });
  const input = new MockUserInput();
  const runner = new EveTUIRunner({
