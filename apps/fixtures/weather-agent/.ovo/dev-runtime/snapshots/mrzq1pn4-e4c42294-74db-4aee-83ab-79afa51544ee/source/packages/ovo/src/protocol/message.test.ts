...
describe("message stream protocol", () => {
  ...
  it("builds authorization.required with optional challenge and webhookUrl", () => {
    ...
    const webhookUrl = `https://ovo.example.com${createEveConnectionCallbackRoutePath(
      "linear",
      "abc",
    )}`;
...
describe("createMessageReceivedEvent", () => {
  ...
  it("unwraps serialized ovo-url refs only when the wrapped URL is client-resolvable", () => {
    expect(
      projectParts([
        {
    ...
    expect(
      projectParts([
        {
          data: "ovo-url:ovo-sandbox:?path=%2Fworkspace%2Fsecret.png&size=10&type=image%2Fpng",
          mediaType: "image/png",
          type: "file",
        },
      ]),
