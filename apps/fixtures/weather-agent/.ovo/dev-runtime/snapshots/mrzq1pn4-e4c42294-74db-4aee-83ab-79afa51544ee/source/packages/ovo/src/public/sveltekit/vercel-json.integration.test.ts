...
async function createTempRoot(): Promise<string> {
  return await mkdtemp(join(tmpdir(), "ovo-sveltekit-vercel-"));
}
...
describe("ensureEveVercelJson", () => {
  it("creates experimentalServices and rewrites when vercel.json is missing", async () => {
    ...
    expect(await readJsonFile(join(svelteKitRoot, "vercel.json"))).toEqual({
      ...
      rewrites: [
        {
          source: "/ovo/v1/:path*",
          destination: `${EVE_SVELTEKIT_SERVICE_PREFIX}/ovo/v1/:path*`,
        },
      ],
    });
  });

  it("resolves the ovo entrypoint relative to the SvelteKit root", async () => {
    const svelteKitRoot = await createTempRoot();
  ...
  it("uses a custom build command", async () => {
    ...
    expect(await readJsonFile(join(svelteKitRoot, "vercel.json"))).toMatchObject({
      experimentalServices: {
        ovo: { buildCommand: "pnpm build:ovo" },
      },
    });
  });

  it("preserves an already-configured ovo service and reuses its prefix", async () => {
    const svelteKitRoot = await createTempRoot();
    const existing = {
      $schema: "https://openapi.vercel.sh/vercel.json",
      experimentalServices: {
      ...
      rewrites: [
        {
          source: "/ovo/v1/:path*",
          destination: "/private/agent/ovo/v1/:path*",
        },
      ],
    };
