...
import {
  ...
  type PrepareSend,
} from "#client/ovo-agent-store.js";
...
/**
 * Immutable point-in-time view of an ovo agent session: projected `data`, the
 ...
 * template, `$derived`, or `$effect` and Svelte will update when ovo streams
 * new events.
 */
...
/**
 * Configuration for a Svelte ovo agent session.
 *
...
export interface UseEveAgentOptions<TData> extends EveAgentStoreCallbacks<TData> {
   ...
   * `/ovo/agents/support/ovo/v1/...`. Do not combine with `host`.
   */
  ...
  /**
   * Base URL for ovo client requests. Do not combine with `agent`. Empty targets same-origin ovo routes
   * such as `/ovo/v1/...`; a same-origin prefix like `/api` routes through an
   * app-owned proxy; an absolute origin hits an ovo server directly.
   *
   * @default ""
   */
  ...
  /**
   * Project submitted user messages before ovo confirms them with a
...
/**
 * Svelte 5 binding that drives an ovo session and projects its events into
