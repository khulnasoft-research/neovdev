...
export interface SlackChannelCredentials {
  ...
  /**
   * Custom inbound webhook verifier. When supplied, ovo skips the
...
export interface SlackInboundEventContext {
  ...
  /** Resolves the active ovo session for a Slack channel thread. */
...
export interface SlackInboundMessageContext extends SlackContext {
  /** Returns whether this message belongs to a thread with an active ovo session. */
...
export interface SlackChannelConfig {
  ...
  /** Override the default webhook route path (`/ovo/v1/slack`). */
