 ...
 * reserved `ovo.*` keys, and callback failures are warning-only so
...
export function buildTelemetryRuntimeContext(
  ...
  return {
    ...
    "ovo.channel.kind": normalizeInstrumentationChannelKind(projection?.kind),
    "ovo.environment": input.environment,
    "ovo.session.id": input.session.sessionId,
    "ovo.step.index": String(input.emissionState.stepIndex),
    "ovo.turn.id": input.emissionState.turnId,
    "ovo.turn.sequence": String(input.emissionState.sequence),
    "ovo.version": input.eveVersion,
  };
}
 ...
 * {@link InstrumentationRuntimeContext} shape, dropping reserved `ovo.*` keys
