...
describe("just-bash backend without the optional dependency installed", () => {
  it("fails with an actionable install hint outside ovo dev", async () => {
    const appRoot = await createScratchDirectory("ovo-just-bash-missing-");
    const backend = createJustBashSandboxBackend();

    // Outside `ovo dev` (no EVE_DEV flag) the loader must not attempt a
  ...
  it("fails without installing when autoInstall is disabled, even in ovo dev", async () => {
    vi.stubEnv("EVE_DEV", "1");
    try {
      const appRoot = await createScratchDirectory("ovo-just-bash-no-autoinstall-");
