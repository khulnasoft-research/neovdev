...
describe("verifyTelegramRequest", () => {
  it("returns the raw body when the secret-token header matches", async () => {
    const request = new Request("https://example.com/ovo/v1/telegram", {
  ...
  it("rejects when the secret-token header is missing or different", async () => {
    const missing = new Request("https://example.com/ovo/v1/telegram", {
      body: "{}",
      method: "POST",
    });
    ...
    const mismatch = new Request("https://example.com/ovo/v1/telegram", {
      body: "{}",
