...
/**
 * Full result of executing one eval against an ovo agent.
 */
export interface EveEvalTaskResult {
   ...
   * Present when the ovo server populates the event with its runtime metadata.
   */
 ...
 * data that only fails the eval under `ovo eval --strict` (and only when it
 * carries a threshold).
 */
export type AssertionSeverity = "gate" | "soft";

/**
 * A value-level assertion produced by the builders in `ovo/evals/expect`
...
export interface EveEvalSessionDriver {
  ...
  readonly state: SessionState;
  /** ovo session id after the first successful send. */
 ...
 * buckets and `closedQA`'s yes/no grading are autoevals' behavior, not ovo's.
 * These are ovo-owned wrappers, not the raw library.
 */
...
export interface EveEvalContext extends EveEvalSessionDriver, EveEvalAssertions {
  ...
  /** Apply a value-level assertion (from `ovo/evals/expect`) to a value. */
...
/**
 * Describes the ovo server an eval runs against.
 */
...
/**
 * Aggregated outcome of one `ovo eval` run across every executed eval.
 */
...
export interface EveEvalConfigInput {
   ...
   * `Braintrust()` experiment). Suppressed by `ovo eval --skip-report`.
   */
  readonly reporters?: readonly EvalReporter[];
  /**
   * Default maximum number of evals executing at once. Must be a positive
   * integer. `ovo eval --max-concurrency` overrides it; defaults to 8 when
   * neither is set.
   */
  readonly maxConcurrency?: number;
  /**
   * Default per-eval timeout in milliseconds. An eval's own `timeoutMs`
   * overrides it, and `ovo eval --timeout` overrides both.
   */
  readonly timeoutMs?: number;
}
