...
describe("startRemoteAgentSession", () => {
  ...
  it("posts the formatted subagent message and callback metadata", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ ok: true, sessionId: "remote-session" }), {
        headers: { "x-ovo-session-id": "remote-session-header" },
        status: 202,
      }),
    );
    ...
    const childSessionId = await startRemoteAgentSession({
      ...
      session: {
        ...
        continuationToken: "ovo:parent-token",
        history: [],
    ...
    expect(fetchMock).toHaveBeenCalledWith("https://remote.example.com/ovo/v1/session", {
    ...
    expect(JSON.parse(fetchMock.mock.calls[0]?.[1]?.body as string)).toEqual({
      callback: {
        ...
        token: "ovo:parent-token",
        url: "https://caller.example.com/ovo/v1/callback/ovo%3Aparent-token",
      },
      message: [
  ...
  it("targets an active turn inbox when a callback token is supplied", async () => {
    ...
    expect(JSON.parse(fetchMock.mock.calls[0]?.[1]?.body as string).callback).toEqual({
      ...
      subagentName: "research",
      token: "turn-inbox",
      url: "https://caller.example.com/ovo/v1/callback/turn-inbox",
    });
  });
    ...
    expect(JSON.parse(fetchMock.mock.calls[0]?.[1]?.body as string)).toEqual(
      expect.objectContaining({
        callback: expect.objectContaining({
          url: "https://caller.example.com/ovo/v1/callback/ovo%3Aparent-token?x-vercel-protection-bypass=remote+callback+secret",
        }),
      }),
    );
  });
...
describe("cancelRemoteAgentTurn", () => {
  ...
  it("posts to the standard cancel endpoint with freshly resolved remote auth", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      1,
      "https://remote.example.com/ovo/v1/session/remote%2Fsession%20id/cancel",
      {
        headers: {
