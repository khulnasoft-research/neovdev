...
async function createMinimalAppRoot(prefix: string): Promise<string> {
  ...
  await writeFile(
    ...
    `${JSON.stringify(
      {
        name: "ovo-cli-start-health-test",
        private: true,
        type: "module",
      },
      null,
...
async function waitForStartUrl(input: {
  ...
  while (Date.now() - startedAt < 60_000) {
    ...
    if (input.child.exitCode !== null || input.child.signalCode !== null) {
      throw new Error(
        [
          `ovo start exited before printing its server URL (code ${String(
  ...
  throw new Error(
    [
      "Timed out waiting for ovo start to print its server URL.",
      `stdout:\n${output.stdout}`,
      `stderr:\n${output.stderr}`,
    ].join("\n\n"),
  );
...
describe("runCli", () => {
  ...
  it("prints fixture information", async () => {
    ...
    expect(getLogOutput(logger)).toContain("ovo Info");
  ...
  it("prints compiled discovery metadata when run inside an ovo app", async () => {
    ...
    expect(output).toContain(join(resolvedFixtureRoot, ".ovo", "compile", "module-map.mjs"));
  ...
  it("passes host and port to the production start host", async () => {
    const workspaceRoot = await createScratchDirectory("ovo-cli-start-options-");
  ...
  it("fails clearly when start runs before build output exists", async () => {
    const workspaceRoot = await createScratchDirectory("ovo-cli-start-missing-output-");
    const resolvedWorkspaceRoot = await realpath(workspaceRoot);
    const previousCwd = process.cwd();
    const logger = {
      error: vi.fn(),
    ...
    try {
      await expect(runCli(["start"], logger)).rejects.toThrow(
        `Missing ovo build output at ${join(
          resolvedWorkspaceRoot,
          ".output",
          "server",
          "index.mjs",
        )}. Run "ovo build" before "ovo start".`,
      );
    } finally {
      process.chdir(previousCwd);
    }
  ...
  it("starts an existing built app with local Workflow data under .ovo", async () => {
    const { buildApplication } = await import("../../src/internal/nitro/host.js");
    const appRoot = await createMinimalAppRoot("ovo-cli-start-health-");
  ...
  it("surfaces discovery diagnostics in build failures", async () => {
    const workspaceRoot = await createScratchDirectory("ovo-cli-build-diagnostics-");
    const resolvedWorkspaceRoot = await realpath(workspaceRoot);
    const previousCwd = process.cwd();
    const logger = {
      error: vi.fn(),
    ...
    expect(thrownError.message).not.toContain(`${join(resolvedWorkspaceRoot, ".ovo", "builds")}/`);
  ...
  it("loads development env files before running build", async () => {
    const workspaceRoot = await createScratchDirectory("ovo-cli-build-env-");
    const resolvedWorkspaceRoot = await realpath(workspaceRoot);
    const previousCwd = process.cwd();
    const logger = {
      error: vi.fn(),
  ...
  it("forwards the sandbox prewarm opt-out to the build host", async () => {
    const workspaceRoot = await createScratchDirectory("ovo-cli-build-skip-prewarm-");
    const resolvedWorkspaceRoot = await realpath(workspaceRoot);
    const previousCwd = process.cwd();
    const logger = {
      error: vi.fn(),
