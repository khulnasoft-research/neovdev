    ...
    server.listen(0, input.host, () => {
      ...
      server.close((error) => {
        ...
        if (address === null || typeof address === "string") {
          rejectPort(new Error("Failed to resolve an available port for ovo start."));
          return;
        }
...
/**
 * Starts a built Nitro server for an ovo application.
 */
export async function startProductionServer(
  ...
  if (!existsSync(serverEntry)) {
    throw new Error(
      `Missing ovo build output at ${serverEntry}. Run "ovo build" before "ovo start".`,
    );
  }
  ...
  try {
    ...
    if (isRecord(error) && error.name === "AbortError") {
      throw new Error("Timed out waiting for built ovo server to respond.", { cause: error });
    }
