...
export async function compileConnectionDefinition(
  ...
  const message = `Expected the connection export "${source.exportName ?? "default"}" from "${source.logicalPath}" to match the public ovo shape.`;
 ...
 * `@vercel/connect/ovo`'s `connect()` helper attaches to its returned
 * authorization definition. Returns the parsed marker when present and
 * shaped correctly, otherwise `undefined`.
 *
 * The compiler does not import `@vercel/connect/ovo` (it should not
