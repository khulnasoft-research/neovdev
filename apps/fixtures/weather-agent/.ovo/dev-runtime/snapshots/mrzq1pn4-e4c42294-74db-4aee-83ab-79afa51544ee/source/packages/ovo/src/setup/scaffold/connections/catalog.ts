/**
 * Build-time catalog of known ovo connections. Each entry drives both the
 * `ovo connections add` picker and the per-entry scaffold template.
 *
 * Connection *identity* (slug, label, transport, description) is owned by
 * `@khulnasoft/ovo-catalog`, the cross-surface source of truth shared with the
...
export type ConnectionAuthSpec =
   ...
   * (e.g. `mcp.linear.app`); when omitted, ovo falls back to the MCP endpoint
 ...
 * `@khulnasoft/ovo-catalog`; this map says how each curated connection authenticates
 ...
 * (`CONNECTION_SLUG_PATTERN` in `ovo/src/discover/grammar.ts`): a lowercase
 ...
 * never writes a connection file that `ovo build` would later refuse to
 * discover.
 */
