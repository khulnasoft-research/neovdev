...
describe("compiler artifacts", () => {
  ...
  it("writes stable discovery artifacts under .ovo", async () => {
    const { agentRoot, appRoot } = await createAppRoot("ovo-compiler-artifacts-", APP_ROOT_OPTIONS);
    ...
    const writtenArtifacts = await writeCompilerArtifacts({
      appRoot,
      artifactLocations: {
        publishedRoot: join(appRoot, ".ovo"),
        writeRoot: join(appRoot, ".ovo"),
      },
      diagnostics: [
        createDiscoverWarningDiagnostic({
          code: "discover/unsupported-directory",
    ...
    expect(normalizeArtifactValue(JSON.parse(discoveryManifestText), appRoot)).toMatchObject({
      ...
      kind: "ovo-agent-discovery-manifest",
      instructions: [
        {
          definition: {
            markdown: "You are a precise assistant.",
    ...
    expect(normalizeArtifactValue(JSON.parse(compiledManifestText), appRoot)).toMatchObject({
      ...
      kind: "ovo-agent-compiled-manifest",
      instructions: {
        name: "instructions",
        logicalPath: "instructions.md",
        markdown: "You are a precise assistant.",
    ...
    expect(normalizeArtifactValue(JSON.parse(diagnosticsText), appRoot)).toMatchObject({
      ...
      kind: "ovo-discovery-diagnostics",
      summary: {
        errors: 0,
        warnings: 1,
      },
    ...
    expect(normalizeCompileMetadata(compileMetadata)).toMatchObject({
      compile: {
        moduleMap: {
          path: ".ovo/compile/module-map.mjs",
          sha256: "<sha256>",
        },
      },
      discovery: {
        diagnostics: {
          path: ".ovo/discovery/diagnostics.json",
          sha256: "<sha256>",
        },
        manifest: {
          path: ".ovo/discovery/agent-discovery-manifest.json",
          sha256: "<sha256>",
        },
  ...
  it("records versioned artifact hashes in compile metadata", () => {
    ...
    const firstMetadata = createCompileMetadata({
      appRoot,
      diagnosticsArtifactJson: '{"kind":"ovo-discovery-diagnostics"}\n',
      diagnosticsSummary: {
        errors: 0,
        warnings: 1,
      },
      discoveryManifestJson: '{"kind":"ovo-agent-discovery-manifest","agentId":"weather-agent"}\n',
    ...
    const secondMetadata = createCompileMetadata({
      ...
      discoveryManifestJson:
        '{"kind":"ovo-agent-discovery-manifest","agentId":"weather-agent-v2"}\n',
      moduleMapSource: "export const moduleMap = {};\n",
      paths,
    });
    ...
    expect(firstMetadata.compile.moduleMap.path).toBe(".ovo/compile/module-map.mjs");
    expect(firstMetadata.discovery.manifest.path).toBe(
      ".ovo/discovery/agent-discovery-manifest.json",
    );
    expect(firstMetadata.discovery.diagnostics.path).toBe(".ovo/discovery/diagnostics.json");
...
describe("compileAgent", () => {
    ...
    const app = await scenarioApp({
      ...
      files: {
        ...
        "node_modules/@acme/crm/package.json": `${JSON.stringify({
          name: "@acme/crm",
          type: "module",
          ovo: { extension: { source: "source", dist: "extension" } },
  ...
  it("compiles authored modules from cjs and cts extensions", async () => {
    const { agentRoot, appRoot } = await createAppRoot(
      "ovo-compile-extension-variants-",
      APP_ROOT_OPTIONS,
    );
  ...
  it("materializes TypeScript-authored skill package files as workspace resources", async () => {
    const { agentRoot, appRoot } = await createAppRoot(
      "ovo-compile-dynamic-skill-files-",
      APP_ROOT_OPTIONS,
    );
  ...
  it("compiles authored schedules into deterministic manifest entries (module + markdown forms)", async () => {
    const { agentRoot, appRoot } = await createAppRoot("ovo-compile-schedules-", APP_ROOT_OPTIONS);
...
`,
  ...
  it("rejects legacy workspace kind fields in authored agent config modules", async () => {
    ...
    await expect(
      ...
      'Expected the agent config export "default" from "agent.mjs" to match the public ovo shape.',
    );
  });

  it("rejects the removed experimental.codeMode field", async () => {
    const { agentRoot, appRoot } = await createAppRoot(
      "ovo-compile-experimental-code-mode-",
      APP_ROOT_OPTIONS,
    );
  ...
  it("compiles remote subagents into the owning node manifest", async () => {
    const { agentRoot, appRoot } = await createAppRoot(
      "ovo-compile-remote-subagent-owned-manifest-",
      APP_ROOT_OPTIONS,
    );
  ...
  it("stores resolved sandbox bootstrap revalidation keys in compiled artifacts", async () => {
    const { agentRoot, appRoot } = await createAppRoot(
      "ovo-compile-sandbox-revalidation-key-",
      APP_ROOT_OPTIONS,
    );
  ...
  it("compiles sandbox bootstrap without a revalidation key", async () => {
    const { agentRoot, appRoot } = await createAppRoot(
      "ovo-compile-sandbox-without-revalidation-key-",
      APP_ROOT_OPTIONS,
    );
  ...
  it("fails fast on discovery errors after writing inspectable artifacts", async () => {
    const { agentRoot, appRoot } = await createAppRoot("ovo-compile-fast-fail-", APP_ROOT_OPTIONS);
...
async function createSandboxRevalidationKeyValidationApp(input: {
  ...
  const app = await createAppRoot(
    `ovo-compile-sandbox-${input.name}-revalidation-key-`,
    APP_ROOT_OPTIONS,
  );
