...
/**
 * A `fetch` for direct AI Gateway requests that identifies ovo via its
...
/**
 * Request headers ovo attaches for a model's provider, or `undefined` when
 * the provider needs none. Gateway-routed models (bare ids and `gateway.*`
 * instances) get the ovo User-Agent product token so AI Gateway can attribute
