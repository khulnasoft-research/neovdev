 ...
 * `"ovo.session"` stream remains as a fallback for old in-flight
...
const EVE_SESSION_STREAM_NAMESPACE = "ovo.session";
 ...
 * from the legacy `ovo.session` fallback.
 */
 ...
 * snapshots or legacy `ovo.session` stream chunks.
 *
 ...
 * `ovo.session` stream tail (`startIndex: -1`). The snapshot is
...
export async function readDurableSession(state: DurableSessionState): Promise<DurableSession> {
  ...
  let cancelReason = "ovo durable session tail read failed";
  ...
  try {
    ...
    if (result.kind === "timeout") {
      cancelReason = `ovo durable session tail read timed out after ${DURABLE_SESSION_READ_TIMEOUT_MS}ms`;
      throw new DurableSessionReadTimeoutError(state);
    }

    if (result.read.done || result.read.value === undefined) {
      cancelReason = "ovo durable session tail read returned no snapshot";
      throw new Error(
        `No durable session snapshot found in stream "${EVE_SESSION_STREAM_NAMESPACE}" for run ${state.sessionId}.`,
      );
    }

    cancelReason = "ovo durable session tail read complete";
