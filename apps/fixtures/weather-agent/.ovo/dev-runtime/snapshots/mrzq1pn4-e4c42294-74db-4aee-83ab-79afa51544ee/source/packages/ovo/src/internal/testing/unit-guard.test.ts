...
describe("unit-guard", () => {
  it("blocks fs/promises write operations", async () => {
    await expect(fsPromises.writeFile("/tmp/ovo-unit-guard-probe", "")).rejects.toThrow(
      /Unit tests may not invoke/,
    );
    await expect(fsPromises.mkdtemp("/tmp/ovo-unit-guard-probe-")).rejects.toThrow(
      /Unit tests may not invoke/,
    );
    await expect(
      fsPromises.mkdir("/tmp/ovo-unit-guard-probe", { recursive: true }),
    ).rejects.toThrow(/Unit tests may not invoke/);
  });

  it("blocks synchronous fs write operations", () => {
    expect(() => fs.writeFileSync("/tmp/ovo-unit-guard-probe", "")).toThrow(
      /Unit tests may not invoke/,
    );
    expect(() => fs.mkdirSync("/tmp/ovo-unit-guard-probe")).toThrow(/Unit tests may not invoke/);
  });
