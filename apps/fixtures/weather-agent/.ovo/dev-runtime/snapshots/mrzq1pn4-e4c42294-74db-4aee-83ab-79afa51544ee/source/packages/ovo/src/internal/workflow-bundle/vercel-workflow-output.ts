...
} from "#internal/workflow-bundle/ovo-service-route-output.js";
...
/**
 * Keeps only ovo-owned Vercel function output and rewrites ovo route function
 * symlinks to a shared ovo-owned server function.
 *
 * Nitro emits generic app routes such as `index.func -> ./__server.func` for
 * ovo's standalone landing page. In a multi-service Next.js deployment those
 * root aliases collide with Next's own functions. The Next integration only
 * proxies ovo's `/ovo/v1/**` transport routes, so Vercel output should expose
 * those route functions and workflow trigger functions, not ovo's root page.
 *
 * Nitro also dedupes every route function through `__server.func`. Preserve
 * that model by copying the shared target once into the ovo-owned tree and
 * repointing ovo route aliases at it before pruning the root target.
 */
...
async function prepareSharedEveServerFunction(functionsDir: string): Promise<string | null> {
  ...
  const stagingPath = `${sharedFunctionPath}.ovo-staging`;
