 ...
 * process alive past the supervisor's kill grace (`ovo start` waits
 * 20s before SIGKILL).
 */
 ...
 *
 * - `ovo dev` workers are excluded: the dev CLI parent already stops
...
export async function runSandboxShutdown(log: (message: string) => void): Promise<void> {
  ...
  const deadline = new Promise<void>((resolve) => {
    timer = setTimeout(() => {
      log("ovo: sandbox shutdown timed out; continuing exit");
      resolve();
