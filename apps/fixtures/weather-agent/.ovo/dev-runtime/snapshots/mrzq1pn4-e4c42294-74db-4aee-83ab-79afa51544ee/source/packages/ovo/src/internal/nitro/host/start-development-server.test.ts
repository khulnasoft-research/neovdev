...
const mocks = vi.hoisted(() => {
  ...
  const nitro = {
    close: vi.fn(async () => undefined),
    options: {
      buildDir: "/tmp/ovo-test/.ovo/nitro",
      devServer: {
        hostname: "127.0.0.1",
        port: 0,
      },
  ...
  return {
    ...
    buildDevelopmentHostCandidate: vi.fn(async (input: { nitro: { close(): Promise<void> } }) => {
      ...
      return {
        entry: "/tmp/ovo-test/.ovo/dev-hosts/test/output/server/index.mjs",
        workerData: {},
      };
    }),
    ...
    prepareDevelopmentApplicationHost: vi.fn(async () => ({
      appRoot: "/tmp/ovo-test",
      compileResult: {
        manifest: { config: { name: "test-agent" } },
      },
      generation: {
        fingerprint: "test",
        runtimeAppRoot: "/tmp/ovo-test/.ovo/dev-runtime/snapshots/test/source/app",
        snapshotRoot: "/tmp/ovo-test/.ovo/dev-runtime/snapshots/test",
        snapshotSourceRoot: "/tmp/ovo-test/.ovo/dev-runtime/snapshots/test/source",
        sourceRoot: "/tmp/ovo-test",
      },
      workspaceExtensions: [],
      workspace: {
        artifactsDir: "/tmp/ovo-test/.ovo/dev-hosts/test/artifacts",
        compilerArtifactsDir: "/tmp/ovo-test/.ovo/dev-hosts/test/compiler",
        nitroBuildDir: "/tmp/ovo-test/.ovo/dev-hosts/test/nitro",
        nitroOutputDir: "/tmp/ovo-test/.ovo/dev-hosts/test/output",
        rootDir: "/tmp/ovo-test/.ovo/dev-hosts/test",
        workflowBuildDir: "/tmp/ovo-test/.ovo/dev-hosts/test/workflow",
      },
    })),
    ...
    readFile: vi.fn(async (path: string) => {
      if (
        path.endsWith("/.ovo/dev-server-state.v1.json") &&
    ...
    resolveDiscoveryProject: vi.fn(async () => ({
      agentRoot: "/tmp/ovo-test/agent",
      appRoot: "/tmp/ovo-test",
      layout: "nested" as const,
    })),
    resolveNitroCompiledArtifactsSource: vi.fn(() => ({
      appRoot: "/tmp/ovo-test/.ovo/dev-runtime-test",
      kind: "disk" as const,
      moduleMapLoaderPath: "/tmp/ovo-package/authored-module-map-loader.ts",
    })),
...
const developmentServerStatePath = join("/tmp/ovo-test", ".ovo", "dev-server-state.v1.json");
...
describe("createDevelopmentServer", () => {
  ...
  it("pins local workflow queue callbacks to the active dev server URL", async () => {
    ...
    const server = await startDevelopmentServer("/tmp/ovo-test");

    expect(mocks.prepareDevelopmentApplicationHost).toHaveBeenCalledWith("/tmp/ovo-test");
    ...
    expect(mocks.pruneLocalSandboxTemplatesInBackground).toHaveBeenCalledWith("/tmp/ovo-test");
    expect(mocks.createParentDevelopmentWorkflowWorld).toHaveBeenCalledWith(
      expect.objectContaining({ agentName: "test-agent", appRoot: "/tmp/ovo-test" }),
    );
    ...
    expect(process.env.EVE_DEV_WORKER_APP_ROOT).toBe("/tmp/ovo-test");
  ...
  it("uses Ovo's default port when no port is requested", async () => {
    const startDevelopmentServer = await loadStartDevelopmentServer();
    Object.assign(mocks.nitro.options.devServer, {
      port: 3000,
    });
  ...
  it("serves runtime artifact state from the parent-owned control handler", async () => {
    const startDevelopmentServer = await loadStartDevelopmentServer();
    const server = await startDevelopmentServer("/tmp/ovo-test");
    const response = await callControlHandler("http://localhost/ovo/v1/dev/runtime-artifacts");

    if (!(response instanceof Response)) throw new Error("Expected a Response.");
    await expect(response.json()).resolves.toEqual({ revision: "/tmp/ovo-test" });
  ...
  it("registers a host-owned runtime rebuild handler that forces the live watcher", async () => {
    ...
    const response = await callControlHandler(
      "http://localhost/ovo/v1/dev/runtime-artifacts/rebuild?force=1",
    );
  ...
  it("registers a host-owned runtime rebuild handler that flushes queued changes", async () => {
    const startDevelopmentServer = await loadStartDevelopmentServer();
    const server = await startDevelopmentServer("/tmp/ovo-test");
    await callControlHandler("http://localhost/ovo/v1/dev/runtime-artifacts/rebuild");
  ...
  it("closes the server when its state record cannot be written", async () => {
    ...
    await expect(startDevelopmentServer("/tmp/ovo-test")).rejects.toThrow("disk full");
  ...
  it("does not start when the state record cannot be read", async () => {
    ...
    await expect(startDevelopmentServer("/tmp/ovo-test")).rejects.toThrow("permission denied");
  ...
  it("reports a healthy recorded server when starting another server", async () => {
    ...
    await expect(startDevelopmentServer("/tmp/ovo-test")).rejects.toThrow(
      [
        "A dev server is already running for this ovo agent.",
        "To connect to the existing instance, run: pnpm exec ovo dev http://localhost:2000/",
      ].join("\n"),
    );
    expect(mocks.createDevelopmentApplicationNitro).not.toHaveBeenCalled();
  });
  ...
  it("reuses the active server recorded for the same app root when requested", async () => {
    ...
    const owner = await startDevelopmentServer("/tmp/ovo-test");
    ...
    delete process.env.PORT;
    const attached = await startDevelopmentServer("/tmp/ovo-test", {
    ...
    expect(mocks.fetch).toHaveBeenCalledWith("http://localhost:2000/ovo/v1/health", {
      redirect: "error",
  ...
  it("close() tears nothing down when the instance attached to an existing owner", async () => {
    ...
    const owner = createDevelopmentServer("/tmp/ovo-test");
    await owner.start();
    // A real attaching TUI is a separate process and does not inherit the
    // owner's internally installed listener port.
    delete process.env.PORT;

    const attaching = createDevelopmentServer("/tmp/ovo-test", {
      existing: "attach-if-unconfigured",
    });
  ...
  it("waits for a pending start before closing an owned server", async () => {
    ...
    const server = createDevelopmentServer("/tmp/ovo-test");
  ...
  it("does not attach when PORT explicitly configures the endpoint", async () => {
    ...
    await expect(
      startDevelopmentServer("/tmp/ovo-test", { existing: "attach-if-unconfigured" }),
    ).rejects.toThrow("A dev server is already running for this ovo agent.");
    expect(mocks.createDevelopmentApplicationNitro).not.toHaveBeenCalled();
  ...
  it("does not reuse a server recorded under another app root", async () => {
    const startDevelopmentServer = await loadStartDevelopmentServer();
    const otherAppRoot = "/tmp/other-ovo-test";

    seedStateRecord(
      { url: "http://127.0.0.1:2999/" },
      join(otherAppRoot, ".ovo", "dev-server-state.v1.json"),
    );
  ...
  it("prefers the explicit port option over the PORT environment variable", async () => {
    ...
    const server = await startDevelopmentServer("/tmp/ovo-test", { port: 5000 });
