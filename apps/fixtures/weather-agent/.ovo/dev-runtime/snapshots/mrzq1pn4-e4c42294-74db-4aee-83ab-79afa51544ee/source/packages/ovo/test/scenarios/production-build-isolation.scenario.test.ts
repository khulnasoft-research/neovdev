...
function startEveProcess(input: {
  ...
  const eveBinPath = join(input.appRoot, "node_modules", "ovo", "bin", "ovo.js");
  ...
  const result = new Promise<ProcessResult>((resolvePromise, reject) => {
    const deadline = setTimeout(() => {
      ...
      reject(
        new Error(
          [
            `Timed out waiting for ovo ${input.args.join(" ")}.`,
            `stdout:\n${stdout}`,
            `stderr:\n${stderr}`,
          ].join("\n\n"),
        ),
  ...
  const url = await new Promise<string>((resolvePromise, reject) => {
    ...
    const deadline = setTimeout(() => {
      settleReject(
        new Error(
          [
            "Timed out waiting for ovo dev readiness.",
    ...
    function handleExit(code: number | null, signal: NodeJS.Signals | null) {
      settleReject(
        new Error(
          [
            `ovo dev exited before readiness (code ${String(code)}, signal ${String(signal)}).`,
            `stdout:\n${processHandle.stdout()}`,
            `stderr:\n${processHandle.stderr()}`,
          ].join("\n\n"),
        ),
...
async function observeConcurrentBuildWorkspaces(input: {
  ...
  const buildsRoot = join(input.appRoot, ".ovo", "builds");
...
async function watchActiveDevHostWorkspace(appRoot: string): Promise<{
  ...
  const devHostsRoot = join(appRoot, ".ovo", "dev-hosts");
...
async function listBuildWorkspaces(appRoot: string): Promise<readonly string[]> {
  try {
    return await readdir(join(appRoot, ".ovo", "builds"));
  } catch (error) {
...
async function readPublishedCompilationState(appRoot: string): Promise<PublishedCompilationState> {
  const metadataPath = join(appRoot, ".output", ".ovo", "compile", "compile-metadata.json");
  ...
  for (const artifactPath of artifactPaths) {
    expect(artifactPath).not.toContain(".ovo/builds/");
...
describe("production build isolation", () => {
  it(
    ...
    async () => {
      ...
      try {
        ...
        expect(
          (await readdir(join(app.appRoot, ".ovo", "dev-hosts"), { withFileTypes: true }))
            .filter((entry) => entry.isDirectory())
            .map((entry) => entry.name),
  ...
  it(
    ...
    async () => {
      ...
      const summaryPath = join(app.appRoot, ".ovo", "agent-summary.json");
  ...
  it(
    ...
    async () => {
      ...
      const outputBackupPath = `${outputDir}.ovo-backup-${token}`;
      const summaryBackupPath = `${summaryPath}.ovo-backup-${token}`;
      const crashedWorkspace = join(app.appRoot, ".ovo", "builds", "crashed");
      ...
      const lockPath = join(app.appRoot, ".ovo", "locks", "output-publication.lock");
      ...
      expect(await readdir(join(app.appRoot, ".ovo", "locks"))).toEqual([]);
      expect(await listBuildWorkspaces(app.appRoot)).toEqual([]);
