...
async function createAgentProject(): Promise<string> {
  const projectRoot = await mkdtemp(join(tmpdir(), "ovo-deploy-command-"));
...
describe("runDeployCommand", () => {
  test("refuses a directory without an ovo agent", async () => {
    const projectRoot = await mkdtemp(join(tmpdir(), "ovo-deploy-empty-"));
  ...
  test("points an unlinked non-interactive run at ovo link", async () => {
    ...
    expect(logger.errors[0]).toContain("Run `ovo link` first");
    expect(process.exitCode).toBe(1);
