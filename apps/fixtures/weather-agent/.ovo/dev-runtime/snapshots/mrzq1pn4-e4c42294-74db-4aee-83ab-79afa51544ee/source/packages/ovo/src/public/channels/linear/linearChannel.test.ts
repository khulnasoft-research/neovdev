...
function signedRequest(payload: Record<string, unknown>): Request {
  ...
  return new Request("https://example.com/ovo/v1/linear", {
    body,
    headers: {
  ...
  return {
    ...
    agentSession: {
      ...
      issue: {
        ...
        url: "https://linear.app/acme/issue/OVO-123",
      },
      issueId: "issue_1",
...
describe("linearChannel inbound Agent Session events", () => {
  it("dispatches created events with auth, context, token, and state", async () => {
    ...
    expect(payload.context[0]).toContain("issue_identifier: OVO-123");
  ...
  it("acks generic data webhooks without dispatch when no hook is configured", async () => {
    ...
    const request = new Request("https://example.com/ovo/v1/linear", {
      body,
      headers: {
        "content-type": "application/json",
        "linear-signature": signLinearWebhookBody(body, SECRET),
...
describe("linearChannel default event handlers", () => {
  it("posts input requests with native Linear select metadata and no visible marker", async () => {
    ...
    expect(input.content.body).not.toContain("ovo-input");
  });
      ...
      jsonResponse({
        data: {
          agentSessionCreateOnIssue: {
            agentSession: {
              id: "agent_session_2",
              issue: { id: "issue_1", identifier: "OVO-123", title: "Linear work" },
