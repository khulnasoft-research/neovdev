...
const EVE_CACHE_DIRECTORY_NAME = ".ovo";
...
const globalStateSymbol = Symbol.for("ovo.next.state");
  ...
  while (true) {
    try {
      ...
      if (Date.now() > deadline) {
        throw new Error(
          `Timed out after ${timeoutMs}ms waiting for another Next.js process to start ovo.`,
        );
      }
...
function createEveBinaryPath(): string {
  return join(resolvePackageRoot(), "bin", "ovo.js");
}
...
function parseLocalServerOrigin(urlText: string): string | undefined {
  ...
  // dependency warnings can print unrelated URLs before ovo reports its listener,
...
function formatEveDevOutputLine(line: string, logLabel: string | undefined): string | undefined {
  ...
  const tag = logLabel === undefined ? "[ovo:dev]" : `[ovo:dev:${logLabel}]`;
...
function startServerProcess(input: {
  ...
  return new Promise((resolvePromise, reject) => {
    ...
    const timeout = setTimeout(() => {
      ...
      reject(
        new Error(
          `Timed out after ${input.timeoutMs ?? DEFAULT_SERVER_READY_TIMEOUT_MS}ms waiting for ovo to print its server URL.`,
        ),
      );
    ...
    const handleEarlyExit = (code: number | null, signal: NodeJS.Signals | null) => {
      ...
      reject(
        new Error(
          `ovo server process exited before printing its server URL (code ${String(code)}, signal ${String(signal)}).`,
        ),
      );
    };
