...
describe("createRemoteConnectionController", () => {
  it.each([
    {
      name: "the exact Ovo OIDC challenge",
      error: eveUnauthorized(),
      expected: { state: "auth-required", challenge: { kind: "ovo-oidc" } },
    },
    {
      name: "an Ovo-shaped 401 with different copy",
  ...
  it("stays ready after an unusable info payload when health confirms Ovo", async () => {
  ...
  it("is unavailable after an unusable info payload when health cannot confirm Ovo", async () => {
    const harness = createHarness({
      info: async () => {
        throw new AgentInfoResponseError(["agent: Required"]);
      },
