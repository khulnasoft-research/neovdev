...
describe("selectInitHandoff", () => {
  it("keeps non-interactive sessions on the direct ovo dev path", async () => {
    const deps = dependencies({ interactive: false, available: ["claude"] });

    await expect(selectInitHandoff({ deps, agentName: "weather-bot" })).resolves.toBe("ovo-dev");
    expect(deps.isCodingAgentReplAvailable).not.toHaveBeenCalled();
    expect(deps.createPrompter).not.toHaveBeenCalled();
  });

  it("keeps the direct ovo dev path when no supported REPL is installed", async () => {
  ...
  it("starts every availability check before waiting for one to finish", async () => {
    ...
    await expect(handoff).resolves.toBe("ovo-dev");
    expect(checksStartedBeforeAnyFinished).toBe(7);
  });

  it("offers only available REPLs and preserves ovo dev as the default", async () => {
    const deps = dependencies({
      available: ["codex"],
      onSelect: (options) => {
        expect(options.message).toBe("How would you like to continue?");
        expect(options.initialValue).toBe("ovo-dev");
        expect(options.options.map((option) => option.value)).toEqual(["ovo-dev", "codex"]);
...
describe("spawnCodingAgentRepl", () => {
  const PROMPT = "Help the user build their ovo agent.";
