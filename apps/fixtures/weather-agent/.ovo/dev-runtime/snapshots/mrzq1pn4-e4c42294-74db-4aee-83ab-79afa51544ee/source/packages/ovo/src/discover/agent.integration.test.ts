...
describe("discoverAgent (memory)", () => {
  ...
  it("silently ignores generated runtime directories", async () => {
    const project = buildMemoryAgentProject({
      agentDirectories: [".ovo", ".next", ".output", ".vercel", "node_modules"],
      agentFiles: {
        "instructions.md": "You are a precise assistant.",
      },
    });
  ...
  it("resolves a published dist-only extension package that omits ovo.extension.source", async () => {
    const project = buildMemoryAgentProject({
      appFiles: {
        "node_modules/@acme/crm/package.json": JSON.stringify({
          name: "@acme/crm",
