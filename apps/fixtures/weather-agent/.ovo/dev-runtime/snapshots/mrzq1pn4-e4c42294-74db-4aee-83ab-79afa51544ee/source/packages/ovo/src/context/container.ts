...
const EVE_CONTEXT_STORAGE_KEY = Symbol.for("ovo.context-storage");

/**
 * Keyed value container that backs one ovo execution scope.
 *
...
/**
 * Process-wide AsyncLocalStorage used by every ovo module copy in the current
 * runtime.
 *
 * Nitro step bundles can inline parts of ovo while authored modules still
 * import `ovo/*` from disk. Backing the storage with a global
 * symbol keeps those copies on the same ALS instance so authored tools,
 * model callbacks, and step code observe one unified ovo context.
 */
