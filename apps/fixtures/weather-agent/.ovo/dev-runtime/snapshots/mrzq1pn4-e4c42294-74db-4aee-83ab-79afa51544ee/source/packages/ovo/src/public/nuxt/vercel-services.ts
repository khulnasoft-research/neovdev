...
/**
 * The top-level Vercel Build Output route that sends ovo transport requests to
 * the generated ovo service.
 */
 ...
 * path the ovo runtime observes to the ovo transport namespace.
 */
...
/**
 * The generated ovo service entry written into the Vercel Build Output
 * `services` record.
 */
export type EveVercelGeneratedService = {
  readonly buildCommand: string;
  readonly framework: "ovo";
  readonly routes: readonly VercelRouteConfig[];
...
function findEveService(
  ...
  return Object.values(services).find((service) => service.framework === "ovo");
}
...
function assertRootServicesIncludeEve(services: Record<string, VercelServiceConfig>): void {
  if (findEveService(services) === undefined) {
    throw new Error(
      `${VERCEL_JSON_FILE_NAME} already defines services, so the ovo Nuxt module cannot add a generated ovo service. Add an ovo service (framework "ovo") and a rewrite from ${EVE_ROUTE_PREFIX}/(.*) to it in ${VERCEL_JSON_FILE_NAME}, or remove services from ${VERCEL_JSON_FILE_NAME}.`,
    );
  }
}

/**
 * Build the top-level Build Output route that exposes the ovo service on the
 * ovo transport namespace (`/ovo/v1/*`).
 */
...
/**
 * Build the ovo service's own route that sets `request.path` so the ovo
...
/**
 * Resolve the stable Vercel services configuration for a Nuxt + ovo
 * deployment.
 *
 ...
 * an ovo service and the module generates nothing. Otherwise this prepares a
 * generated ovo service — creating its isolated build root under
 * `.ovo/vercel-services/ovo` so the ovo build output cannot collide with the
...
export async function ensureEveVercelServicesConfig(input: {
  ...
  if (rootVercelConfig.experimentalServices !== undefined) {
    console.warn(
      `[ovo] ${VERCEL_JSON_FILE_NAME} defines experimentalServices, which Vercel no longer routes. The ovo Nuxt module now generates the stable services config automatically — remove experimentalServices from ${VERCEL_JSON_FILE_NAME}.`,
    );
  }
...
/**
 * Merge the generated ovo service and its public route into a Nitro Vercel
 ...
 * ovo route always resolves before the Nuxt app's filesystem routing. An ovo
...
export function mergeEveVercelConfig(
  ...
  const configuredEveEntry = Object.entries(existingServices).find(
    ([name, service]) => name === EVE_SERVICE_NAME || service.framework === "ovo",
  );
