...
Research complex weather questions before replying.`;
  ...
  it("silently ignores authored name fields in skill markdown frontmatter", () => {
    ...
    // `name`; ovo derives identity from the file path, so we accept and drop
...
---
When the user asks about weather, call the weather tool before answering.`),
    ).toThrow("Expected authored skill markdown to match the public ovo shape.");
  });
...
---
body`),
    ).toThrow("Expected authored schedule markdown to match the public ovo shape.");
  });
