...
export class TerminalRenderer implements AgentTUIRenderer {
  ...
  async readPrompt(options?: AgentTUISessionOptions): Promise<string> {
    ...
    // initial draft (`ovo dev --input`) wins over it.
  ...
  async renderStream(
    ...
    // A turn not born at the prompt (`ovo dev --input`) arms its own clock;
  ...
  #installLogCapture(): void {
    ...
    // The recorder takes ownership of ovo's own structured log records for
  ...
  /**
   * Displays one structured record from ovo's own logger (the diagnostics
  ...
  #displayLogRecord(record: LogRecord): void {
    ...
    this.#handleCapturedStderr(`[ovo:${record.namespace}] ${record.message}${fieldsText}`);
    this.#paint();
  }
...
function parseSandboxLogLine(line: string): string | undefined {
  ...
  if (!trimmed.startsWith("ovo: ")) {
    return undefined;
  }

  const message = trimmed.slice("ovo: ".length);
