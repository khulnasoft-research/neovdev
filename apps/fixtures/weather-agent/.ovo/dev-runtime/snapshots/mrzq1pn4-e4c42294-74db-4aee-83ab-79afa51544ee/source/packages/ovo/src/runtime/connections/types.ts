 ...
 * helper-based `getToken` implementations (e.g. `@vercel/connect/ovo`)
...
interface AuthorizationDefinitionBase {
   ...
   * `@vercel/connect/ovo` so downstream tooling can detect Vercel
   * Connect-backed connections at compile time without inspecting
   * `getToken`'s closure state. Examples: a future ovo compiler step
   ...
   * owns *below* ovo's per-step cache, for the given `principal`.
   *
   * ovo always drops its own per-step entry when a resolved bearer is
   ...
   * in-process token cache — that ovo cannot see. Without this hook the
