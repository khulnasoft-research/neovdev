...
describe("resolveEveDestinationPrefix", () => {
  ...
  it("ignores non-server URLs in dev server output while waiting for the listening URL", async () => {
    ...
    const destination = resolveEveDestinationPrefix({
      ...
      productionDestinationPrefix: "/internal/ovo",
    });
    ...
    expect(stdoutWrites).toContain(
      '[ovo:dev:support] dependency metadata: "homepage": "https://rolldown.rs/"\n',
    );
    expect(stderrWrites).toContain(
      "[ovo:dev:support] server listening at http://127.0.0.1:33449\n",
    );
  });

  it("suppresses low-signal ovo dev startup output", async () => {
    vi.stubEnv("NODE_ENV", "development");
    const appRoot = await createTempAppRoot();
    const child = createMockChildProcess();
    const stdoutWrites: string[] = [];
    ...
    expect(stdoutWrites).toEqual([
      "[ovo:dev:billing] server listening at http://127.0.0.1:33450\n",
    ]);
  });
});

async function createTempAppRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), "ovo-next-server-"));
  tempRoots.push(root);
  return root;
}
...
async function readRegisteredOrigin(appRoot: string): Promise<string> {
  const registry = JSON.parse(
    await readFile(join(appRoot, ".ovo", "next-dev-server.json"), "utf8"),
  ) as { readonly origin?: unknown };
  if (typeof registry.origin !== "string") {
    throw new Error("ovo dev server registry did not record a string origin.");
  }
