...
function buildRuntimeSandboxTemplateKey(
  ...
  return sanitizeRuntimeSandboxKey(
    `ovo-sbx-tpl-${input.backendName}-${parts.scope}-${templateHash}`,
  );
}
 ...
 * The ovo package version deliberately does not participate: upgrading
 * ovo must not discard session sandbox state.
 */
function buildRuntimeSandboxSessionKey(
  ...
  return sanitizeRuntimeSandboxKey(
    `ovo-sbx-ses-${input.backendName}-${parts.scope}-${version}-${input.sessionId}-${nodeScope}`,
  );
}

/**
 * Resolves the ovo package version that participates in template keys.
 *
