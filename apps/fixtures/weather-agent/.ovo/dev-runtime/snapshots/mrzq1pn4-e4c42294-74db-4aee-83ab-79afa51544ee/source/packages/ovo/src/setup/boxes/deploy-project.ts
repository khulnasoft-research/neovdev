...
export interface DeployProjectOptions {
   ...
   * inside `perform` (the `ovo channels add` composition; onboarding always has
 ...
 * `ovo channels add` composition), it falls back to the interactive bare
 ...
 * the host app instead of the stale `ovo` agent preset. It is a no-op for a
 * plain agent (no host framework on disk) or an already-correct preset, so
 * every deploy surface — `ovo channels add`, the dev TUI `/deploy`, onboarding —
  ...
  return {
    ...
    async perform({ state, input, signal }): Promise<DeployProjectPayload> {
      ...
      if (productionUrl !== undefined) {
        ...
        log.warning("Deployment succeeded, but ovo could not verify its production URL.");
      }
