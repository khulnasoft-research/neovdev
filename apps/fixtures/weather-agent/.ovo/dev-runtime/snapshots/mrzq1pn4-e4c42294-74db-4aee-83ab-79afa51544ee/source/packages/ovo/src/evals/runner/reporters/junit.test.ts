...
describe("JUnit", () => {
  it("writes one testsuite with passed and failed evals as JUnit XML", async () => {
    const reporter = JUnit({ filePath: "/tmp/ovo/junit.xml", suiteName: "suite & one" });

    await reporter.onRunComplete(makeSummary());

    expect(fsMocks.mkdir).toHaveBeenCalledWith("/tmp/ovo", { recursive: true });
    ...
    expect(xml).toContain('<testcase classname="ovo.eval" name="runtime/passes"');
    expect(xml).toContain('<failure message="contains: expected hello">');
  });

  it("defaults the suite name", async () => {
    const reporter = JUnit({ filePath: "/tmp/ovo/junit.xml" });

    await reporter.onRunComplete(makeSummary());

    const [, xml] = fsMocks.writeFile.mock.calls[0]!;
    expect(xml).toContain('<testsuite name="ovo evals" tests="2" failures="1" skipped="0"');
  });
