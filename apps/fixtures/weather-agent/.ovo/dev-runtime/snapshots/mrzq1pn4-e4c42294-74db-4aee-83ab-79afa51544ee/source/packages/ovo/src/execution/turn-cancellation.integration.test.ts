...
import { eveChannel } from "#public/channels/ovo.js";
...
function createCancelRouteCaller(): (
  ...
  ).find((route) => route.method === "POST" && route.path === "/ovo/v1/session/:sessionId/cancel");
...
describe("turn cancellation integration", () => {
  ...
  it("cancels a turn through the ovo channel cancel route", async () => {
      ...
      const run = await start(workflowEntry, [
        {
          ...
          serializedContext: {
            ...
            "ovo.capabilities": { requestInput: true },
          },
        },
      ]);
      const stream = captureTurnEvents(run);
