...
const EVE_PROTOCOL_PREFIX = "/ovo/v1";

describe("normalizeRoutePrefix", () => {
  it("prepends a leading slash when missing", () => {
    expect(normalizeRoutePrefix("internal/ovo")).toBe("/internal/ovo");
  });
...
describe("joinRoutePrefix", () => {
  it("joins with exactly one slash", () => {
    expect(joinRoutePrefix("/_eve_internal/ovo", "/ovo/v1")).toBe("/_eve_internal/ovo/ovo/v1");
  });
  ...
  it("joins onto an absolute origin", () => {
    expect(joinRoutePrefix("http://127.0.0.1:4274", "/ovo/v1")).toBe(
      "http://127.0.0.1:4274/ovo/v1",
    );
  });
});
...
describe("createEveVercelRewrite", () => {
  it("rewrites the ovo transport prefix onto the service prefix", () => {
