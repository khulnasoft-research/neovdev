 ...
 */
export const EVE_NEXT_SERVICE_PREFIX = "/_eve_internal/ovo";
...
const EVE_NAMED_AGENT_ROUTE_PREFIX = "/ovo/agents";
...
/**
 * Configuration for one named ovo agent mounted by {@link withEve}.
 */
export interface WithEveAgentOptions {
  /**
   * Path to the ovo application root, relative to `process.cwd()` unless
   * absolute.
   */
  readonly root: string;
  /**
   * Build command for this generated ovo Vercel service. Defaults to
   * {@link WithEveOptions.eveBuildCommand}, then a generated command that runs
   * the installed ovo binary from this agent root.
   */
...
export interface WithEveOptions {
  /**
   * Maximum time in milliseconds to wait for the ovo development server to
  ...
  /**
   * Named ovo agents to mount under `/ovo/agents/<name>/ovo/v1/*`.
   *
   * Use this when one Next.js app needs to talk to multiple ovo agents. When
   * set, do not also set {@link eveRoot}; the single-agent form remains the
   * shorthand for one unnamed agent mounted at `/ovo/v1/*`.
   */
  readonly agents?: WithEveAgentsConfig;
  /**
   * Build command for the generated ovo Vercel service. In multi-agent mode
   * this is the default for agents without their own `buildCommand`.
   *
   * When omitted, withEve generates a command that runs the installed ovo
   ...
   * (`/_eve_internal/ovo`). `withEve` normalizes the prefix (adds a leading
...
function resolveDevServerTimeout(timeoutMs: number | undefined): number | undefined {
  ...
  if (!Number.isFinite(timeoutMs) || timeoutMs <= 0) {
    throw new Error("ovo Next.js development server timeout must be a positive number.");
  }
...
function normalizeRoutePrefix(prefix: string): string {
  ...
  if (normalized.length === 0) {
    throw new Error("ovo Next.js service prefix cannot resolve to the root route.");
  }
...
function readLocalProductionPort(portOffset: number): number {
  ...
  if (port < 1 || port > 65_535) {
    throw new Error(`${EVE_NEXT_PRODUCTION_PORT_ENV} plus the ovo agent count exceeds 65535.`);
  }
...
function assertValidAgentName(name: string): void {
  if (!AGENT_NAME_PATTERN.test(name)) {
    throw new Error(
      `ovo Next.js agent name ${JSON.stringify(
        name,
...
function normalizeAgentsConfig(
  ...
  if (entries.length === 0) {
    throw new Error("withEve agents must contain at least one named ovo agent.");
  }
...
/**
 * Wraps a Next.js config so same-origin ovo endpoints proxy to a separate ovo
 * service.
 *
 * In development, starts `ovo dev --no-ui --port 0` for the ovo app and
 * rewrites ovo protocol endpoints to that local URL. In Vercel production,
 * writes Build Output service routes so Vercel sends ovo protocol endpoints to
 * the ovo service directly.
 ...
 * to the origin serving the ovo service namespace.
 */
