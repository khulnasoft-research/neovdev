 ...
 *
 * Scenario tests that need a real on-disk app (to run `ovo build`, the Nitro
 ...
 * package manifest, the ovo tarball dependency, and (when requested) an
...
export interface ScenarioAppDescriptor {
  ...
  /**
   * Additional dependencies installed alongside `ovo`. Keys are
   * package names, values are npm version specifiers or `file:` specifiers.
   *
   * `ovo` is always wired via the test tarball and must not be
   ...
   * containing the `ovo` tarball and the requested dependencies.
   *
   * Set this for scenarios that spawn subprocesses (`ovo dev`, `ovo build`)
   * or start the Nitro dev server — anything that resolves `ovo`
 ...
 *
 * `ovo` is installed from a worker-local tarball cached across
...
export async function materializeScenarioApp(
  ...
  const appRoot = await mkdtemp(join(tmpdir(), `ovo-scenario-${descriptor.name}-`));
 ...
 * `package.json` and `ovo` tarball were already written by
...
/**
 * Returns the on-disk path to the ovo tarball used by scenario apps.
 *
...
/**
 * Returns a per-worker cache directory used to store the ovo tarball and the
...
async function resolveScenarioWorkerCacheDirectory(): Promise<string> {
  cachedScenarioWorkerCacheDirectoryPromise ??= (async () => {
    ...
    const root = join(tmpdir(), "ovo-scenario-cache", `worker-${workerId}`);
