...
} from "#execution/ovo-workflow-attributes.js";
...
function buildSerializedContext(overrides: {
  ...
  const context: Record<string, unknown> = {
    "ovo.auth": overrides.auth ?? null,
    "ovo.bundle": { source: createBundledRuntimeCompiledArtifactsSource() },
    "ovo.channel": { kind: overrides.channelKind, state: {} },
    "ovo.continuationToken": overrides.continuationToken,
    "ovo.mode": overrides.mode,
  };
  if (overrides.parent !== undefined) {
    context["ovo.parentSession"] = overrides.parent;
  }
  return context;
}
...
describe("workflowEntry integration", () => {
  ...
  it("emits `$ovo.*` session attributes onto the parent workflow run", async () => {
    ...
    await runtime.run(async () => {
      ...
      try {
        ...
        expect(attrs["$ovo.type"]).toBe("session");
        expect(attrs["$ovo.trigger"]).toBe("http");
        expect(attrs["$ovo.title"]).toContain("session tag round-trip");
        // Top-level sessions have no parent or subagent name on the root run.
        expect(attrs["$ovo.parent"]).toBeUndefined();
        expect(attrs["$ovo.subagent"]).toBeUndefined();
      } finally {
        stream.dispose();
        await run.cancel();
      }
  ...
  it("emits parent lineage onto a subagent workflow run", async () => {
    ...
    await runtime.run(async () => {
      ...
      expect(attrs["$ovo.type"]).toBe("subagent");
      expect(attrs["$ovo.parent"]).toBe("parent-session");
      expect(attrs["$ovo.parent_call"]).toBe("call-subagent-1");
      expect(attrs["$ovo.parent_turn"]).toBe("turn-parent");
      expect(attrs["$ovo.root"]).toBe("root-session");
      expect(attrs["$ovo.trigger"]).toBe("subagent");
    });
  });
});
