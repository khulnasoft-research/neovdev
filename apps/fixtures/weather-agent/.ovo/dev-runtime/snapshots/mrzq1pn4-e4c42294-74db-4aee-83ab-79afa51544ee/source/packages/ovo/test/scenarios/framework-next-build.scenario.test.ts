...
import { createNextEveProxyDescriptor } from "../../src/internal/testing/scenario-apps/next-ovo-proxy.js";
    ...
    ".vercel/project.json": `${JSON.stringify(
      {
        ...
        projectName: "next-ovo-middleware",
        settings: {
...
describe("framework-next build", () => {
  it("builds the Next.js framework fixture after regenerating ovo dist", async () => {
  ...
  it("preserves Next middleware when Vercel assembles the generated ovo service", async () => {
    ...
    await expect(
      access(join(outputRoot, "services", "ovo", "functions", "__server.func", ".vc-config.json")),
    ).resolves.toBeUndefined();
    await expect(
      access(
        join(outputRoot, "services", "ovo", "functions", "_middleware.func", ".vc-config.json"),
      ),
    ).resolves.toBeUndefined();
  }, 240_000);
});
