...
export async function compileAgentConfig(
  ...
  const definition = normalizeAgentDefinition(
    ...
    configModule === undefined
      ? `Expected the default agent config to match the public ovo shape.`
      : `Expected the agent config export "${configModule.exportName ?? "default"}" from "${configModulePath}" to match the public ovo shape.`,
  );
