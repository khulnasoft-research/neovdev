...
/**
 * Lists `ovo dev` diagnostic logs for an app root, most recent first. The
 ...
 * the file name, the transcript's `.ovo/logs/...` path, or any unambiguous
...
export function resolveDevDiagnosticLog(
  ...
  if (matches.length === 0) {
    throw new Error(
      `No diagnostic log matches "${reference}". Run \`ovo logs ls\` to see available logs.`,
    );
  }
  throw new Error(
    [
...
/**
 * `ovo logs [logid]`: prints one diagnostic log to stdout — the most recent
 ...
 * `ovo logs 2>&1 | jq` always parses; discover ids and paths with
 * `ovo logs ls`. With `--dump`, the log's environment dump (a JSON
 ...
 * processes, so window selection is by time; concurrent `ovo dev`
...
/** Machine-readable row emitted by `ovo logs ls --json`. */
...
/** `ovo logs ls`: lists diagnostic logs, most recent first. */
