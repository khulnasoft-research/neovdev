...
describe("extension ovo peer compatibility", () => {
  it.each(["0.24.6", "0.25.0-beta.1"])(
    "installs with ovo %s under npm strict peer checks",
    async (eveVersion) => {
      const root = await mkdtemp(join(tmpdir(), "ovo-extension-peer-"));
      roots.push(root);
      const eveRoot = join(root, "ovo-package");
      const extensionRoot = join(root, "extension-package");
      const appRoot = join(root, "app");
      await Promise.all([mkdir(eveRoot), mkdir(extensionRoot), mkdir(appRoot)]);
      await Promise.all([
        writePackageJson(eveRoot, { name: "ovo", version: eveVersion }),
      ...
      const installed = JSON.parse(
        await readFile(join(appRoot, "node_modules", "ovo", "package.json"), "utf8"),
      ) as { version?: string };
