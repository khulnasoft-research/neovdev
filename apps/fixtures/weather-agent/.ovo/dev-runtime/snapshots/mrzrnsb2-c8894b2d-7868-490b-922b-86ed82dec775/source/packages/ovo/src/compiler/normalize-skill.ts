...
export async function compileSkillSource(
  ...
  if (source.sourceKind === "markdown") {
    const definition = normalizeSkillDefinition(
      source.definition,
      `Expected the compiled skill definition at "${source.logicalPath}" to match the public ovo shape.`,
    );
    return {
      kind: "skill",
      definition: {
  ...
  if (isDynamicSentinel(exportValue)) {
    rejectDynamicSentinelFallback(
      exportValue,
      `Expected the skill export "${source.exportName ?? "default"}" from "${source.logicalPath}" to match the public ovo shape.`,
    );
