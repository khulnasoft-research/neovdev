...
export async function installPackageIntoProject(input: {
  ...
  console.info(
    `[ovo:dev] installing optional dependency "${input.packageName}" via \`${packageManager} ${args.join(" ")}\`...`,
  );
  ...
  console.info(`[ovo:dev] installed "${input.packageName}".`);
}

/**
 * Loads an optional engine package, auto-installing it into the
 * project when missing. Installs run only during `ovo dev`; any other
    ...
    try {
      ...
      throw new Error(
        `${input.missingMessage} Automatic installation completed, but "${input.packageName}" ` +
          ...
          `Node/package-manager environment changed while ovo dev was running. Last load error: ${toMessage(postInstallImportError)}`,
