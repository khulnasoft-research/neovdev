...
export interface OnboardingBoxesOptions {
   ...
   */
  headless?: boolean;
  /** ovo package metadata baked into the scaffolded `package.json`. */
...
export function composeOnboardingBoxes(options: OnboardingBoxesOptions): AnySetupBox<SetupState>[] {
  ...
  return [
    resolveTarget({
      ...
      // Only headless re-runs converge onto an existing ovo project directory;
    ...
    // locally through `ovo dev`. Adding channels to an existing project
    // (in-project setup, `ovo channels add`) keeps deploying for any
