...
function createCliProgram(logger: CliLogger, runtime: CliRuntimeOverrides): Command {
  ...
  program
    .name("ovo")
    .description("Build and run an ovo application.")
  ...
  extension
    .command("build")
    .description("Build the current package as an ovo extension.")
    .action(async () => {
  ...
  program
    ...
    // matching `ovo init .`.
    .command("init [target]")
    .description("Create a new ovo agent, or add one to an existing project directory.")
    ...
    .action(
      async (
        ...
        if (options.yes) {
          logger.error("warning: --yes has no effect for ovo init.");
        }
  ...
  program
    .command("start")
    .description("Start a built ovo application.")
  ...
  program
    .command("dev")
    .description("Start the ovo development server or connect to an existing URL.")
    ...
    .addHelpText(
      "after",
      "\nYou can also pass a bare URL, for example: ovo dev https://example.com\n",
    )
  ...
  const logs = program
    .command("logs")
    .description("Inspect local `ovo dev` diagnostic logs (.ovo/logs).");
  ...
  program
    .command("eval")
    .description("Run evals against an ovo agent.")
    .argument(
      "[evalIds...]",
...
export async function runCli(
  ...
  try {
    ...
    if (error instanceof CommanderError) {
      ...
      // A coding agent that fumbles `ovo init` can trip commander before the
