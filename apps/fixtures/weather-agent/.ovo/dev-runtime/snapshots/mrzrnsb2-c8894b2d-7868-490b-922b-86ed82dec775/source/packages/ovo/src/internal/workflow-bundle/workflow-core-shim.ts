...
export function sleep(): never {
  throw new Error("`sleep()` is not available in ovo workflow body bundles");
}
 ...
 * ovo workflow-body bundle does not have to pull the real type in.
 */
export interface ExperimentalSetAttributesOptions {
  /**
   * Permit attribute keys that start with the reserved `$` prefix. ovo
   * framework code passes `true` so it can write the `$ovo.*` namespace;
...
/**
 * Workflow-body implementation of `experimental_setAttributes` for the ovo
 ...
 *    ovo already relies on to materialize `"use step"` proxies).
 ...
 * `$ovo.*` reserved namespace; bouncing through the runtime's full
