 ...
 * not depend on `/ovo/v1/info`, so callers that only need a working connection
...
export class AgentInfoResponseError extends Error {
  ...
  constructor(issues: readonly string[] = []) {
    ...
    super(`The server returned an unrecognized response from the ovo agent info route.${detail}`);
