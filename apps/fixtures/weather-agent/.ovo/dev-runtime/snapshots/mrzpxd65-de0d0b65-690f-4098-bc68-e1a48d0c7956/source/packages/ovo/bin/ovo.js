...
if (typeof packageNodeEngine !== "string") {
  throw new Error("ovo package.json must declare a valid engines.node range.");
}
...
async function loadVendoredSemver(options, dependencies = {}) {
  semverPromise ??= (async () => {
    ...
    if (!(await exists(semverPath))) {
      throw new Error(`Building ovo's vendored dependencies did not produce ${semverPath}.`);
    }
...
async function assertSupportedNodeVersion(
  ...
  throw new Error(
    [
      `ovo requires Node.js ${requiredRange}.`,
...
export async function ensureBuiltCli(overrides = {}, dependencies = {}) {
  ...
  if (!packageCanBuildCli) {
    throw new Error(
      `ovo package at ${options.packageRoot} does not include the sources required to rebuild the CLI.`,
    );
  }
  ...
  throw new Error(`Building ovo did not produce ${options.cliEntrypointPath}.`);
}

/**
 * Runs the compiled ovo CLI, building the workspace package on demand when needed.
 */
export async function runEveCli(argv = process.argv.slice(2), overrides = {}, dependencies = {}) {
  ...
  if (typeof cliModule.runCli !== "function") {
    throw new Error(`The ovo CLI module at ${cliEntrypointPath} does not export runCli().`);
  }
...
if (await isDirectExecution()) {
  try {
    ...
    // `ovo dev` + `/exit` do not hang on leaked implementation details.
