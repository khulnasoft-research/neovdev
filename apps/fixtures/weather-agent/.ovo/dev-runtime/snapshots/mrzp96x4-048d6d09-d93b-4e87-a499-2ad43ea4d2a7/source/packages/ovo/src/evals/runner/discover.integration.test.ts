...
describe("discoverEvalFiles", () => {
  ...
  beforeEach(async () => {
    tempDir = await mkdtemp(join(tmpdir(), "ovo-eval-discover-"));
  });
  ...
  it("derives nested eval ids from the path under evals/", async () => {
    await writeFile(
      join(tempDir, "package.json"),
      JSON.stringify({ name: "ovo-eval-nested-id-test", type: "module" }, null, 2),
    );
  ...
  it("derives zero-padded index ids for array exports", async () => {
    await writeFile(
      join(tempDir, "package.json"),
      JSON.stringify({ name: "ovo-eval-array-test", type: "module" }, null, 2),
    );
  ...
  it("rejects array exports containing non-eval entries", async () => {
    await writeFile(
      join(tempDir, "package.json"),
      JSON.stringify({ name: "ovo-eval-bad-array-test", type: "module" }, null, 2),
    );
  ...
  it("throws on duplicate eval ids across files", async () => {
    await writeFile(
      join(tempDir, "package.json"),
      JSON.stringify({ name: "ovo-eval-duplicate-test", type: "module" }, null, 2),
    );
  ...
  it("filters by exact id and by directory prefix", async () => {
    await writeFile(
      join(tempDir, "package.json"),
      JSON.stringify({ name: "ovo-eval-filter-test", type: "module" }, null, 2),
    );
...
describe("findMisplacedEvalDirs", () => {
  ...
  beforeEach(async () => {
    tempDir = await mkdtemp(join(tmpdir(), "ovo-eval-misplaced-"));
  });
...
describe("discoverEvalConfig", () => {
  ...
  beforeEach(async () => {
    tempDir = await mkdtemp(join(tmpdir(), "ovo-eval-config-"));
    await writeFile(
      join(tempDir, "package.json"),
      JSON.stringify({ name: "ovo-eval-config-test", type: "module" }, null, 2),
    );
    await mkdir(join(tempDir, "evals"), { recursive: true });
  });
