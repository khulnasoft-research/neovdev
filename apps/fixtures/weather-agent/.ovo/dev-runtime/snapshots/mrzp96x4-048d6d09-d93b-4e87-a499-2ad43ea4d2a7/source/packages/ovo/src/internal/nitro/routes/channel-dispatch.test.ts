...
const DEVELOPMENT_ARTIFACTS_CONFIG = {
  appRoot: "/app/agent",
  devRuntimeArtifactsPointerPath: "/app/agent/.ovo/dev-runtime/current.json",
  kind: "development",
  moduleMapLoaderPath: "/ovo/src/internal/authored-module-map-loader.ts",
} as const;
...
function createEvent(input?: {
  ...
  const request = new Request("https://ovo.test/slack", { headers: input?.headers });
...
describe("dispatchChannelRequest", () => {
  ...
  it("hands websocket route handlers the same route args", async () => {
    ...
    await expect(
      Promise.resolve(hooks.upgrade?.(new Request("https://ovo.test/voice"))),
    ).resolves.toEqual({
      context: { ok: true },
    });
  ...
  it("rejects websocket upgrades when no websocket channel matches", async () => {
    ...
    let thrown: unknown;
    try {
      hooks.upgrade?.(new Request("https://ovo.test/missing"));
    } catch (error) {
      thrown = error;
    }
