 ...
 * `imports#*.js` runtime resolution, `bin/ovo.js` -> `dist/src/cli/run.js`,
 * etc.). The `ovo-source` resolver condition makes `#*.js` follow
 ...
 * every ovo dist module imports `node:module` transitively, and any
...
function createStripUnusedRolldownRuntimeImportPlugin() {
  return {
    name: "ovo-strip-unused-rolldown-runtime-import",
    renderChunk(code) {
 ...
 * into ovo's dist tree. Three categories:
 *
 ...
 *     runtime against the ovo installation.
...
function isExternalPackageSpecifier(source) {
  ...
  // `ovo-source` condition to follow these specifiers back to the
...
await buildWithNitroRolldown({
  ...
  output: {
    ...
    // builtin every time it pulled an ovo file into its graph.
