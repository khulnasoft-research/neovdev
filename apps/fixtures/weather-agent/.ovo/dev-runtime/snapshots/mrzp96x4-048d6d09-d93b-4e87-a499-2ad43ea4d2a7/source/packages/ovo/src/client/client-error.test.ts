...
describe("ClientError", () => {
  it("uses structured ovo JSON error bodies as the public message", () => {
  ...
  it("preserves normalized response headers", () => {
    const source = new Headers({ Location: "https://vercel.com/sso-api?url=https://ovo.test" });
    const error = new ClientError(302, "Redirecting...", source);
    source.set("location", "https://example.com");

    expect(error.headers).toEqual({
      location: "https://vercel.com/sso-api?url=https://ovo.test",
    });
  });
});
