 ...
 * crashed `ovo dev` never closes its run streams, so a live reader on
 ...
 * diagnostic-log line so `ovo logs --events` output stays one uniform JSONL
 ...
 * workflow store (`.ovo/.workflow-data`), at query time — nothing is
 ...
 * time window: events from any `ovo dev` process in the window are
