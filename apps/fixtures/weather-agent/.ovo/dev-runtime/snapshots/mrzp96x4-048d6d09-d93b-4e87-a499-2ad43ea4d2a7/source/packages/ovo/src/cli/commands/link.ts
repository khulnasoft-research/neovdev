...
/**
 * `ovo link`: pick a Vercel team, then create or select a project (re-linking
...
export async function runLinkCommand(
  ...
  if (!dependencies.hasInteractiveTerminal()) {
    logger.error(
      "`ovo link` needs an interactive terminal to pick the team and project. In CI, run `vercel link --project <name> --yes --non-interactive` instead.",
    );
    process.exitCode = 1;
    return;
  }
  ...
  prompter.intro("Link your ovo agent to Vercel");
  try {
