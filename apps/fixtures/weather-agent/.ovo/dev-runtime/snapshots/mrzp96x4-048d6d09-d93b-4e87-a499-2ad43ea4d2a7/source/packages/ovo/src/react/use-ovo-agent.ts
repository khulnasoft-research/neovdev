...
/**
 * Lifecycle status of an ovo agent session.
 *
...
/**
 * Snapshot of an ovo agent session: `data` (the reducer projection), `events`
...
/**
 * Configuration for creating or binding a React ovo agent session.
 *
...
export interface UseEveAgentOptions<TData> extends EveAgentStoreCallbacks<TData> {
  ...
  /**
   * Base URL for ovo client requests. Do not combine with `agent`.
   *
   * Defaults to same-origin ovo routes such as `/ovo/v1/...`. Pass a same-origin
   * prefix such as `/api` for an app-owned proxy, or an absolute origin to talk
   * to an ovo server directly.
   *
   * @default ""
   */
   ...
   * exposed through `events`, which remains the authoritative ovo stream.
   *
   * @default true
   */
...
/**
 * React hook that drives an ovo session and projects its event stream into UI data.
 *
