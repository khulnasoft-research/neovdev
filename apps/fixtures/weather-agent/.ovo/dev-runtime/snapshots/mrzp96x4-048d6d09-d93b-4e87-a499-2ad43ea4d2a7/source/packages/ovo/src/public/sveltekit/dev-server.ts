  ...
  while (true) {
    try {
      ...
      if (Date.now() > deadline) {
        throw new Error(
          `Timed out after ${DEV_SERVER_REGISTRY_TIMEOUT_MS}ms waiting for another SvelteKit process to start ovo.`,
        );
      }
...
function startServerProcess(input: {
  ...
  return new Promise((resolvePromise, reject) => {
    ...
    const timeout = setTimeout(() => {
      ...
      reject(
        new Error(
          `Timed out after ${DEFAULT_SERVER_READY_TIMEOUT_MS}ms waiting for ovo to print its server URL.`,
        ),
      );
...
/**
 * Resolve a shared ovo dev server for {@link appRoot}, reusing a healthy
 * registered server when one exists and otherwise spawning a new one behind a
 * cross-process lock so concurrent SvelteKit processes don't each boot ovo.
 */
