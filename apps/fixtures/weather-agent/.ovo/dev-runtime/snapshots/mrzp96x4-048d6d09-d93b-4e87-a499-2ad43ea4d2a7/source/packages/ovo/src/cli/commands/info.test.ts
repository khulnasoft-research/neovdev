...
const MESSAGING = {
  createSessionRoutePath: "/ovo/v1/session",
  continueSessionRoutePattern: "/ovo/v1/session/:id",
  streamRoutePattern: "/ovo/v1/session/:id/stream",
};
...
function makeCompiledState(
  ...
  const channels: CompiledChannelEntry[] = [
    ...
    },
    {
      kind: "channel",
      name: "ovo",
      logicalPath: "agent/channels/ovo.ts",
      method: "POST",
      urlPath: "/ovo/v1/session",
      sourceId: "memory::ovo",
      sourceKind: "module",
      adapterKind: "http",
    },
  ...
  return {
    ...
    metadata: {
      ...
      generator: { name: "ovo", version: "0.0.0-test" },
    ...
    paths: {
      appRoot: APP_ROOT,
      compiledManifestPath: `${APP_ROOT}/.ovo/compile/compiled-agent-manifest.json`,
      compileDirectoryPath: `${APP_ROOT}/.ovo/compile`,
      compileMetadataPath: `${APP_ROOT}/.ovo/compile/compile-metadata.json`,
      diagnosticsPath: `${APP_ROOT}/.ovo/discovery/diagnostics.json`,
      discoveryManifestPath: `${APP_ROOT}/.ovo/discovery/agent-discovery-manifest.json`,
      discoveryDirectoryPath: `${APP_ROOT}/.ovo/discovery`,
      moduleMapPath: `${APP_ROOT}/.ovo/compile/module-map.mjs`,
    },
...
describe("buildApplicationInfoJson", () => {
  test("projects a compiled agent into the JSON contract", () => {
    ...
    expect(json.channels).toEqual([
      { name: "slack", kind: "slack", method: "POST", urlPath: "/ovo/v1/slack" },
      { name: "ovo", kind: "http", method: "POST", urlPath: "/ovo/v1/session" },
    ]);
    expect(json.messaging.create).toBe("/ovo/v1/session");
  ...
  test("reports an unavailable contract when the project is not compiled", () => {
    ...
    expect(json.messaging.stream).toBe("/ovo/v1/session/:id/stream");
  });
});
