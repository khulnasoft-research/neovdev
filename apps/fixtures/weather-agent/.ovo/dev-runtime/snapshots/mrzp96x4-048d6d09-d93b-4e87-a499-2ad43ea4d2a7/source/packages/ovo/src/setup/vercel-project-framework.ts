...
const EVE_FRAMEWORK_PRESET = "ovo";
...
const EVE_FRAMEWORK_INTEGRATIONS: Readonly<Record<string, EveFrameworkIntegration>> = {
  nextjs: { label: "Next.js", importSpecifier: "ovo/next" },
  nuxt: { label: "Nuxt", importSpecifier: "ovo/nuxt" },
  nuxtjs: { label: "Nuxt", importSpecifier: "ovo/nuxt" },
  sveltekit: { label: "SvelteKit", importSpecifier: "ovo/sveltekit" },
};
...
export interface CreatedProjectFrameworkOptions extends VercelProjectOperationOptions {
  /** Detects whether project sources already use the ovo integration for a host framework. */
  detectFrameworkIntegrationImport?: (
    projectRoot: string,
    importSpecifier: string,
  ) => Promise<boolean>;
  /** Never prompt; ambiguous host framework detections fall back to the ovo preset. */
  headless?: boolean;
}
    ...
    options: [
      {
        ...
        hint: `Use this for a host app that integrates ovo through ${integration.importSpecifier}.`,
      },
      {
        value: false,
        label: "No, switch to ovo",
        hint: "Use this for a standalone ovo agent.",
      },
    ],
  });
  return keepDetected ? "keep" : "switch-to-ovo";
}
...
async function resolveDetectedFrameworkAction(
  ...
  if (integration === undefined) return "switch-to-ovo";
  ...
  if (options.headless) return "switch-to-ovo";
 ...
 *
 * A project created as a standalone ovo agent keeps the `ovo` preset; adding a
 * host framework via `ovo channels add web` leaves it stale, so the deploy would
...
export async function ensureCreatedProjectFramework(
  ...
  if (frameworkAction === "switch-to-ovo") {
    await setProjectFramework(
      projectRoot,
      team,
      projectId,
