...
const TOKEN_SOURCES: Readonly<Record<string, TokenSource>> = {
  [versionToken("EVE_PACKAGE")]: { kind: "ovo-version" },
  [NODE_ENGINE_TOKEN]: { kind: "ovo-node-engine" },
...
function resolveTokenFromDevTree(token: string): string | undefined {
  ...
  try {
    ...
    if (source.kind === "ovo-version") {
      const packageJson = JSON.parse(readFileSync(join(packageRoot, "package.json"), "utf8")) as {
        version?: unknown;
      };
      return typeof packageJson.version === "string" ? packageJson.version : undefined;
    }
    if (source.kind === "ovo-node-engine") {
      const packageJson = JSON.parse(readFileSync(join(packageRoot, "package.json"), "utf8")) as {
  ...
  if (resolved === undefined) {
    throw new Error(
      `Scaffold received unstamped version token (${field}=${value}). ` +
        "Build ovo before using its dist entrypoint.",
    );
  }
