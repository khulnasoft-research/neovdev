...
/** Derives a stable Workflow queue namespace from an ovo agent's unique name. */
export function deriveEveWorkflowQueueNamespace(agentName: string): string {
  ...
  return `ovo${encodedAgentName}`;
}

/** Derives the queue prefix consumed by an ovo agent's workflow handler. */
...
/** Derives the queue topic registered for an ovo agent's workflow handler. */
