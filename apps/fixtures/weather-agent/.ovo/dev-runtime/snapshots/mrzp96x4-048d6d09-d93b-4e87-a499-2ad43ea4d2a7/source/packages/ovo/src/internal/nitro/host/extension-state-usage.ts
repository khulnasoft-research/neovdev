...
const STATE_MODULE = "ovo/context";
const STATE_EXPORT = "defineState";

/**
 * Detects whether an authored extension tree calls `defineState` from
 * `ovo/context`. Usage is followed through local re-export barrels — aliased
 ...
 * `ovo/context`'s `defineState`, under whatever names the barrels chose.
 */
...
/** `./lib/ovo.js` in nodenext-style source resolves to `lib/ovo.ts` on disk. */
