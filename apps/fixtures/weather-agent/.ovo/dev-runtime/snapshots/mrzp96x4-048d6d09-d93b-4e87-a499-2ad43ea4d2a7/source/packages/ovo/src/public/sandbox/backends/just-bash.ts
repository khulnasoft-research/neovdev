 ...
 * over a virtual filesystem stored under `.ovo/sandbox-cache/`. It
 * needs no daemon or VM, but commands run in a simulated shell — no
 * real binaries (`git`, `node`, package managers) and no network
 * isolation.
 *
 * The `just-bash` package is not bundled with ovo. When it is missing,
 * `ovo dev` installs it into the application automatically (disable
