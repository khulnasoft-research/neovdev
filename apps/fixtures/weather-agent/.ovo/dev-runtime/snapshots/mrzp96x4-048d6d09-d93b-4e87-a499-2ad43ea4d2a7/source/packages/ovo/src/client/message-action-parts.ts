...
export function mergeToolMetadata(
  ...
  const kind = next.ovo?.kind ?? current?.ovo?.kind ?? "unknown";
  const name = next.ovo?.name ?? current?.ovo?.name ?? "unknown";

  return {
    ovo: {
      ...current?.ovo,
      ...next.ovo,
      inputRequest: next.ovo?.inputRequest ?? current?.ovo?.inputRequest,
      inputResponse: next.ovo?.inputResponse ?? current?.ovo?.inputResponse,
      kind,
      name,
    },
  };
  ...
  switch (action.kind) {
    ...
    case "subagent-call":
      return {
        ...
        toolName: `ovo:subagent:${action.subagentName}`,
      };
    case "remote-agent-call":
      return {
        kind: "subagent-call",
        name: action.remoteAgentName,
        toolName: `ovo:subagent:${action.remoteAgentName}`,
      };
  }
}
  ...
  switch (result.kind) {
    ...
    case "subagent-result":
      return {
        ...
        toolName: `ovo:subagent:${result.subagentName}`,
      };
  }
}
