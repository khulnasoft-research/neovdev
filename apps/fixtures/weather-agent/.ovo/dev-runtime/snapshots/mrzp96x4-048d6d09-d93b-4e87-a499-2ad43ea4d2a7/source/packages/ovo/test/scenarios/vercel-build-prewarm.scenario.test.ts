...
describe("Vercel build-time sandbox prewarm", () => {
  ...
  it("prewarms root and subagent sandbox templates without running onSession", async () => {
    ...
    expect(events.templateKeys.every((templateKey) => templateKey.startsWith("ovo-sbx-tpl-"))).toBe(
      true,
    );
    expect([...events.commands].sort()).toEqual(["echo child-bootstrap", "echo root-bootstrap"]);
    expect(log.mock.calls.map(([message]) => message)).toEqual([
      "ovo: initializing 2 sandbox templates...",
      "ovo: initialized 2 sandbox templates (0 reused, 2 built).",
    ]);
  });
...
async function createTemplateFreeScenarioAppRoot(): Promise<string> {
  const appRoot = await createScratchDirectory("ovo-vercel-build-without-prewarm-");
...
async function createScenarioAppRoot(
  ...
  const appRoot = await createScratchDirectory("ovo-vercel-build-prewarm-");
  const agentRoot = join(appRoot, "agent");
