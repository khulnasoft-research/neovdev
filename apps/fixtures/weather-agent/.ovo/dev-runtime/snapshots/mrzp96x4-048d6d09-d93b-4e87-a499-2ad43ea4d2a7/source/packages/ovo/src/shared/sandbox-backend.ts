...
export interface SandboxBackendHandle<SO = Record<string, never>> {
  ...
  /**
   * Stops the underlying compute because the ovo server is shutting
 ...
 * native tagging system. ovo supplies stable tags such as the active
...
export interface SandboxBackendCreateInput {
  /**
   * Reusable template key to open this session from. `null` means ovo
 ...
 * sandbox. ovo ships built-in backends (`docker()`,
...
export interface SandboxBackend<BO = Record<string, never>, SO = Record<string, never>> {
  ...
  /**
   * Build-time prewarm hook. ovo invokes this for every authored
