...
function mockArtifacts(): void {
  mockedRunnerDependencies.resolveArtifactDirectory.mockReturnValue("/tmp/ovo-evals");
...
describe("runEvals", () => {
  ...
  it("writes run artifacts once with the aggregated summary", async () => {
    ...
    expect(mockedRunnerDependencies.writeArtifacts).toHaveBeenCalledWith("/tmp/ovo-evals", summary);
  });
});
