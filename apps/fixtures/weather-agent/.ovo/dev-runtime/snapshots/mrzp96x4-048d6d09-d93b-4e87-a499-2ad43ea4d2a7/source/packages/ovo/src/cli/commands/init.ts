  ...
  if (options.channelWebNextjs === true) {
    throw new Error(
      "`--channel-web-nextjs` is not supported when adding an agent to an existing project. " +
        "Run `ovo channels add web` from the project afterwards instead.",
    );
  }
...
async function scaffoldProject(
  ...
  const stagingDirectory = await mkdtemp(join(parentPath, ".ovo-init-"));
...
/**
 * Creates a new ovo agent (`target` is a project name), or adds one to an
 ...
 * nothing scaffolded, since a bare `ovo init` means it has not yet chosen what to
 * build.
 *
 * For extension packages, use `ovo extension init` instead.
 */
export async function runInitCommand(
  ...
  // A coding agent that runs `ovo init` with no target has not decided what to
  ...
  // Strictly the ovo binary, never the project's dev script, which in an
  ...
  logger.log(pc.dim(freshScaffold ? "$ ovo dev --input /model" : "$ ovo dev"));
  if (
