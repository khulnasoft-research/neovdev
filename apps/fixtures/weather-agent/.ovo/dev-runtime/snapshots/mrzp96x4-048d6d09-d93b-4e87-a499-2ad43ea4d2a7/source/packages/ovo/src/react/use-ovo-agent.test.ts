...
import { useEveAgent, type UseEveAgentHelpers } from "#react/use-ovo-agent.js";
...
describe("useEveAgent", () => {
  ...
  it("sends a message and projects streamed events with the default reducer", async () => {
    const events = [
      ...
      createSessionWaitingEvent("ovo:http:session_1"),
    ];
  ...
  it("prepares fresh clientContext before sending without projecting it optimistically", async () => {
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        createEagerStreamResponse([createSessionWaitingEvent("ovo:http:session_1")]),
      );
  ...
  it("targets named agent routes when agent is configured", async () => {
    ...
    expect(fetchMock.mock.calls[0]?.[0]).toBe("/ovo/agents/support/ovo/v1/session");
    expect(fetchMock.mock.calls[1]?.[0]).toBe(
      "/ovo/agents/support/ovo/v1/session/session_1/stream",
    );
  });
