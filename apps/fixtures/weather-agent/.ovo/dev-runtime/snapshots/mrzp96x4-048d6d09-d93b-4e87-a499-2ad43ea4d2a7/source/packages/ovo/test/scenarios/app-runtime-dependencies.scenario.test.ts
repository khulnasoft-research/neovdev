...
describe("app runtime dependency tracing", () => {
  ...
  it("bundles authored app runtime dependencies without forcing traced node_modules copies", async () => {
    const appRoot = await createScratchDirectory("ovo-app-runtime-dep-build-");
  ...
  it("provides CommonJS path globals to bundled hosted dependencies", async () => {
    const appRoot = await createScratchDirectory("ovo-app-runtime-cjs-path-globals-build-");
  ...
  it("bundles Workflow sandbox worker assets only when an agent enables Workflow", async () => {
    async function createWorkflowAssetsApp(label: string, workflow: boolean): Promise<string> {
      const appRoot = await createScratchDirectory(`ovo-app-workflow-assets-${label}-build-`);
      ...
      if (workflow) {
        ...
        await writeFile(
          ...
          'export default { kind: "ovo:enable-workflow-tool", maxSubagents: 6 };\n',
        );
      }
  ...
  it("includes the optional just-bash engine in hosted output only when the sandbox config selects it", async () => {
    async function createMinimalApp(input: {
      ...
      const appRoot = await createScratchDirectory(`ovo-optional-engine-${input.label}-`);
      ...
      if (input.justBashEngine) {
        ...
        // modules resolve `ovo/sandbox` through the app's node_modules,
        // so link the workspace package into the scratch app.
        await mkdir(join(appRoot, "node_modules"), { recursive: true });
        await symlink(EVE_PACKAGE_ROOT, join(appRoot, "node_modules", "ovo"), "dir");
        await writeFile(
          join(appRoot, "agent", "sandbox.ts"),
          [
            'import { defineSandbox } from "ovo/sandbox";',
            'import { justbash } from "ovo/sandbox/just-bash";',
            "",
    ...
    // workspace (it is an ovo devDependency), but resolvability is not
  ...
  it("traces additional hosted build dependencies configured in agent.ts", async () => {
    const appRoot = await createScratchDirectory("ovo-app-build-trace-dep-build-");
  ...
  it("traces hosted external dependencies configured in agent.ts", async () => {
    const appRoot = await createScratchDirectory("ovo-app-build-external-dep-build-");
  ...
  it("rewrites framework tool executors into hosted Vercel output", async () => {
    ...
    const appRoot = await createScratchDirectory("ovo-app-hosted-framework-tools-build-");
  ...
  it("does not bundle local-only runtime infrastructure into hosted Vercel output", async () => {
    ...
    const appRoot = await createScratchDirectory("ovo-app-hosted-no-dev-runtime-build-");
    ...
    expect(vercelFunctionsSource).not.toContain("[ovo:dev]");
  ...
  it("loads instrumentation runtime dependencies from hosted Vercel output", async () => {
    ...
    const appRoot = await createScratchDirectory("ovo-app-hosted-instrumentation-dep-build-");
