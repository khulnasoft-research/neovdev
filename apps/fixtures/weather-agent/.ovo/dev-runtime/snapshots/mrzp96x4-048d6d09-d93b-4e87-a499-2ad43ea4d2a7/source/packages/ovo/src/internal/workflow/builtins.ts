/**
 * Built-in Workflow response body steps used by ovo-owned workflow bundles.
 *
 * These mirror Workflow's tiny `workflow/internal/builtins` module without
 * requiring ovo to depend on Workflow's umbrella package.
 */
 ...
 * ovo-vendored builtins module that the registry visits. The ovo bundler
...
export async function __builtin_set_attributes(
  ...
  if (typeof world?.runs?.experimentalSetAttributes !== "function") {
    if (g[EVE_UNSUPPORTED_WORLD_WARNED] !== true) {
      ...
      console.warn(
        `[ovo] setAttributes: the current world implementation${worldName} does not implement experimentalSetAttributes; this call (and any subsequent setAttributes calls in this process) is a no-op. Attributes will become available once the world adapter adds support.`,
      );
    }
    return;
  }
  ...
  try {
    ...
    console.error(
      `[ovo] setAttributes: failed to post tags after ${EVE_INTERNAL_ATTRIBUTES_MAX_ATTEMPTS} attempts; dropping the internal attribute write. ${formatUnknownError(error)}`,
    );
  }
}
