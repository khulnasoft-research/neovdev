...
const mocks = vi.hoisted(() => ({
  buildAgentInfoResponseFromManifest: vi.fn(() => ({ kind: "ovo-agent-info", version: 1 })),
  ...
  resolveAgentInfoCompiledArtifactsSource: vi.fn(() => ({
    appRoot: "/tmp/app/.ovo/dev-runtime/snapshots/current/app",
    kind: "disk" as const,
  })),
}));
...
const ROUTE_INPUT = {
  appRoot: "/tmp/app",
  devRuntimeArtifactsPointerPath: "/tmp/app/.ovo/dev-runtime/current.json",
  kind: "development",
  moduleMapLoaderPath: "/tmp/ovo/src/internal/authored-module-map-loader.ts",
} as const;
