...
describe("Client request policy", () => {
  it("includes host query parameters on every agent request", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        Response.json({ continuationToken: "ovo:test", sessionId: "session_1" }, { status: 202 }),
      )
      .mockResolvedValueOnce(
        new Response(`${JSON.stringify({ data: {}, type: "session.completed" })}\n`),
      );
    const client = new Client({
      host: "https://ovo.test?x-vercel-protection-bypass=secret",
    });
  ...
  it("enforces its redirect policy for info, health, raw fetch, and sessions", async () => {
    ...
    const client = new Client({ host: "https://ovo.test", redirect: "manual" });
  ...
  it("includes response headers in info request errors", async () => {
    vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response("Redirecting...", {
        status: 302,
        headers: { location: "https://vercel.com/sso-api?url=https://ovo.test" },
      }),
    );
    const client = new Client({ host: "https://ovo.test", redirect: "manual" });
    ...
    expect((error as ClientError).headers.location).toBe(
      "https://vercel.com/sso-api?url=https://ovo.test",
    );
  });
  ...
  it("keeps an in-flight remote request on one credential snapshot after rollback", async () => {
    ...
    const target = await resolveTestVercelTarget({ host: "ovo.test", projectId: "prj_eve" });
    const credentials = createDevelopmentCredentialGate("https://ovo.test");
    const rollback = credentials.authorize({
      target,
      resolveToken: async () => "candidate-token",
    });
    const client = new Client(
      resolveRemoteDevelopmentClientOptions({ credentials, serverUrl: "https://ovo.test" }),
    );
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(new Response(null));

    const request = client.fetch("/ovo/v1/info");
    rollback();
    await request;
  ...
  it("accepts a tool whose undefined output schema was omitted during JSON serialization", async () => {
    const toolWithoutOutputSchema = {
      ...
      inputSchema: null,
      logicalPath: "ovo:framework/web-search",
      name: "web_search",
      origin: "framework",
    ...
    const client = new Client({ host: "https://ovo.test" });
  ...
  it("rejects a non-Ovo response from the agent info route", async () => {
    vi.spyOn(globalThis, "fetch").mockResolvedValue(
      Response.json({ kind: "ovo-agent-info", version: 1 }),
    );
    const client = new Client({ host: "https://ovo.test" });
