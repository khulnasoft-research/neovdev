...
async function createNextStyleImportSnapshotFixture(): Promise<{ readonly appRoot: string }> {
  const appRoot = await createScratchDirectory("ovo-dev-runtime-next-imports-");
  const agentRoot = join(appRoot, "agent");
  const compileDirectoryPath = join(appRoot, ".ovo", "compile");
  ...
  await mkdir(join(appRoot, "src", "features", "editor", "ovo"), { recursive: true });
  ...
  await writeFile(
    ...
    [
      ...
      'import { authSessionAuth } from "@/features/editor/ovo/auth-session";',
      "",
  ...
  await writeFile(
    join(appRoot, "src", "features", "editor", "ovo", "auth-session.ts"),
...
describe("development runtime artifact snapshots", () => {
  it("stages snapshots without moving the latest runtime pointer", async () => {
    const appRoot = await createScratchDirectory("ovo-dev-runtime-artifacts-stage-");
    const agentRoot = join(appRoot, "agent");
    const compileDirectoryPath = join(appRoot, ".ovo", "compile");
  ...
  it("restores the prior runtime pointer when activation is rolled back", async () => {
    const appRoot = await createScratchDirectory("ovo-dev-runtime-activation-rollback-");
    const agentRoot = join(appRoot, "agent");
    const compileDirectoryPath = join(appRoot, ".ovo", "compile");
    await mkdir(agentRoot, { recursive: true });
    await mkdir(compileDirectoryPath, { recursive: true });
  ...
  it("stages package-less flat markdown agents with generated runtime package metadata", async () => {
    const appRoot = await createScratchDirectory("ovo-dev-runtime-flat-package-less-");
    const compileDirectoryPath = join(appRoot, ".ovo", "compile");
  ...
  it("excludes generated output and dependency directories from source snapshots", async () => {
    const appRoot = await createScratchDirectory("ovo-dev-runtime-generated-output-");
    const agentRoot = join(appRoot, "agent");
    const compileDirectoryPath = join(appRoot, ".ovo", "compile");
    const manifestPath = join(compileDirectoryPath, "compiled-agent-manifest.json");
  ...
  it("mounts declared physical dependency directories into source snapshots", async () => {
    const appRoot = await createScratchDirectory("ovo-dev-runtime-physical-dependency-");
    const agentRoot = join(appRoot, "agent");
    const compileDirectoryPath = join(appRoot, ".ovo", "compile");
  ...
  it("stops copying at nested git repository boundaries", async () => {
    const appRoot = await createScratchDirectory("ovo-dev-runtime-nested-git-");
    const agentRoot = join(appRoot, "agent");
    const compileDirectoryPath = join(appRoot, ".ovo", "compile");
  ...
  it("prunes stale snapshots while preserving the active and recent snapshots", async () => {
    const appRoot = await createScratchDirectory("ovo-dev-runtime-artifacts-prune-");
    const snapshotsRoot = join(appRoot, ".ovo", "dev-runtime", "snapshots");
  ...
  it("records when the active generation is superseded", async () => {
    const appRoot = await createScratchDirectory("ovo-dev-runtime-activated-retention-");
    const snapshotsRoot = join(appRoot, ".ovo", "dev-runtime", "snapshots");
  ...
  it("retains the active, five newest retired, and recently retired generations", async () => {
    const appRoot = await createScratchDirectory("ovo-dev-runtime-retired-pruning-");
    const snapshotsRoot = join(appRoot, ".ovo", "dev-runtime", "snapshots");
    const activeSnapshotRoot = join(snapshotsRoot, "active");
  ...
  it("starts a full grace period for an activated generation from an older format", async () => {
    const appRoot = await createScratchDirectory("ovo-dev-runtime-legacy-retirement-");
    const snapshotsRoot = join(appRoot, ".ovo", "dev-runtime", "snapshots");
    const activeSnapshotRoot = join(snapshotsRoot, "active");
  ...
  it("applies retirement policy without inspecting local Workflow storage", async () => {
    const appRoot = await createScratchDirectory("ovo-dev-runtime-world-independent-pruning-");
    const snapshotsRoot = join(appRoot, ".ovo", "dev-runtime", "snapshots");
    const activeSnapshotRoot = join(snapshotsRoot, "active");
  ...
  it("removes a partially staged snapshot when staging fails", async () => {
    const appRoot = await createScratchDirectory("ovo-dev-runtime-artifacts-failed-stage-");
    const agentRoot = join(appRoot, "agent");
    const compileDirectoryPath = join(appRoot, ".ovo", "compile");
    const manifestPath = join(compileDirectoryPath, "compiled-agent-manifest.json");
    ...
    await expect(readdir(join(appRoot, ".ovo", "dev-runtime", "snapshots"))).resolves.toEqual([]);
  });

  it("freezes authored source and rewrites runtime manifest roots for new sessions", async () => {
    const appRoot = await createScratchDirectory("ovo-dev-runtime-artifacts-");
    const agentRoot = join(appRoot, "agent");
    const compileDirectoryPath = join(appRoot, ".ovo", "compile");
    ...
    await expect(
      ...
      appRoot,
      kind: "ovo-dev-runtime-artifacts-pointer",
    ...
    await expect(
      readFile(
        join(snapshot.runtimeAppRoot, ".ovo", "compile", "compiled-agent-manifest.json"),
        "utf8",
      ),
  ...
  it("keeps compatibility with v1 dev runtime pointers", async () => {
    const appRoot = await createScratchDirectory("ovo-dev-runtime-artifacts-pointer-v1-");
    const runtimeAppRoot = join(appRoot, ".ovo", "dev-runtime", "snapshots", "legacy");
    const pointerPath = resolveDevelopmentRuntimeArtifactsPointerPath(appRoot);

    await mkdir(join(appRoot, ".ovo", "dev-runtime"), { recursive: true });
    await writeFile(
      pointerPath,
      `${JSON.stringify(
        {
  ...
  it("preserves workspace-relative tsconfig extends in runtime snapshots", async () => {
    const workspaceRoot = await createScratchDirectory("ovo-dev-runtime-workspace-");
  ...
  it("freezes local workspace packages resolved through app node_modules symlinks", async () => {
    const workspaceRoot = await createScratchDirectory("ovo-dev-runtime-linked-package-");
