...
describe("defineState", () => {
  it("get() throws outside ALS", () => {
    ...
    expect(() => counter.get()).toThrow(/No active ovo context/);
  });

  it("rejects names using the reserved ovo. prefix", () => {
    // Authoring `defineState("ovo.channel", ...)` would otherwise register a
    // codec-less key over the framework's codec-carrying internal key and
    // silently corrupt context serialization.
    expect(() => defineState("ovo.channel", () => null)).toThrow(/reserved/);
    expect(() => defineState("ovo.budget", () => ({ count: 0 }))).toThrow(/reserved/);
  });
  ...
  it("update() throws outside ALS", () => {
    ...
    expect(() => counter.update((s) => ({ count: s.count + 1 }))).toThrow(/No active ovo context/);
  });
