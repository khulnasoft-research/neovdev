/**
 * Public, versioned summary of an ovo agent emitted by `ovo`
 * during `ovo build`.
 *
 * This schema is the stable contract between ovo and Vercel platform
 ...
 * dedicated `/v6/deployments/:id/files/ovo-agent-summary` endpoint with
 ...
 * (`<appRoot>/.ovo/agent-summary.json`) and is consumed by whatever
 ...
 */
export const VERCEL_EVE_AGENT_SUMMARY_KIND = "vercel-ovo-agent-summary" as const;
...
/**
 * Output path (relative to the agent's `appRoot`) where ovo writes the
 * summary file at build time.
 *
 * Lives under `.ovo/` — the existing ovo-internal cache namespace that
 * already holds `nitro/`, `nitro-output/`, `sandbox-cache/`, etc. (see
 * `packages/ovo/src/internal/application/paths.ts`). Intentionally
 ...
 */
export const VERCEL_EVE_AGENT_SUMMARY_OUTPUT_PATH = ".ovo/agent-summary.json";

/**
 * Display category ovo exposes to the dashboard for one channel chip. Built
...
export interface VercelEveConnectionEntry {
   ...
   * `@vercel/connect/ovo`, the connector identifier the author passed
...
/**
 * Versioned public summary of one ovo agent, emitted into the Vercel Build
 * Output during `ovo build` and ingested by Vercel as deployment metadata.
 */
export interface VercelEveAgentSummary {
  ...
  /**
   * Version of the `ovo` package that produced this summary,
