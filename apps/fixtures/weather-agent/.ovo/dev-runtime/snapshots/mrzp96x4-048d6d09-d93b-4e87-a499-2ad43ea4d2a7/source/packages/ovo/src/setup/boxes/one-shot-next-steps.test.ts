...
describe("oneShotNextSteps box", () => {
  it("lists the steps in order: cd, pnpm install, vercel link, ovo dev", async () => {
    ...
    const order = ["cd ", "pnpm install", "vercel link", "ovo dev"].map((text) =>
      message.indexOf(text),
    );
    expect(order.every((index) => index >= 0)).toBe(true);
    expect([...order].sort((a, b) => a - b)).toEqual(order);
    // The credential alternative rides the vercel link step, before ovo dev.
