...
export const CACHED_CHANNEL_PREFIX = "ovo-cached-channel:";
...
export function createGenerationPackageBoundaryPlugin(input: {
  ...
  return {
    name: "ovo-generation-package-boundary",
    async resolveId(
...
export function createRuntimeLoaderPackageBoundaryPlugin(input: {
  ...
  return {
    name: "ovo-runtime-loader-package-boundary",
    async resolveId(
      this: RolldownResolveContext,
      source: string,
      importer: string | undefined,
        ...
        if (resolved === null || typeof resolved.id !== "string") {
          ...
          throw new Error(
            `Cannot resolve package "${source}" imported from "${importerPath}". ` +
              ...
              `a running \`ovo dev\` retries on the next rebuild.`,
          );
        }
...
export function createDistributionPackageBoundaryPlugin(input: {
  ...
  return {
    name: "ovo-extension-distribution-package-boundary",
    async resolveId(
      this: RolldownResolveContext,
      source: string,
      importer: string | undefined,
...
function isFrameworkRuntimeImport(source: string, importer: string | undefined): boolean {
  if (source === "ovo" || source.startsWith("ovo/")) {
    return true;
  }
  ...
  // bundle keep their own copies instead: ovo vendors `@workflow/*`, so a
