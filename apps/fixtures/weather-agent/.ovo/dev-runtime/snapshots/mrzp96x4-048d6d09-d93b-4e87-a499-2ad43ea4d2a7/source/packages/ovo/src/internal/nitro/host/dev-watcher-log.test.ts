...
describe("formatChangeDetectedLogLine", () => {
  it("displays paths inside the app root relative to it", () => {
    ...
    expect(line).toBe(
      "[ovo:dev] change detected (1 event: change agent/agent.ts), rebuilding authored artifacts...",
    );
  });
...
describe("parseDevRebuildLogLine", () => {
  ...
  it("recognizes the lifecycle outcome lines", () => {
    ...
    expect(parseDevRebuildLogLine("[ovo:dev] rebuild failed: boom")).toEqual({
      kind: "failed",
      message: "boom",
    });
    expect(parseDevRebuildLogLine("[ovo:dev] rebuild queue error: queue boom")).toEqual({
      kind: "failed",
      message: "queue boom",
    });
  });
