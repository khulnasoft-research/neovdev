...
/**
 * Creates a UIMessage-compatible ovo reducer for chat and agent UIs.
 *
 * The returned projection keeps ovo-owned types while following the AI SDK
  ...
  return updateToolPart(data, existing.toolCallId, {
    ...
    toolMetadata: mergeToolMetadata(existing.toolMetadata, {
      ovo: {
        ...
        kind: existing.toolMetadata?.ovo?.kind ?? "unknown",
        name: existing.toolMetadata?.ovo?.name ?? existing.toolName,
      },
    }),
