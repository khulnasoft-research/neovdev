...
describe("stageAttachmentsToSandbox (integration)", () => {
  it("writes FilePart bytes into the active sandbox and rewrites data to an ovo-sandbox: ref", async () => {
    ...
    // Staged `data` is an ovo-sandbox: ref (NOT raw bytes). The
  ...
  it("resolves URL FileParts via the bound adapter's fetchFile", async () => {
    ...
    // `data` is replaced with an ovo-sandbox: ref — the bytes live in
...
describe("hydrateSandboxAttachments (integration)", () => {
  ...
  it("hydrates small images inline as bytes — provider consumes them multimodally", async () => {
    ...
    // still carries an ovo-sandbox: URL, not bytes, so it remains
  ...
  it("degrades to a text reference when an inlinable sandbox ref points at a missing file", async () => {
    ...
    const danglingRef = new URL(
      "ovo-sandbox:?path=%2Fworkspace%2Fattachments%2Fdeadbeefdeadbeef%2Fghost.png&size=5&type=image%2Fpng",
    );
    const messages = [
      {
        content: [
