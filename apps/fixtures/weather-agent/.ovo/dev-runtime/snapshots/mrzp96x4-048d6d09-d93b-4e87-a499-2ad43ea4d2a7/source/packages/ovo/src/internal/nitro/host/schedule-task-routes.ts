 ...
 * ovo authored schedule. Each registered task points at its own virtual id so
 * Rollup can resolve the generated `defineTask({...})` source without writing
 * a physical handler file.
 */
const EVE_SCHEDULE_TASK_VIRTUAL_ID_PREFIX = "#ovo-schedule-task/";
...
/**
 * Replaces the currently-registered ovo schedule task handlers when the
...
/**
 * Removes every ovo-owned schedule task entry, virtual handler module, and
