...
/**
 * THE LINK FLOW, shared by `ovo link` and the dev TUI `/model` menu's provider row (its
...
export async function runLinkFlow(input: {
  ...
  signal?: AbortSignal;
  /**
   * Whether the caller may only link an existing project (`ovo link`, the
  ...
  const boxes: AnySetupBox<SetupState>[] = [
    resolveProvisioning({
      ...
      // `ovo link` links an existing project (the default); the `/model`
  ...
  signal?.throwIfAborted();
  // Success stays silent here: the caller owns the closing line (`ovo link`'s
