...
// fallback's sources — ovo's package.json and the workspace catalog — are the
...
describe("resolveVersionToken", () => {
  ...
  it("resolves the ovo version token from ovo's own package.json", () => {
  ...
  it("resolves the node engine token from ovo's own package.json engines.node", () => {
