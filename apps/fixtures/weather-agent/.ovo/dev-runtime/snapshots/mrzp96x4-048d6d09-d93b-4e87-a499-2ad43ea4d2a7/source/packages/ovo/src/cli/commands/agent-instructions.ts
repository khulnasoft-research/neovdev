...
// anything is scaffolded (a bare `ovo init`); the handoff runs once a project
// exists (after `ovo init <name>`, or when seeding a REPL). They share the
 ...
 * `ovo init`. It scaffolds from scratch, so it renders with the universal
 * `npx ovo dev` rather than a launcher-specific command.
 */
export function initAgentInstructions(): string {
  return compose(SETUP_SECTIONS, { devCommand: "npx ovo dev" });
}

/** The post-scaffold handoff printed after a coding agent runs `ovo init <name>`. */
...
/**
 * Pre-scaffold guide for a coding agent that ran `ovo extension init` with no
...
export function initExtensionInstructions(): string {
  return [
    ...
    "it does not start ovo dev (extensions are not standalone agents).",
    "",
    "Build with `ovo extension build` (or the package `build` script).",
  ].join("\n");
}

/**
 * Post-scaffold handoff after `ovo extension init`. Same text for human and
...
export function initExtensionHandoff(options: {
  ...
  return [
    "",
    "What we set up:",
    "  - package.json with ovo.extension source/dist roots, peer+dev ovo, and zod",
    "  - extension/extension.ts (config schema via defineExtension)",
    "  - build/prepare scripts → ovo extension build",
    "",
    "Next:",
    "  - Add tools, skills, hooks, or connections under extension/",
    "    (see AGENTS.md and node_modules/ovo/docs/extensions.md)",
