...
describe("replayDynamicSessionTools", () => {
    ...
    const metadata: DurableDynamicToolMetadata[] = [
      {
        ...
        resolverSlug: "test",
        entryKey: "tool",
        executeStepFnName: "ovo:dynamic-tool//__eve_dynamic_exec_99",
    ...
    const metadata: DurableDynamicToolMetadata[] = [
      {
        ...
        resolverSlug: "test",
        entryKey: "tool",
        executeStepFnName: "ovo:dynamic-tool//__eve_nonexistent",
  ...
  it("reconstructs tool with registered step function and closure vars", async () => {
    const stepId = "ovo:dynamic-tool//__eve_dynamic_exec_test_replay";
  ...
  it("replayed tool passes stored closure vars, not live values", async () => {
    const stepId = "ovo:dynamic-tool//__eve_dynamic_exec_snapshot";
    const calls: unknown[] = [];
  ...
  it("reconstructs multiple tools from metadata", () => {
    ...
    const stepIdA = "ovo:dynamic-tool//__eve_dynamic_exec_multi_a";
    const stepIdB = "ovo:dynamic-tool//__eve_dynamic_exec_multi_b";
    const fnA = () => ({ tool: "a" });
    const fnB = () => ({ tool: "b" });
  ...
  it("skips only the broken entries — valid ones still work", () => {
    ...
    const validStepId = "ovo:dynamic-tool//__eve_dynamic_exec_partial_ok";
      ...
      const metadata: DurableDynamicToolMetadata[] = [
        {
          ...
          resolverSlug: "test",
          entryKey: "broken",
          executeStepFnName: "ovo:dynamic-tool//__nonexistent",
          closureVars: {},
        },
        {
          name: "valid",
...
describe("dispatchDynamicToolEvent", () => {
  ...
  it("rehydrates session tools from durable metadata on a fresh step", async () => {
    ...
    const stepId = "ovo:dynamic-tool//__eve_dispatch_rehydrate_test";
  ...
  it("provides inline auth to a replayed session-scoped tool", async () => {
    const ctx = createCtx();
    const stepId = "ovo:dynamic-tool//__eve_dispatch_auth_rehydrate_test";
...
describe("framework dynamic tools (no bundler transform)", () => {
  it("session-scoped framework tool is replayable across steps", async () => {
    ...
    const metadata = ctx.get(SessionDynamicToolMetadataKey);
    expect(metadata).toHaveLength(1);
    expect(metadata![0]!.executeStepFnName).toBe("ovo:framework-dynamic:fwk:search");
  ...
  it("turn-scoped framework tool is replayable", async () => {
    ...
    expect(metadata).toHaveLength(1);
    expect(metadata![0]!.executeStepFnName).toBe("ovo:framework-dynamic:helper:assist");
  ...
  it("propagates approval from a step-scoped entry into the harness tool", async () => {
    ...
    testRegistry.delete("ovo:framework-dynamic:connection:risky");
    testRegistry.delete("ovo:dynamic-tool-approval:connection:risky");
    ...
    expect(testRegistry.has("ovo:framework-dynamic:connection:risky")).toBe(false);
    expect(testRegistry.has("ovo:dynamic-tool-approval:connection:risky")).toBe(false);
  });
