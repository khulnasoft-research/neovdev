  ...
  return {
    ...
    accessor: {
      ...
      set: (key: { name: string }, value: unknown | ((current: unknown) => unknown)) => {
        ...
        if (key.name === "ovo.continuationToken") {
...
function buildSignedRequest(input: {
  ...
  const signature = `v0=${createHmac("sha256", secret)
    ...
    .digest("hex")}`;
  return new Request("https://example.com/ovo/v1/slack", {
    method: "POST",
    headers: {
...
describe("rebuildSlackContext", () => {
  ...
  it("auto-anchors state.threadTs and re-keys the session on the first post", async () => {
    ...
    const tokenWrites = writes.filter(([key]) => key === "ovo.continuationToken");
    expect(tokenWrites).toEqual([["ovo.continuationToken", "slack:C01:1800000000.123456"]]);
    ...
    const allTokenWrites = writes.filter(([key]) => key === "ovo.continuationToken");
...
describe("slackChannel() webhookVerifier credentials path", () => {
  ...
  it("uses webhookVerifier instead of HMAC when supplied", async () => {
    ...
    const { body } = buildMentionBody();
    const req = new Request("https://example.com/ovo/v1/slack", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body,
    });
