...
export interface ExtensionBuildConfig {
  /** Absolute authoring root from `ovo.extension.source`. */
  readonly sourceRoot: string;
  /** Absolute agent-shaped distribution root from `ovo.extension.dist`. */
...
export async function tryReadExtensionBuildConfig(
  ...
  const extension = parseExtensionPackageRoots(pkg.ovo?.extension);
  if (extension === null) {
    return null;
  }
  if (extension.source === undefined) {
    throw new Error(
      "`ovo.extension.dist` is declared without `ovo.extension.source`. Building an extension requires the authoring root; add `ovo.extension.source` to package.json.",
    );
  }
  ...
  assertManagedPackagePath(appRoot, sourceRoot, "ovo.extension.source");
  assertManagedPackagePath(appRoot, distRoot, "ovo.extension.dist");
  assertManagedPackagePath(appRoot, outDir, "ovo.extension.dist output directory");
  if (sourceRoot === distRoot) {
    throw new Error("`ovo.extension.source` and `ovo.extension.dist` must be different paths.");
  }
  if (
    sourceRoot === outDir ||
    ...
    throw new Error("`ovo.extension.source` and the managed dist output cannot overlap.");
  }
