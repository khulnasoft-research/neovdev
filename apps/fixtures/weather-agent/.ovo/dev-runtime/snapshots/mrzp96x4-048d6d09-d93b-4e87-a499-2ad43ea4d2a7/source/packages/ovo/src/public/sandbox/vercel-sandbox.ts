 ...
 * for fresh templates and template-less sessions, so ovo can install
 ...
 *
 * `runtime` is excluded as well: ovo always boots its sandboxes from the
 * published ovo image, which is mutually exclusive with a stock runtime.
 *
 ...
 * so `source` is stripped from the session-create path. ovo does not
