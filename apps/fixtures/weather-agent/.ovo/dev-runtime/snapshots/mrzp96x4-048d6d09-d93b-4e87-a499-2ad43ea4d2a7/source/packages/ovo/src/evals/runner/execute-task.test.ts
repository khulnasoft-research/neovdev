...
const target = createEvalTargetHandle({
  ...
  client: new Client({ host: "https://ovo.test" }),
  kind: "local",
  url: "https://ovo.test",
});
...
describe("executeTask", () => {
  ...
  it("runs a scripted eval with HITL helpers", async () => {
    ...
    expect(server.posts.map((post) => post.body)).toEqual([
      { message: "run pwd" },
      {
        continuationToken: "ovo:session_1",
  ...
  it("coordinates started and externally watched live turns through one stream owner", async () => {
      ...
      [
        {
          ...
          events: [
            ...
            sessionWaiting("ovo:parent-session"),
          ],
        },
        {
          ...
          events: [
            ...
            sessionWaiting("ovo:child-session-next"),
          ],
        },
      ],
      {
        ...
        streams: [
          {
            ...
            events: [
              ...
              sessionWaiting("ovo:child-session"),
            ],
          },
        ],
      },
    ...
    expect(server.posts[1]?.body).toEqual({
      continuationToken: "ovo:child-session",
  ...
  it("sends a follow-up on an attached session via the stream-recovered continuation token", async () => {
      ...
      [
        {
          ...
          events: [
            ...
            turnCompleted("turn_2"),
            sessionWaiting("ovo:channel-rekeyed"),
          ],
        },
      ],
      {
    ...
    expect(new URL(server.posts[0]!.url).pathname).toBe("/ovo/v1/session/channel-session");
    expect(server.posts[0]?.body).toEqual({
      continuationToken: "ovo:channel-rekeyed",
  ...
  return {
    ...
    async fetch(request: string | URL | Request, init?: RequestInit): Promise<Response> {
      ...
      if (method === "POST") {
        ...
        return Response.json(
          {
            continuationToken: `ovo:${next.sessionId}`,
            ok: true,
...
function sessionWaiting(continuationToken = "ovo:session_1"): HandleMessageStreamEvent {
  return {
