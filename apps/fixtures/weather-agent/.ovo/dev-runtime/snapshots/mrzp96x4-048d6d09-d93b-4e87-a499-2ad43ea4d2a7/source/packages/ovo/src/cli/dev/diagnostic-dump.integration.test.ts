...
const ENVIRONMENT: DevEnvironmentInfo = {
  ...
  sessionsDirectory: { path: ".ovo/.workflow-data", files: 12, bytes: 2048 },
};
...
describe("createDevDiagnosticDump", () => {
  ...
  async function makeRoot(): Promise<string> {
    const root = await mkdtemp(join(tmpdir(), "ovo-dump-"));
    roots.push(root);
    await mkdir(join(root, ".ovo", "logs"), { recursive: true });
    return root;
  }

  it("derives the dump path from the log instance and writes environment + stats", async () => {
    const root = await makeRoot();
    const logPath = join(root, ".ovo", "logs", "dev-2026-07-17T12-00-00.000Z-123.log");
    ...
    expect(dump.path).toBe(join(root, ".ovo", "logs", "dev-2026-07-17T12-00-00.000Z-123.dump"));
    expect(dump.displayPath).toBe(".ovo/logs/dev-2026-07-17T12-00-00.000Z-123.dump");
  ...
  it("keeps a pending marker when environment collection fails", async () => {
    const root = await makeRoot();
    const logPath = join(root, ".ovo", "logs", "dev-2026-07-17T12-00-00.000Z-9.log");
...
describe("measureSessionsDirectory", () => {
  ...
  it("measures the local sessions directory when present", async () => {
    const root = await mkdtemp(join(tmpdir(), "ovo-dump-env-"));
    roots.push(root);
    const dataDir = join(root, ".ovo", ".workflow-data", "runs");
