...
interface DevelopmentRuntimeArtifactsPointerV1 {
  ...
  readonly kind: "ovo-dev-runtime-artifacts-pointer";
  readonly version: 1;
}
...
export function resolveDevelopmentRuntimeArtifactsPointerPath(appRoot: string): string {
  return join(appRoot, ".ovo", DEV_RUNTIME_ARTIFACTS_DIRECTORY, "current.json");
}

function resolveDevelopmentRuntimeArtifactsSnapshotsDirectory(appRoot: string): string {
  return join(appRoot, ".ovo", DEV_RUNTIME_ARTIFACTS_DIRECTORY, "snapshots");
}
...
export async function stageDevelopmentRuntimeArtifactsSnapshot(
  ...
  try {
    ...
    await cp(
      ...
      join(sourceSnapshotPlan.runtimeAppRoot, ".ovo", "compile"),
      {
        recursive: true,
      },
    );
  ...
  try {
    ...
    if (
      ...
      await recordRetiredDevelopmentRuntimeArtifactsSnapshot(previousPointer.snapshotRoot).catch(
        (error) => {
          console.warn(
            `[ovo:dev] failed to record retired runtime generation "${previousPointer.snapshotRoot}": ${String(error)}`,
          );
        },
      );
    }
...
function readDevelopmentRuntimeArtifactsPointer(
  ...
  try {
    ...
    if (
      pointer.kind !== "ovo-dev-runtime-artifacts-pointer" ||
