...
async function writeOptionalApplicationBuildProfile(input: {
  ...
  try {
    ...
    console.warn(
      `ovo: failed to write optional build profile to ${input.profileOutputPath}; continuing with the published build output: ${toErrorMessage(error)}`,
    );
  }
}
...
/**
 * Resolve the route prefix an ovo service is mounted under when it is
 * co-deployed behind a host web service (Next.js, Nuxt, SvelteKit etc.).
 *
 * Any service whose framework is not `ovo` is treated as a host that proxies
 * ovo's transport routes behind a prefix. A standalone ovo deployment (no host
...
/**
 * Builds the production Nitro output for an ovo application.
 */
export async function buildApplication(
  ...
  // Extension packages use `ovo extension build`. Keep agent `ovo build` agent-only
  ...
  if (extensionBuild !== null) {
    throw new Error(
      `Package "${extensionBuild.packageName}" is an ovo extension. Run \`ovo extension build\` instead of \`ovo build\`.`,
    );
  }
