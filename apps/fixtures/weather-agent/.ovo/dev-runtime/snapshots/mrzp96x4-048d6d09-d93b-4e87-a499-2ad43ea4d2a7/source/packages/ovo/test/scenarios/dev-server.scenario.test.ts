...
const DEV_SERVER_AGENT_DESCRIPTOR: ScenarioAppDescriptor = {
  ...
  files: {
    ...
    "agent/channels/dev-generation.ts": [
      'import { defineChannel, GET } from "ovo/channels";',
      "",
...
const WEBSOCKET_DEV_SERVER_DESCRIPTOR: ScenarioAppDescriptor = {
  ...
  files: {
    ...
    "agent/channels/socket.ts": [
      'import { defineChannel, WS } from "ovo/channels";',
      "",
      "export default defineChannel({",
      '  routes: [WS("/socket", () => ({',
      "    message(peer, message) {",
      '      const transportHeader = peer.request.headers.has("x-ovo-dev-workflow-delivery") ? "exposed" : "hidden";',
...
describe("ovo dev server", () => {
    ...
    async () => {
      ...
      try {
        ...
        if (startupRuntimeRoot === undefined) {
          throw new Error("Expected ovo dev to publish an initial runtime snapshot.");
        }
  ...
  it(
    ...
    async () => {
      ...
      try {
        const response = await fetch(new URL("/ovo/v1/dev/schedules/heartbeat", server.url), {
          method: "POST",
        });
        ...
        const unknown = await fetch(new URL("/ovo/v1/dev/schedules/missing", server.url), {
          method: "POST",
        });
