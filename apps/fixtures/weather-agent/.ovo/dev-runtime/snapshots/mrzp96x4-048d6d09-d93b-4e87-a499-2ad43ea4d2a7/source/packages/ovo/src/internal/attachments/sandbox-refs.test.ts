...
describe("encodeSandboxRef", () => {
  it("builds an ovo-sandbox: URL carrying path, size, and media type as query params", () => {
...
describe("decodeSandboxRef", () => {
  ...
  it("throws when the URL scheme is not ovo-sandbox:", () => {
    const wrongSchemeUrl = new URL("https://example.com/?path=/a&size=1&type=text/plain");
    expect(() => decodeSandboxRef(wrongSchemeUrl)).toThrow(/must use scheme "ovo-sandbox:"/);
  });
...
describe("isSandboxRefUrl", () => {
  it("returns true for a URL with the ovo-sandbox: scheme", () => {
    const url = encodeSandboxRef({
      mediaType: "text/plain",
      path: "/a",
      size: 1,
  ...
  it("returns false for a URL with a different scheme", () => {
    expect(isSandboxRefUrl(new URL("ovo-attachment:?v=1&p=e30"))).toBe(false);
  ...
  it("returns false for strings (even valid ovo-sandbox: strings)", () => {
    ...
    // before dispatching.
    expect(isSandboxRefUrl("ovo-sandbox:?path=/a&size=1&type=text/plain")).toBe(false);
  });
