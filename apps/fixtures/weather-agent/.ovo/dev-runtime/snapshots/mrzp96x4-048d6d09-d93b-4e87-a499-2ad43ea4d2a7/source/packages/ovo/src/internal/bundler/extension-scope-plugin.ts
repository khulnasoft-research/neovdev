...
function shimSource(kind: "context" | "extension", namespace: string): string {
  ...
  return [
    `import { defineExtension as __eveScopedDefineExtension } from "ovo/extension";`,
 ...
 * `ovo/context`/`ovo/extension` imports redirected to a generated shim that
...
export function createExtensionScopePlugin(
  ...
  return scopeHooks("ovo-extension-scope", (importer) => {
...
export function createFixedNamespaceScopePlugin(namespace: string): ExtensionScopeBundlerPlugin {
  return scopeHooks("ovo-extension-scope-fixed", () => namespace);
}
