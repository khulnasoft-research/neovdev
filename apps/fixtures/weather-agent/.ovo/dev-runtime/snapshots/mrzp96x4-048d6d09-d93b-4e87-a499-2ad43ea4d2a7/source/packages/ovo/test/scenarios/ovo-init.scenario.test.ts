...
const EVE_BIN_PATH = fileURLToPath(new URL("../../bin/ovo.js", import.meta.url));
 ...
 * launched by a coding agent whose markers would flip `ovo init` onto the
...
async function createFakePnpmEnvironment(scratch: string): Promise<{
  ...
  const env = {
    ...
    GIT_AUTHOR_EMAIL: "ovo-init@example.com",
    GIT_AUTHOR_NAME: "ovo Init",
    GIT_COMMITTER_EMAIL: "ovo-init@example.com",
    GIT_COMMITTER_NAME: "ovo Init",
...
describe("ovo init smoke", () => {
  it("creates the base template with the default model and no Vercel state", async () => {
    ...
    expect(await fakePnpm.readCalls()).toEqual([
      ...
      {
        args: ["--dir", canonicalProjectDir, "exec", "ovo", "dev", "--input", "/model"],
        cwd: canonicalProjectDir,
      },
    ]);
    expect(result.stdout).toContain("Created an ovo agent in ");
    ...
    ).resolves.toMatchObject({ stdout: "Initial commit from ovo\n" });
    await expect(
  ...
  it("adds Web Chat without Vercel configuration", async () => {
    const scratch = await createScratchDirectory("ovo-init-web-");
    const fakePnpm = await createFakePnpmEnvironment(scratch);
    ...
    expect(devCall?.args.slice(-5)).toEqual(["exec", "ovo", "dev", "--input", "/model"]);
  });

  it("adds Web Chat through npm without writing pnpm configuration", async () => {
    const scratch = await createScratchDirectory("ovo-init-web-npm-");
  ...
  it("adds an agent to an existing pnpm project targeted as a directory", async () => {
    const scratch = await createScratchDirectory("ovo-init-dir-");
    const fakePnpm = await createFakePnpmEnvironment(scratch);
    await writeFile(
    ...
    expect(result.exitCode, result.stderr).toBe(0);
    expect(result.stdout).toContain("Added an ovo agent to ");
  ...
  it("hands a coding agent the setup guide when it omits the target", async () => {
    const scratch = await createScratchDirectory("ovo-init-agent-bare-");
    const fakePnpmRoot = await createScratchDirectory("ovo-init-agent-bare-pnpm-");
    ...
    expect(result.exitCode, result.stderr).toBe(0);
    // A bare `ovo init` from an agent prints the setup guide and scaffolds
    // nothing — no agent files written. (No install runs, so the fake pnpm is
    // never invoked and writes no call log.)
    expect(result.stdout).toContain("Set up an ovo agent");
    expect(result.stdout).toContain("npx ovo@latest init <name>");
  ...
  it("warns and continues when a coding agent passes the compatibility yes flag", async () => {
    const scratch = await createScratchDirectory("ovo-init-agent-fumble-");
    const fakePnpmRoot = await createScratchDirectory("ovo-init-agent-fumble-pnpm-");
    const fakePnpm = await createFakePnpmEnvironment(fakePnpmRoot);
    ...
    expect(result.exitCode, result.stderr).toBe(0);
    expect(result.stderr).toContain("warning: --yes has no effect for ovo init.");
    expect(result.stdout).toContain("ovo dev --no-ui");
  ...
  it("scaffolds the current empty directory when the target is omitted", async () => {
    const scratch = await createScratchDirectory("ovo-init-human-bare-");
    const fakePnpmRoot = await createScratchDirectory("ovo-init-human-bare-pnpm-");
    const fakePnpm = await createFakePnpmEnvironment(fakePnpmRoot);
  ...
  it("scaffolds for a coding agent but prints the dev command instead of starting the TUI", async () => {
    const scratch = await createScratchDirectory("ovo-init-agent-named-");
    const fakePnpm = await createFakePnpmEnvironment(scratch);
  ...
  it("rejects path-like names without writing outside the current directory", async () => {
    const scratch = await createScratchDirectory("ovo-init-invalid-name-");
