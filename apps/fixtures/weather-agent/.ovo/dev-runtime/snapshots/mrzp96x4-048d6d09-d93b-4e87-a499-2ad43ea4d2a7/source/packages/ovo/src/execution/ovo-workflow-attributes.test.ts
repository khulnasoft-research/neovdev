...
const slackChannelCtx = {
  "ovo.channel": { kind: "slack", state: { team: "T1" } },
} satisfies Record<string, unknown>;

const subagentChainCtx = {
  "ovo.channel": { kind: "slack", state: {} },
  "ovo.parentSession": {
...
describe("readChannelKind", () => {
  ...
  it("returns undefined when the slot is missing or malformed", () => {
    ...
    expect(readChannelKind({ "ovo.channel": { kind: "" } })).toBeUndefined();
    expect(readChannelKind({ "ovo.channel": { kind: 42 } })).toBeUndefined();
  });
});
...
describe("buildSessionAttributes", () => {
  it("emits type=session with trigger and derived title", () => {
    ...
    expect(attrs).toEqual({
      "$ovo.channel_request_id": undefined,
      "$ovo.type": "session",
      "$ovo.trigger": "slack",
      "$ovo.title": "ship the thing please",
    });
  });
  ...
  it("omits the trigger when no channel is on the context", () => {
    ...
    expect(attrs["$ovo.trigger"]).toBeUndefined();
    expect(attrs["$ovo.title"]).toBe("hi");
  });
  ...
  it("emits the channel request id when present", () => {
    ...
    expect(attrs["$ovo.channel_request_id"]).toBe("req_session");
  });
});
...
describe("buildSubagentRootAttributes", () => {
  it("emits type=subagent with parent, root session, subagent node, and trigger", () => {
    ...
    expect(attrs).toEqual({
      "$ovo.channel_request_id": undefined,
      "$ovo.type": "subagent",
      "$ovo.parent": "wrun_parent_subagent",
      "$ovo.parent_call": "call_subagent_0",
      "$ovo.parent_turn": "turn_0",
      "$ovo.root": "wrun_top_level_session",
      "$ovo.subagent": "subagents/linear",
      "$ovo.trigger": "slack",
    });
  });
  ...
  it("emits the channel request id when present", () => {
    ...
    expect(attrs["$ovo.channel_request_id"]).toBe("req_subagent");
  });
});
...
describe("buildTurnAttributes", () => {
  it("emits type=turn with parent and root session", () => {
    ...
    expect(attrs).toEqual({
      "$ovo.channel_request_id": undefined,
      "$ovo.type": "turn",
      "$ovo.parent": "wrun_session_123",
      "$ovo.root": "wrun_session_123",
    });
  });
  ...
  it("emits the channel request id when present", () => {
    ...
    expect(attrs["$ovo.channel_request_id"]).toBe("req_turn");
  });
});
