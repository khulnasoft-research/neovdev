 ...
 * when the matching ovo integration import is present; otherwise missing,
 * unsupported, or rejected framework detections are switched back to ovo.
  ...
  const linked = await withSpinner(
    ...
    () =>
      runVercel(["link", "--project", project.projectId, ...scope, "--yes"], {
        ...
        // input. ovo strips the coding-agent env markers (so the CLI does not
