...
const APP_ROOT = "/tmp/ovo-test";
const ARTIFACTS_CONFIG = {
  appRoot: APP_ROOT,
  devRuntimeArtifactsPointerPath: "/tmp/ovo-test/.ovo/dev-runtime/latest.json",
  kind: "development",
  moduleMapLoaderPath: "/tmp/ovo-test/module-map-loader.js",
} as const;
...
async function postSchedule(scheduleIdInUrl: string): Promise<Response> {
  ...
  const request = new Request(`http://localhost:3000/ovo/v1/dev/schedules/${scheduleIdInUrl}`, {
    method: "POST",
  });
