...
export const CHANNEL_SENTINEL = "ovo:channel" as const;
const CHANNEL_INSTRUMENTATION_KIND = Symbol.for("ovo.channel.instrumentationKind");
const CHANNEL_INSTRUMENTATION_KINDS = Symbol.for("ovo.channel.instrumentationKinds");
...
export function isCompiledChannel(value: unknown): value is CompiledChannel {
  return (
    typeof value === "object" &&
    value !== null &&
    (value as { __kind?: unknown }).__kind === "ovo:channel"
  );
}
