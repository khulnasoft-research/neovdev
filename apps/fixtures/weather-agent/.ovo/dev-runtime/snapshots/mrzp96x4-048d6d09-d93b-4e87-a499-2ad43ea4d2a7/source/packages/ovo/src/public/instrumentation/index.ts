...
// Re-export channel metadata types so existing `ovo/instrumentation`
// imports continue to work. The canonical home is `ovo/channels`.
export {
  isChannel,
 ...
 *
 * Keys beginning with `ovo.` are reserved for framework-owned context
...
/**
 * Input passed to `events["step.started"]`. ovo runs the callback after
...
/**
 * Result of a `step.started` callback. ovo merges `runtimeContext` into the
 * AI SDK telemetry span; child spans inherit the values. Keys beginning with
 * `ovo.` and non-JSON-serializable values are dropped. Return `undefined` to
...
/**
 * Export the result as the default export of `agent/instrumentation.ts`. ovo
