...
describe("resolveTarget box", () => {
  ...
  it("resumes an existing ovo project when the composition opted in (headless re-runs)", async () => {
    mockedPathExists.mockImplementation(async (path) => path === "/tmp/parent/demo-agent");
  ...
  it("still refuses an existing directory that is not an ovo project, even when resumable", async () => {
    mockedPathExists.mockImplementation(async (path) => path === "/tmp/parent/demo-agent");
    mockedIsEveProject.mockResolvedValue(false);
    const box = resolveTarget({
      asker: headlessAsker(),
