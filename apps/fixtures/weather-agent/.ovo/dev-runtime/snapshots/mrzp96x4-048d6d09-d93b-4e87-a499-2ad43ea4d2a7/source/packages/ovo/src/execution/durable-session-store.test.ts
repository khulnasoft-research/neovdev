...
describe("durable-session-store cross-version contract", () => {
  ...
  it("preserves unrecognized DurableSessionState fields via spread (forward compat)", () => {
    // Hypothetical state shape introduced by a newer ovo version.
  ...
  it("NextDriverAction `kind` is the closed driver dispatch surface", () => {
    ...
    const ctx: Record<string, unknown> = { "ovo.sessionId": "wrun_action" };
  ...
  it("cancels the legacy tail read after loading one durable session snapshot", async () => {
    ...
    expect(getReadable).toHaveBeenCalledWith({
      namespace: "ovo.session",
      startIndex: -1,
    });
    expect(cancel).toHaveBeenCalledWith("ovo durable session tail read complete");
  ...
  it("throws a named timeout and cancels the legacy tail read when the stream hangs", async () => {
    ...
    const assertion = expect(promise).rejects.toMatchObject({
      message: expect.stringContaining(
        'Timed out reading durable session snapshot from stream "ovo.session" for run wrun_tail_timeout after 10000ms.',
      ),
    ...
    expect(cancel).toHaveBeenCalledWith("ovo durable session tail read timed out after 10000ms");
    expect(stream.locked).toBe(false);
  });
});
