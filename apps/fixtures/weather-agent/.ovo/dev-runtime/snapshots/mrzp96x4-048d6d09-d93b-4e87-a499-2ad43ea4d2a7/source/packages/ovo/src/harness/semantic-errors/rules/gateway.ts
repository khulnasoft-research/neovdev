...
export const GATEWAY_RULES: readonly SemanticErrorRule[] = [
  {
    ...
    hint: "Update or unset `AI_GATEWAY_API_KEY` (check your shell profile if you did not set it for this project) — manage keys at https://vercel.com/dashboard/ai/api-keys. Unsetting it falls back to OIDC via `ovo link`.",
  },
  {
    ...
    hint: "Run `ovo link` to refresh `VERCEL_OIDC_TOKEN` in `.env.local`, or set `AI_GATEWAY_API_KEY` — create a key at https://vercel.com/dashboard/ai/api-keys.",
  },
  {
    ...
    hint: "Run `ovo link` to populate `VERCEL_OIDC_TOKEN`, or set `AI_GATEWAY_API_KEY` — create a key at https://vercel.com/dashboard/ai/api-keys.",
  },
  {
    id: "model-not-found",
    ...
    hint: "Check the model id in `agent.ts` (`model:`) or switch models with `/model` in `ovo dev` — browse available models at https://vercel.com/ai-gateway/models.",
  },
  {
  ...
  },
  {
    ...
    hint: "This is transient — retry shortly, or switch models with `/model` in `ovo dev`.",
  },
];
