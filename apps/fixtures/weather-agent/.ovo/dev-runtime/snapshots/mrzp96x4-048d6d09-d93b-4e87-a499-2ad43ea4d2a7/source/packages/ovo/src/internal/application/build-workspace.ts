...
/**
 * Creates the invocation-owned directory tree under `.ovo/builds/<id>` that
...
export async function createApplicationBuildWorkspace(
  ...
  const rootDir = join(resolvedAppRoot, ".ovo", "builds", buildId);
  const compilerRootDir = join(rootDir, "compiler");
  const workspace: ApplicationBuildWorkspace = {
    appRoot: resolvedAppRoot,
    compiler: {
      artifactsDir: join(compilerRootDir, ".ovo"),
