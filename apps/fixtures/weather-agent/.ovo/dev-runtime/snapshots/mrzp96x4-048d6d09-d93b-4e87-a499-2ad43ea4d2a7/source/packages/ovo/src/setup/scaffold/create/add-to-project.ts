...
/**
 * Adds an ovo agent to an existing package: writes the `agent/` files, adds
 * missing runtime dependencies, reconciles `engines.node` with ovo's
...
export async function addAgentToProject(
  ...
  if (!(await pathExists(packageJsonPath))) {
    throw new Error(
      `Cannot add an ovo agent to "${options.projectRoot}" because it has no package.json. ` +
        "Run `ovo init <name>` to create a new project instead.",
    );
  }
  ...
  if (conflicts.length > 0) {
    throw new Error(
      `Cannot add an ovo agent to "${options.projectRoot}" because it already has: ` +
        `${conflicts.join(", ")}.`,
    );
  }
  ...
  const wanted: Record<string, string> = {
    ...
    ai: aiVersion,
    ovo: formatEveDependencySpecifier(evePackage.version),
    zod: zodVersion,
  };
