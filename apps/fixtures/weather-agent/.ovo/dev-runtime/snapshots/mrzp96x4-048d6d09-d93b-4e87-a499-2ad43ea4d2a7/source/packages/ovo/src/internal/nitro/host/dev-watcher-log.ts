...
export const AUTHORED_ARTIFACTS_UPDATED_LOG_LINE = "[ovo:dev] authored artifacts updated.";

export const STRUCTURAL_RELOAD_LOG_LINE =
  "[ovo:dev] structural change detected, reloading Nitro worker...";

/**
 * Formats the watcher's "change detected" line, e.g.
 * `[ovo:dev] change detected (1 event: change agent/agent.ts), rebuilding
...
export function formatChangeDetectedLogLine(
  ...
  return `[ovo:dev] change detected (${formatChangeEventList(appRoot, events)}), rebuilding authored artifacts...`;
}

const CHANGE_DETECTED_PATTERN =
  /^\[ovo:dev\] change detected \(\d+ events?: (.+)\), rebuilding authored artifacts\.\.\.$/u;
const MORE_EVENTS_PATTERN = /^\+(\d+) more$/u;
const REBUILD_FAILED_PATTERN = /^\[ovo:dev\] rebuild (?:failed|queue error): (.+)$/u;
