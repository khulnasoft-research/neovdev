...
describe("resolveVercelInvocation", () => {
  test("uses shell fallback for PATH Vercel CLI shims on Windows", () => {
    const invocation = resolveVercelInvocation("/tmp/ovo-agent", ["whoami"], "win32");
  ...
  test("keeps Unix invocation shell-free", () => {
    const invocation = resolveVercelInvocation("/tmp/ovo-agent", ["whoami"], "linux");
...
describe("captureVercel", () => {
  test("resolves with stdout on a clean exit", async () => {
    ...
    const result = captureVercel(["whoami"], { cwd: "/tmp/ovo-agent" });
  ...
  test("spawns from the nearest existing ancestor when cwd does not exist yet", async () => {
    ...
    const result = captureVercel(["whoami"], { cwd: "/tmp/ovo-smoke-missing-xyz/my-agent" });
    child.stdout.emit("data", Buffer.from("rconti\n"));
    child.emit("close", 0);
    ...
    expect(spawnedCwd).not.toBe("/tmp/ovo-smoke-missing-xyz/my-agent");
