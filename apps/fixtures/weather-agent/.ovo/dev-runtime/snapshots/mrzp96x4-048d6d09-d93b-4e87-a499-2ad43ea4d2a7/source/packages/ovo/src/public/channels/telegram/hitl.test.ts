...
describe("renderTelegramInputRequest", () => {
  it("renders option requests as compact inline-keyboard callbacks", () => {
    ...
    expect(rendered.replyMarkup).toEqual({
      inline_keyboard: [
        [
          { callback_data: "ovo:0", text: "Approve" },
          { callback_data: "ovo:1", text: "Deny" },
        ],
      ],
    });
    expect(Object.keys(state.hitlCallbacks ?? {})).toEqual(["ovo:0", "ovo:1"]);
    expect("ovo:0".length).toBeLessThanOrEqual(64);
  });
...
describe("resolveTelegramInputResponses", () => {
  ...
  it("skips stale synthetic responses and preserves normal responses", () => {
    const staleCallback = {
      optionId: "selected",
      requestId: `${TELEGRAM_CALLBACK_RESPONSE_PREFIX}ovo:old`,
    };
    const staleReply = {
