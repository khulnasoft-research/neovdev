...
export function resolveOutputPublicationLockPath(appRoot: string): string {
  return join(resolve(appRoot), ".ovo", "locks", "output-publication.lock");
}
...
function hasTokenDerivedBackupPaths(journal: OutputPublicationJournal): boolean {
  return (
    journal.outputBackupPath === `${journal.finalOutputDir}.ovo-backup-${journal.token}` &&
    journal.summaryBackupPath === `${journal.finalSummaryPath}.ovo-backup-${journal.token}`
  );
}

// The scratch directory is the failed build's preserved workspace; removing
// it after recovery is what keeps `.ovo/builds` from accumulating orphans.
