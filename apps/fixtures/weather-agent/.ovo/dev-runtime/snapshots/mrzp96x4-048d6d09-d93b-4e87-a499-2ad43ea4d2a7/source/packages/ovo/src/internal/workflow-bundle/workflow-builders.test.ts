...
describe("applyWorkflowTransform", () => {
  it("keeps ovo workflow references stable when ovo is the project root", async () => {
    ...
    expect(transformed.workflowManifest.workflows?.[filename]?.turnWorkflow).toEqual({
      workflowId: "workflow//ovo//turnWorkflow",
    });
  });
  ...
  it("registers workflow functions in workflow mode", async () => {
    ...
    expect(transformed.workflowManifest).toEqual({
      workflows: {
        "src/execution/workflow-entry.ts": {
          workflowEntry: {
            workflowId: "workflow//ovo//workflowEntry",
          },
        },
      },
    });
    expect(transformed.code).toContain(
      'workflowEntry.workflowId = "workflow//ovo//workflowEntry";',
    );
    expect(transformed.code).toContain(
      'globalThis.__private_workflows.set("workflow//ovo//workflowEntry", workflowEntry);',
    );
  });
  ...
  it("strips the @<version> stamp for stable workflow names but not for steps", async () => {
    ...
    expect(transformed.workflowManifest).toEqual({
      steps: {
        "src/execution/turn-workflow.ts": {
          notifyDriverStep: {
            stepId: "step//ovo@1.2.3//notifyDriverStep",
          },
        },
      },
      workflows: {
    ...
    expect(transformed.code).toContain('turnWorkflow.workflowId = "workflow//ovo//turnWorkflow";');
    expect(transformed.code).toContain(
      'globalThis.__private_workflows.set("workflow//ovo//turnWorkflow", turnWorkflow);',
    );
  });
});
