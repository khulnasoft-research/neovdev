...
describe("createExtensionScopePlugin (path containment)", () => {
  ...
  it("redirects ovo/context to a namespaced shim for extension-owned importers", () => {
    const id = pathPlugin().resolveId("ovo/context", "/pkg/crm/extension/tools/budget.ts");
    expect(id).toBe("\0eve-ext-scope:context:acme-crm");
  });

  it("redirects ovo/extension to a namespaced shim for extension-owned importers", () => {
    const id = pathPlugin().resolveId("ovo/extension", "/pkg/crm/extension/config.ts");
    expect(id).toBe("\0eve-ext-scope:extension:acme-crm");
  });

  it("ignores importers outside every extension source root", () => {
    expect(pathPlugin().resolveId("ovo/context", "/app/agent/tools/local.ts")).toBeUndefined();
  });

  it("does not redirect a sibling directory that shares the source-root prefix", () => {
    expect(pathPlugin().resolveId("ovo/context", "/pkg/crm/extras/tool.ts")).toBeUndefined();
  });

  it("only intercepts the scoped framework modules", () => {
    expect(
      pathPlugin().resolveId("ovo/tools", "/pkg/crm/extension/tools/budget.ts"),
    ).toBeUndefined();
...
describe("createFixedNamespaceScopePlugin (dev per-module)", () => {
  it("scopes every non-virtual importer to the fixed namespace", () => {
    ...
    expect(plugin.resolveId("ovo/context", "/anywhere/on/disk/tool.ts")).toBe(
      "\0eve-ext-scope:context:acme-crm",
    );
    expect(plugin.resolveId("ovo/extension", "/anywhere/config.ts")).toBe(
  ...
  it("never re-enters through virtual shim importers", () => {
    const plugin = createFixedNamespaceScopePlugin("acme-crm");
    expect(plugin.resolveId("ovo/context", "\0eve-ext-scope:context:acme-crm")).toBeUndefined();
  });

  it("only intercepts the scoped framework modules", () => {
    const plugin = createFixedNamespaceScopePlugin("acme-crm");
    expect(plugin.resolveId("ovo/tools", "/anywhere/tool.ts")).toBeUndefined();
  });
});
...
describe("shim baking (shared)", () => {
  it("bakes the namespace into the defineState shim", () => {
    ...
    expect(shim?.code).toContain(
      `import { defineState as __eveScopedDefineState } from "ovo/context"`,
    );
  ...
  it("bakes the namespace into the defineExtension shim", () => {
    ...
    expect(shim?.code).toContain(`from "ovo/extension"`);
