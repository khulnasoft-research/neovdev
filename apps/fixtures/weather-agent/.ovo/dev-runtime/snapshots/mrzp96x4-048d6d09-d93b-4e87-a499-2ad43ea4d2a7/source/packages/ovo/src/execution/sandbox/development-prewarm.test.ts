...
describe("development sandbox prewarm coordination", () => {
  ...
  it("waits for an in-flight prewarm by authored app root or compiled snapshot root", async () => {
    ...
    const appRoot = "/tmp/ovo-app";
    const snapshotRoot = "/tmp/ovo-app/.ovo/dev-runtime/snapshots/current";
  ...
  it("waits for an in-flight prewarm by the stable sandbox app root", async () => {
    ...
    mocks.prewarmAppSandboxes.mockReturnValueOnce(prewarm.promise);
    const appRoot = "/tmp/ovo-app";
    const snapshotRoot = "/tmp/ovo-app/.ovo/dev-runtime/snapshots/current/app";
    const compiledArtifactsSource = createDiskRuntimeCompiledArtifactsSource(snapshotRoot, {
  ...
  it("replays and forwards prewarm progress logs to waiters", async () => {
    ...
    prewarmLog?.('ovo: sandbox template "root" (microsandbox): preparing base runtime inside VM');
    ...
    await Promise.resolve();
    prewarmLog?.('ovo: sandbox template "root" (microsandbox): apt-get update');
    ...
    expect(logs).toEqual(
      expect.arrayContaining([
        'ovo: sandbox template "root" (microsandbox): preparing base runtime inside VM',
        'ovo: sandbox template "root" (microsandbox): apt-get update',
      ]),
    );
  });
  ...
  it("replays retained logs and forwards future logs to subscribers", async () => {
    ...
    prewarmLog?.('ovo: built sandbox template "root" on backend "docker".');
    ...
    prewarmLog?.('ovo: sandbox template "root" (docker): apt-get update');

    unsubscribe();
    prewarmLog?.('ovo: sandbox template "root" (docker): apt-get install curl');
    prewarm.resolve();
    await prewarm.promise;

    expect(logs).toEqual([
      'ovo: built sandbox template "root" on backend "docker".',
      'ovo: sandbox template "root" (docker): apt-get update',
    ]);
  });

  it("keeps completed background prewarm logs for one TUI subscription", async () => {
    const appRoot = "/tmp/ovo-completed-app";
    const compiledArtifactsSource = createDiskRuntimeCompiledArtifactsSource(appRoot);
    mocks.prewarmAppSandboxes.mockImplementationOnce(async (input) => {
      input.log?.('ovo: built sandbox template "root" on backend "docker".');
    });
    ...
    expect(firstLogs).toEqual(['ovo: built sandbox template "root" on backend "docker".']);
    expect(secondLogs).toEqual([]);
  });

  it("skips completed prewarm work when the sandbox signature is unchanged", async () => {
    const appRoot = "/tmp/ovo-signature-app";
    const compiledArtifactsSource = createDiskRuntimeCompiledArtifactsSource(appRoot);
    let prewarmCount = 0;
