...
describe("withPackageUserAgent", () => {
  it("appends the ovo token to an existing user-agent", async () => {
    ...
    expect(userAgentOf(init)).toMatch(/^vercel\/sandbox\/2\.2\.0 ovo\/.+/);
  });

  it("sets the ovo token as the user-agent when none is present", async () => {
    const inner = vi.fn<typeof globalThis.fetch>().mockResolvedValue(new Response());
    const wrapped = withPackageUserAgent(inner);
    ...
    const [, init] = inner.mock.calls[0]!;
    expect(userAgentOf(init)).toMatch(/^ovo\/.+/);
  });
