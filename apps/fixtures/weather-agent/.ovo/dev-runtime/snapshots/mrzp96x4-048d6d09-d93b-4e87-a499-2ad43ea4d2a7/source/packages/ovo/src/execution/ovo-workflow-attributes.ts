/**
 * Builders for the framework's reserved `$ovo.*` workflow attributes.
 *
 ...
 *
 * `$ovo.*` is the framework-owned attribute namespace. Authored code
 ...
 *
 * Tag inventory (recap):
 * - `$ovo.type`         — `"session" | "turn" | "subagent"`
 * - `$ovo.parent`       — sessionId of the **immediate** parent
 * - `$ovo.root`         — sessionId of the **root** session in the chain
 * - `$ovo.parent_call`  — parent runtime-action tool call id (subagent rows only)
 * - `$ovo.parent_turn`  — parent turn id that dispatched the subagent (subagent rows only)
 * - `$ovo.subagent`     — active compiled graph node id (subagent rows only)
 * - `$ovo.trigger`      — channel adapter kind (session/subagent rows)
 * - `$ovo.title`        — truncated session title from the first user message
 * - `$ovo.channel_request_id` — inbound channel request id
 */
...
export function readChannelKind(serializedContext: Record<string, unknown>): string | undefined {
  const channel = serializedContext["ovo.channel"] as SerializedChannelAdapter | undefined;
...
export function readParentLineage(
  ...
  const parent = serializedContext["ovo.parentSession"] as SerializedSessionParent | undefined;
 ...
 *
 * `ovo.parentSession.rootSessionId` is denormalized at every dispatch
 ...
 * runs, which carry no `ovo.parentSession`.
 */
...
/**
 * Maximum visible length (in code points) of a derived `$ovo.title`.
 *
...
/**
 * Derives the `$ovo.title` value from the first user message of a
 * top-level session.
 *
...
/**
 * Builds the `$ovo.*` attribute payload for a top-level session run
 * (`workflowEntry` invoked without an `ovo.parentSession`).
 *
 * `$ovo.root` is intentionally omitted — the session row IS the root,
...
export function buildSessionAttributes(input: {
  ...
  return {
    "$ovo.channel_request_id": readChannelRequestId(input.serializedContext),
    "$ovo.type": "session",
    "$ovo.trigger": readChannelKind(input.serializedContext),
    "$ovo.title": deriveSessionTitle(input.inputMessage),
  };
}

/**
 * Builds the `$ovo.*` attribute payload for a delegated subagent root
 * run (`workflowEntry` invoked with an `ovo.parentSession`).
 *
 * `$ovo.root` carries the **root** session id so the dashboard can
 * group every descendant under one query: `search($ovo.root=<root>)`
...
export function buildSubagentRootAttributes(input: {
  ...
  return {
    "$ovo.channel_request_id": readChannelRequestId(input.serializedContext),
    "$ovo.type": "subagent",
    "$ovo.parent": input.parentSessionId,
    "$ovo.parent_call": input.parentCallId,
    "$ovo.parent_turn": input.parentTurnId,
    "$ovo.root": input.rootSessionId,
    "$ovo.subagent": input.identity.nodeId,
    "$ovo.trigger": readChannelKind(input.serializedContext),
  };
}

/**
 * Builds the `$ovo.*` attribute payload for one turn workflow run.
 *
 * Turns live one level below their session: `$ovo.parent` always points
 * to the parent's sessionId (which is the session-row's
 * `workflowRunId`), and `$ovo.root` denormalizes the chain root (equal
 * to `$ovo.parent` for turns of top-level sessions). Turn ordering is
...
export function buildTurnAttributes(input: {
  ...
  return {
    "$ovo.channel_request_id": input.requestId,
    "$ovo.type": "turn",
    "$ovo.parent": input.parentSessionId,
    "$ovo.root": input.rootSessionId,
  };
