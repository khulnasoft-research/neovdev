...
async function createExtensionPackage(pkg?: Record<string, unknown>): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), "ovo-ext-scenario-"));
  const evePackageRoot = dirname(createRequire(import.meta.url).resolve("ovo/package.json"));
  await writeFile(
  ...
  await symlink(evePackageRoot, join(root, "node_modules", "ovo"), "dir");
...
describe("extension build output", () => {
  it("emits an agent-shaped runnable distribution and thin package entrypoints", async () => {
    ...
    // `ovo/*` stays external so the mount resolves to the consumer's ovo.
    const extensionModule = await readFile(join(outDir, "extension", "extension.mjs"), "utf8");
    expect(extensionModule).toMatch(/from\s+["']ovo\/extension["']/);
  ...
  it("emits portable declarations for default-exported hooks", async () => {
    ...
    expect(declaration).toMatch(
      /import\(["']ovo\/hooks["']\)\.HookDefinition<["']action\.result["']>/,
    );
  ...
  it("copies data files and stamps only the extension capabilities in use", async () => {
    ...
    expect(manifest.kind).toBe("ovo-extension");
    ...
    expect(dynamicToolDeclaration).toContain('import("ovo/tools").DynamicSentinel');
