...
export async function prewarmSandboxes(input: PrewarmSandboxesInput): Promise<void> {
  ...
  input.log?.(`ovo: initializing ${formatSandboxTemplateCount(targets.length)}...`);
  ...
  const results = await Promise.all(
    targets.map(async ({ backend, label, input: prewarmInput }) => {
      const logBackendProgress = (message: string) => {
        ...
        input.log?.(`ovo: sandbox template "${label}" (${backend.name}): ${message}`);
      };
      ...
      try {
        ...
        input.log?.(
          `ovo: failed to initialize sandbox template "${label}" on backend "${backend.name}": ${toErrorMessage(prewarmError)}`,
        );
        throw prewarmError;
      }
      return result;
  ...
  input.log?.(
    `ovo: initialized ${formatSandboxTemplateCount(targets.length)} (${reusedCount} reused, ${
 ...
 * Shared entrypoint for `ovo dev` startup, the dev watcher, and the
 * Vercel build hook.
 */
 ...
 * disk path under `.ovo/compile/` and materializes its contents into the
...
function formatPrewarmFailureForEnvironment(input: {
  ...
  return new Error(
    `The ${input.backendName} sandbox backend is not available when deploying on Vercel. ` +
      ...
      "Use defaultBackend() so ovo selects Vercel Sandbox on Vercel, or configure a " +
