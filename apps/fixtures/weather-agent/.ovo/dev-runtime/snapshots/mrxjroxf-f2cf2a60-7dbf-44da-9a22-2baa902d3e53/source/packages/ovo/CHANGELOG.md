...
- 6ded22e: Preserve every queued delivery's message, context, and input responses when ovo batches payloads before routing.
...
- 1db41fd: `ovo/nuxt` now deploys the agent through Vercel's stable services model: on Vercel builds the module generates an ovo Build Output service and a `/ovo/v1/*` service route instead of writing legacy `experimentalServices` to `vercel.json`, which Vercel no longer routes (every agent request returned a platform NOT_FOUND). The `configureVercelJson` and `servicePrefix` module options and the `EVE_NUXT_SERVICE_PREFIX` export were removed. Delete any generated `experimentalServices` block from `vercel.json` — the module warns when it sees one — or declare the ovo service and its rewrite yourself under the stable `services` field to keep managing routing manually.

  A generated ovo service build now also skips host middleware preservation when the host's Build Output config is not yet present, instead of failing the build. Unlike the Next.js integration, which writes that config early, the Nuxt web service emits it only at the end of its own build, so an isolated ovo service build could crash reading a file that had no middleware to preserve.
...
- c0e368a: Routes protected by `httpBasic()` now advertise a standards-compliant `WWW-Authenticate: Basic` challenge on 401, using an optional realm that defaults to `"ovo"`; HTTP Basic credentials are normalized to Unicode NFC to match the advertised UTF-8 encoding. `routeAuth` collects challenges from the configured auth strategies instead of always emitting `Bearer`.
...
- 938ef92: Identify Ovo and its version on AI Gateway, Sandbox, and Workflow service requests.
...
- dad9472: `ovo info` now reports discovered subagents and schedules in both the human table and the `--json` output, matching what the CLI reference already documented. Previously both surfaces silently omitted them even though discovery resolved them correctly.
...
- 65a7823: `ovo dev` now writes a private per-process diagnostic log under `.ovo/logs/` capturing stderr, stdout (including sandbox and rebuild lines), tool failures, workflow errors, and ovo framework log records (stored structured, with level, namespace, and JSON fields). The file is JSON Lines: every line is one JSON record with `at` and `source` fields. Long stderr output collapses in the transcript to a one-line summary pointing at the log file (the raw text stays available in the `all` log mode), and error details reference the log instead of flooding the transcript.
- 65a7823: `ovo dev` now writes an environment dump next to each diagnostic log (`.ovo/logs/dev-<instance>.dump`): one JSON document capturing ovo, Node.js, and Vercel CLI versions, the Vercel CLI path, local session-store size, and running session stats (prompts, token usage, tool calls by name, subagent dispatches). `ovo logs --dump` prints the dump and its JSONL log together as one parseable report.
...
- 65a7823: New `ovo logs` command for reading the diagnostic logs `ovo dev` writes under `.ovo/logs/`: `ovo logs` prints the most recent log, `ovo logs ls [--json]` lists them, and `ovo logs <logid>` prints a specific log by id, file name, transcript path, or unambiguous prefix.
- 65a7823: `ovo logs --events` interleaves session events (session/turn/step lifecycle, message deltas) into the diagnostic-log output, resolved at query time from the local workflow store — nothing extra is written while `ovo dev` runs.
...
- b9bb8b2: Scaffold extensions with separate source and dist roots, a required wildcard ovo peer, and dist-only publication. `ovo extension build` now emits an agent-shaped JavaScript distribution with declarations, assets, and capability compatibility metadata instead of requiring author TypeScript in the published package.

### Patch Changes

- 29ecffc: Build mounted source-backed workspace extensions before `ovo dev` compiles the agent, then rebuild only the affected extension when its source changes. Failed extension builds keep the previous dist and active development generation serving.
- 46b78b8: Stop development microsandboxes on `ovo dev` shutdown by discovering resources from their run labels and loading the application's installed microsandbox version during cleanup.
...
- bbec675: Extensions installed with a registry-style store layout (e.g. from npm with pnpm) now work in `ovo dev` and `ovo eval`. Extension modules reached through a node_modules symlink resolve their dependencies from the package's real location — matching standard resolver semantics — instead of failing with `UNRESOLVED_IMPORT`/`ERR_MODULE_NOT_FOUND` or silently picking up another copy of the dependency from the consuming app.
- 14501dc: Every ovo-owned tool input is now validated against its schema before execution, so invalid calls are returned to the model for retry instead of failing the run. Subagent calls treat an empty `outputSchema` as absent, and OpenAPI operations with invalid schemas are omitted with a warning.
...
- bfbbe92: Add `ovo build --profile <path>` for a machine-readable build-timing and final-output-size report. Profile collection is best-effort, so reporting failures do not fail an otherwise successful build.
...
- 20cd9a1: Added `POST /ovo/v1/session/:sessionId/cancel` to the ovo HTTP channel for requesting cancellation of an in-flight turn. The optional `{ turnId }` body limits the request to the turn the caller observed; the response reports `"accepted"` when a cancellation hook accepts it or the benign `"no_active_turn"` when no resumable target exists.
- 02698fd: Fix Vercel deploys for the Next.js web channel. `ovo` no longer scaffolds a
  ...
  block was also redundant — `withEve()` generates the ovo service and
  `/ovo/v1/*` routes into the Build Output at build time — so the scaffold now
  writes a minimal `vercel.json`.
- e45a066: `ovo link` now lets you create a Vercel project or link an existing one, matching the project setup available through `/model` usage.
- 5f8818b: `ovo channels add web` now updates the Vercel Framework Preset when adding a
  Next.js web channel to an already-linked ovo project to prevent deployment
  failures.
...
- d9d3226: Fix `withEve` producing a broken Vercel build command in npm workspaces, where the `ovo` module is located in the workspace root.
...
- 0dbea62: Store local Workflow World data under `.ovo/.workflow-data` for both `ovo dev` and `ovo start`.
- 9058962: Keep an active `ovo dev` REPL session across successful authored-source reloads, so its next turn continues the durable conversation on the latest generation.
- 4531a3d: Route all of ovo's single-file bundles through a shared Rolldown helper that always disables code splitting. This closes a remaining gap where a dynamic import reachable from a final workflow bundle could still fail with "Expected one bundled ..." during builds.
...
- 159d674: Turns are now cancellable: resuming a session's durable cancel hook (`{sessionId}:cancel`, with an optional `turnId` guard) aborts in-flight work and settles the turn as a new `turn.cancelled` stream event followed by `session.waiting` — never as a failure. Channels and stream-event hooks can handle `turn.cancelled`, and `ovo/client` finalizes partially streamed messages. The HTTP cancellation API ships in a following release.

## 0.24.3

### Patch Changes

- dddfb20: Fix `ovo dev` generation bundling for authored modules whose dependencies use dynamic imports. Development generations now bundle ordinary dependencies directly instead of inheriting a copied server-external package list and tracing dependency closures; Nitro remains the sole owner of hosted dependency packaging.
- 2494b33: Bound `ovo dev` runtime snapshot storage with a World-independent retention policy. Superseded generations remain available for at least 30 minutes, and the five most recently superseded generations are retained as an additional rebuild-rate safety net.
...
- 0333a63: Rework `ovo dev` structural reloads to never interrupt admitted work: an isolated candidate must compile, bundle, and start before it is promoted atomically, the retired worker keeps serving the responses and sockets it already admitted until they settle, and a failed candidate or crashed worker leaves the server available with shutdown bounded even while streams are open.
- 5473f76: Keep active local Workflow turns on the development generation they selected across reloads, retries, and server restarts. New turns use the latest successful generation, and `ovo dev` stores local Workflow state under `.workflow-data`.
...
- d810570: Keep generated ovo service builds isolated from a colocated Next.js Build Output and preserve host middleware mappings when Vercel collects the generated service.
...
- 6aac45d: `ovo build` now uses invocation-owned compiler, host, Nitro, Workflow, and output workspaces. Concurrent builds can run beside `ovo dev`, and failed builds preserve the last successfully published output.
...
- e0f09b4: The `ovo dev` schedule dispatch route now reuses the module loader path resolved when the server is built, preventing module resolution failures in the bundled Windows dev server.
...
- 9a594c2: Infer authored channel metadata directly from the channel definition passed to `isChannel`, without compiler-generated declarations. Use `isChannel(...)` whenever you need authored metadata type narrowing; direct `channel:<name>` comparisons continue to identify channels but no longer narrow authored metadata.
  ...
  The `.ovo/**/*.d.ts` TypeScript include is no longer needed. Existing apps may remove it, but leaving the unmatched glob in place does not change typechecking.

## 0.22.6

### Patch Changes

- 5035812: Vercel deployments now emit `framework: { slug: "ovo" }` alongside the version in the Build Output API config. Vercel's build-output deserializer drops the entire `framework` object when `slug` is absent, so this restores framework attribution end to end — `framework_slug` and `framework_version` are now populated in AI Gateway routing and access logs.
...
- caa0c17: `ovo dev` now keeps Nitro build inputs outside prunable runtime snapshots. Long-running development servers no longer fail structural rebuilds with stale import errors after snapshot cleanup.
- 01f0345: Resume active local workflow runs on the agent-scoped queue after restarting the ovo server.

## 0.22.5

### Patch Changes

- c8f00aa: Add `experimental_chatgpt` under the new `ovo/models/openai` subpath: it returns an AI SDK language model served through the local Codex login (`codex login`), billed to the ChatGPT subscription, and defaults to `gpt-5.6-sol`. Direct provider API request errors now also surface their upstream message when one is available.
- 640cd8e: Keep provider streams moving while durable event writes are in flight. ovo now coalesces only adjacent queued text or reasoning appends behind an ordered writer, preserving event order while avoiding one durable round trip per provider delta.
- a5b43e7: Add `ovo extension init` and `ovo extension build` for scaffolding and building extension packages.
...
- f00f084: Add named multi-agent routing to `withEve` and `useEveAgent`. Next.js apps can now configure multiple ovo roots with `agents`, then target one from the frontend with `useEveAgent({ agent: "name" })`.
...
- 79df338: feat(ovo): scaffold projects with bundler module resolution

  `ovo init` now writes a `tsconfig.json` using `"moduleResolution": "bundler"` (and `"module": "esnext"`), which matches how ovo compiles authored agents and extensions. Relative imports in your agent and extension source no longer need `.js` extensions (e.g. `import extension from "../extension"`).
...
- fdf56ef: feat(ovo): mounted extensions

  Package ovo capabilities — tools, connections, skills, instructions, hooks — as a reusable package and mount it under `agent/extensions/`, as a file (`crm.ts`) or a directory with co-located override slots that shadow the extension's own contributions. Contributions compose into the agent under a `<namespace>__` prefix. Author with `defineExtension` from `ovo/extension`, taking an optional Standard-Schema `config` read via `extension.config`; `defineState` is auto-scoped to the package. `ovo build` compiles the package to runnable JavaScript with type declarations and fills its `exports`, so a published extension installs and mounts with no second compiler. `ovo` is a peer dependency whose declared range ovo enforces at mount; an extension cannot declare a sandbox, agent config, schedules, or limits, or mount other extensions.
...
- aff35e2: Stop `ovo dev` source snapshots from copying nested Git repositories and worktrees, preventing duplicate checkouts from inflating each development snapshot.
...
- 72c58ae: Recover `ovo dev <url>` authentication when Vercel Deployment Protection returns an SSO redirect or a structured protected-deployment response.
...
- c1c4ee5: Preserve query parameters passed to `ovo dev` and send them on every agent request, including session POSTs and streams.
...
- 4446f96: Update the vendored Workflow SDK packages to the latest 5.0 beta releases. ovo now delegates world target selection and construction to the upstream SDK instead of maintaining parallel factory and compatibility logic, and no longer disables stable Workflow Turbo mode.
...
- 2958abf: feat(ovo): add `konsistent` with initial config to enforce structural conventions
...
- bd287b1: fix(ovo): pass error messages when tool call input is invalid back to model instead of throwing so that it can try again

## 0.21.1

### Patch Changes

- 0b42ba1: `ovo eval` now shuts down tracked sandbox handles after a local one-shot eval run completes. This prevents local sandbox compute, including microsandbox sessions, from outliving the eval process.

## 0.21.0

### Minor Changes

- 79e9959: Expand the Chat SDK channel (`chatSdkChannel`): post completed assistant messages as markdown, stream replies via post-then-edit (configurable with `streaming` and `streamingEditIntervalMs`), surface typing status on turn start and tool calls, and degrade optional adapter operations (`startTyping`, `editMessage`) gracefully when an adapter does not implement them. Add the `messageToUserContent` inbound helper and export `isNotImplemented`. The default adapter webhook route is now `/ovo/v1/{adapter}`.
- 73a9bf9: feat(ovo): write skills into `$HOME/.agents/skills` instead of the workspace directory
...
- 6f9364a: Sandboxes are now stopped when the ovo server shuts down. Self-hosted production servers stop every open sandbox (microsandbox VMs, Docker containers, Vercel sandboxes, just-bash interpreters) on `SIGTERM`/`SIGINT`, matching the cleanup `ovo dev` already performs, and sessions reattach from persisted state on the next start. Breaking change for custom sandbox backends: `SandboxBackendHandle` gains a required `shutdown()` and the unused `dispose()` is removed.

### Patch Changes

- 7699e98: `ovo eval` now prints a clear, actionable message when it finds no evals but detects `*.eval.ts` files placed inside `agent/`. Instead of the generic "No evals found", it names the offending directories and reminds you that eval files belong in the top-level `evals/` directory (a sibling of `agent/`).
...
- 8892504: Render the deployment home page with the ovo SVG wordmark, baked-in agent name, and refined ready-state layout.
...
- c5da8e7: Sandbox API requests now append an `ovo/<version>` token to the `user-agent` so the sandbox control plane can attribute traffic to ovo.
...
- a75dd47: Vercel sandbox: drop the `runtime` option. ovo now always boots its hosted sandboxes from the published ovo image.

### Patch Changes

- afc7ded: Allow `ovo dev` to start from a package-less flat agent that only has `instructions.md`. Development runtime snapshots now generate private package metadata when the authored app has none.
- 7d1084d: fix(ovo): remove unnecessary preview deployment check that prevented production access from `ovo dev`
...
- ba2e0ce: Create new Vercel projects through `vercel link` instead of posting directly to the projects API. This lets the Vercel CLI apply its framework and local config handling while ovo reads the resulting link metadata, keeps framework-specific ovo host integrations when detected, and otherwise ensures new projects use the ovo framework preset.

## 0.17.2

### Patch Changes

- afa22f8: Cap recursive subagent delegation at three child-session levels by default, configurable with `defineAgent({ limits: { maxSubagentDepth } })`. At the limit, ovo no longer advertises subagent tools and blocks stale delegated calls before starting another child session.
- 55e9ad5: Update the scaffold's default agent model to `anthropic/claude-sonnet-5`. New agents created with `ovo init` (and the setup model picker's pre-selected default) now use Claude Sonnet 5 instead of Claude Sonnet 4.6.
...
- ed8a935: Keep the `Workflow` orchestration tool root-only. Delegated subagent sessions can still call visible subagent tools directly until the configured depth cap, but ovo no longer advertises `Workflow` from those child sessions.
- 37bd2bb: Use authored `eveChannel()` auth for `GET /ovo/v1/info` so remote `ovo dev` can authenticate with the same policy as the session routes.
...
- 39c90de: Add explicit `dev:ovo`, `build:ovo`, and `start:ovo` scripts to generated Web Chat projects so users can run the embedded ovo app directly when needed.

## 0.17.1

### Patch Changes

- 97aa99b: Add HTTP Basic userinfo and repeatable `-H, --header` support to `ovo dev` URL targets so the terminal UI can send credentials or routing headers to protected remote deployments.
...
- 739af96: Update ovo's bundled Workflow SDK dependency set to the latest 5.0.0 beta releases, keeping the core package and local workflow world aligned.

## 0.17.0

### Minor Changes

- 02ed501: Remove the experimental `ExperimentalWorkflow` opt-in marker from the public `ovo/tools` API and remove the dynamic Workflow docs. The internal runtime path remains in place for existing compiled manifests, but authored apps can no longer enable the tool through the public API.
...
- 9580a88: Disable the Workflow SDK turbo first-delivery path for ovo. Workflow runs now stay on the fully ordered runtime path instead of the beta turbo mode.

## 0.16.1

### Patch Changes

- 8470695: HTTP channels can now opt into browser CORS with preflight handling. Use `defineChannel({ cors })` for custom channels or `eveChannel({ cors: true | options })` for the ovo channel; omitted CORS remains disabled.
...
- c14b022: The `ovo dev` TUI retries one transient agent-inspection failure before treating a local or remote agent as unavailable.
...
- ddda14c: Fresh agents now start model setup from their prefilled `/model` prompt, installing the Vercel CLI and logging in when those prerequisites are missing. Other `ovo dev` sessions leave missing model setup as an attention prompt.
...
- 2e00da7: Scope workflow queue prefixes to each ovo agent so multiple uniquely named agents can deploy in the same project without consuming one another's workflow messages.
...
- b049756: Use the active ovo development server URL for connection authorization callbacks. Local Vercel Connect flows now return to ovo's actual port instead of Workflow's port 3000 fallback.
- 2933ab2: The local `ovo dev` status bar now shows a gray `:port` badge and retains it as terminal width narrows. Status segments now use tighter spacing.
- 2e4e15d: `ovo init` now accepts `--yes` as a no-op compatibility flag and warns before continuing.
- 2933ab2: Running `ovo dev` interactively now reconnects to the healthy loopback dev server recorded for the same app root, with a fresh session for each attached terminal UI. Ovo replaces stale or malformed state when it starts a new server. `--host`, `--port`, or `PORT` skips reconnection and reports a healthy recorded server instead.
...
- d83b418: ovo's health endpoint (`/ovo/v1/health`) now responds to `HEAD` requests, not just `GET`, so load balancers and uptime monitors that probe with `HEAD` (UptimeRobot, Kubernetes probes, and others) no longer report a healthy deployment as down.
- e5ccf93: Self-hosted `ovo start` now registers the workflow queue handler for custom (non-Vercel) worlds, so jobs dispatched by a configured world no longer return `Unhandled queue` or leave runs stuck `pending` — and you no longer need `ovo dev --no-ui` to run a local world in production. ovo also fails fast at boot with an actionable error when a configured workflow world's `@workflow/*` version is incompatible with the line ovo bundles, instead of surfacing a cryptic `ZodError` deep in workflow replay.
...
- a3d8441: Fix dynamic `Workflow` fan-out so concurrent subagent calls dispatch together, replay in deterministic program order, and resume reliably across runtime isolates. Generated pnpm workspaces now exempt the bundled code-mode package from release-age gating so fresh ovo releases install immediately.
- 91e43ae: Upgrade ovo to the stable AI SDK 7 release and copy vendored AI SDK declarations directly from the installed packages. Newly scaffolded pnpm workspaces now exempt the AI SDK, Vercel, and Workflow package families from minimum release age checks.
...
- ff44c4c: Clarify scaffolded guidance for locating bundled ovo package docs in workspaces and local installs.
- 30c5965: Preserve dynamic tool approval gates when session- and turn-scoped tools are replayed from durable metadata. If a replayed approval callback cannot be recovered, ovo now requires approval by default instead of silently running the tool unguarded.
...
- a63dfa2: Remote `ovo dev --url` now treats `/ovo/v1/info` as best-effort inspection rather than a connection gate. Once authentication succeeds and the deployment is reachable, the session connects even when the agent info route is absent (confirmed via the public health route) or returns an unrecognized shape (e.g. a deployment built from an older ovo). Inspection-only data is simply omitted from the header in that case, and the underlying parse failure now names the offending fields instead of an opaque message.
- a63dfa2: Remote `ovo dev --url` sessions now show deployment and authentication state, try refreshed project-scoped OIDC credentials at startup, and open a cancellable `/vc:login` recovery flow when access is rejected. The flow can update the target project's Trusted Sources after confirmation.
- c5071e6: Ensure every ovo Workflow runtime entrypoint installs the ovo queue namespace through a single guarded boundary.
- c9e895b: Fix `ovo dev` streaming throughput and time-to-first-token degrading as parked (`ask_question` / HITL) sessions accumulate. The dev runtime's NDJSON event-stream reader now forwards cancellation to the underlying run stream, so disconnecting from a parked session no longer leaks a filesystem polling loop for the life of the dev server.
- c6b2da8: Add `$ovo.channel_request_id` workflow attributes from Vercel's `x-vercel-id` header so session and turn workflow runs can be joined back to the inbound request that started or resumed them.
...
- a63dfa2: The dev TUI's `/vc` and `/login` commands are now `/vc:install` and `/vc:login`. `/vc:login` is the single Vercel authentication command: it logs in locally and, in remote (`ovo dev --url`) sessions, recovers access with Vercel OIDC.
...
- 25b1b14: fix(ovo): catch unserializable tool output values instead of sending them to the model

## 0.13.4

### Patch Changes

- efca390: Make optional sandbox engine loading more resilient after auto-install. ovo now
...
- 7079d08: Bundle client-safe vendored dependencies in a neutral chunk group so `ovo/react` can use the Zod-backed `/ovo/v1/info` validator without pulling in Node-only vendored runtime helpers.
...
- 36b67fc: Make `ovo init` respect ancestor package-manager workspaces when scaffolding nested packages. The scaffold now updates workspace-owned package policy at the npm, pnpm, Yarn, or Bun workspace root instead of writing nested root-only config into the generated package.
...
- 27a9701: Resolve extensionless relative imports whose target basename contains dots when bundling authored modules. Local files such as `./mock-registry.schemas` and dependency requires such as `./Reflect.getPrototypeOf` now probe Ovo's configured `.ts` and `.js` extensions before being treated as asset imports.
- 3a64a8f: `ovo init` with no target, when run by a coding agent, now prints a setup guide — what to ask the user, then the scaffold command — instead of scaffolding the current directory. The guide routes both channels (Slack credentials) and connections (per-user OAuth) through Vercel Connect so credentials are provisioned rather than hand-managed. `ovo init <name>` and `ovo init .` are unchanged.
- 3a64a8f: `ovo init` now offers to open an installed coding-agent REPL when its CLI is on `PATH`, while keeping `ovo dev` as the default. It detects Claude Code, Codex, Cursor, Droid, Gemini CLI, opencode, and Pi. The selected REPL starts with a project-specific setup prompt and `ovo dev --no-ui` verification guidance. Coding-agent and non-interactive launches, plus systems without any supported CLI, keep the existing development-server handoff.
...
- 25ab1e7: Preserve dev-runtime snapshots that are still referenced by local durable workflow data so parked HITL turns can resume after `ovo dev` rebuilds.
- 504f59e: Allow `ovo eval` target checks to match a scoped package name such as `@acme/agent` against the runtime agent identity `agent`.
...
- 8f7d97b: Keep Vercel Sandbox option types synchronized with the installed SDK by vendoring its upstream declaration files instead of maintaining a hand-written copy. Vercel-backed file reads now convert provider Node streams to Ovo's public Web stream contract.
...
- f68ecbe: Set the Ovo Vercel framework preset when creating standalone Ovo projects.
- c084232: Verify remote Vercel deployment origins against the owner and project supplied by `VERCEL_ORG_ID` and `VERCEL_PROJECT_ID`, or by a local project link, before sending ambient credentials. Remote dev and eval clients now refresh scoped OIDC tokens per request and refuse to forward credentials across redirects. Remote `ovo dev` and `ovo eval --url` targets now require `https://` (loopback hosts may still use `http://`).
...
- 10e9237: Fix code-defined models under `ovo dev`, including NodeNext `.js` imports that target authored `.ts` files. Runtime model resolution now reuses the active agent bundle's module map and node scope, so child agents resolve their own models without rebuilding authored modules on each step.

## 0.11.10

### Patch Changes

- c707ca3: Keep `ovo init` and local `ovo dev` progress on one terminal row. Init now includes elapsed completion times and preserves useful package-manager diagnostics on failure. With `EVE_LOG_LEVEL=debug`, both commands use plain phase logs instead of animation.
...
- 4bfbaa0: Add root agent `experimental.workflow.world` configuration for selecting an installed Workflow world package. Ovo now loads and registers the configured world at runtime and documents how self-hosted deployments can provide a custom Workflow world.

## 0.11.8

### Patch Changes

- 4622d94: Point the npm README, runtime landing page, and setup guidance at the canonical ovo documentation domain.
- bfc7191: Use the official TypeScript 7 `tsc` compiler for ovo builds, base generated projects, and fixture typechecks. Next.js projects and generated Web Chat apps pin `typescript@6.0.3`, which still provides the JavaScript compiler API Next.js requires.
...
- 7b8df64: Serialize optional sandbox engine auto-installs and reload newly installed engines through their package entrypoint file instead of retrying the cached bare specifier. This prevents first-run `ovo dev` sessions from racing microsandbox installation or surfacing Node's stale same-process module-not-found result after Bun installs `microsandbox`.

  `ovo init` also supports `EVE_INIT_PACKAGE_SPEC` so local tarball/source validation can make the generated project install the same ovo build under test instead of resolving the published semver range from the registry.
...
- 93ff280: The `ovo dev` header now shows the beta-terms link inline (`ovo is currently in preview: <url>`), clickable via the terminal's own URL matcher. The verbose preview notice is dropped from the boot banner and from `ovo init` output.
- 432503d: Clarify the duplicate `ovo dev` process error with a copyable package-manager command for connecting to the existing local server instead of stopping it.
- c0c5cbf: Upgrades the workflow dependency to 5.0.0-beta.19
- 602e9e0: Detect parent workspace package managers when running `ovo init <name>` so fresh agents created inside monorepos install with the workspace manager instead of always following the launcher.
...
- e5b777b: Resolve AI Gateway OIDC readiness through Vercel's token resolver so `ovo dev` recognizes projects linked by the Vercel CLI without requiring an environment pull or showing a missing-credentials setup issue.

## 0.11.3

### Patch Changes

- 1e2e8ef: Standardize the product name as `ovo` across documentation, CLI output, diagnostics, generated text, and runtime messages.
...
- e7cdefd: Handle missing sandbox template and session state more gracefully across Vercel, Microsandbox, and Docker backends. ovo now treats stale Vercel template references, missing Microsandbox session/template snapshots, and Docker template image races as recoverable provisioning misses so the runtime can rebuild or create a fresh sandbox automatically.
...
- ff80e38: The `ovo eval --verbose` help text now refers to `t.log` (the actual eval context logging API) instead of the outdated `ctx.log`.
...
- c2ac540: Initial public release of the ovo framework
