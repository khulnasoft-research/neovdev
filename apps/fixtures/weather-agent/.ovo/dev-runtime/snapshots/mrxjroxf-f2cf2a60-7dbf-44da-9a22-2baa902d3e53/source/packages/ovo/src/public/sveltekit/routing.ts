...
/**
 * Private route namespace for hosting ovo as a separate experimental Vercel
 * service behind the SvelteKit app.
 */
export const EVE_SVELTEKIT_SERVICE_PREFIX = "/_eve_internal/ovo";
...
export function normalizeRoutePrefix(prefix: string): string {
  ...
  if (normalized.length === 0) {
    throw new Error("ovo SvelteKit service prefix cannot resolve to the root route.");
  }
  return normalized;
}
...
/**
 * A Vercel rewrite that forwards ovo transport requests (`/ovo/v1/**`) to the
 * private ovo service prefix (`/_eve_internal/ovo/ovo/v1/**`).
 */
...
/**
 * Build the Vercel rewrite that forwards browser ovo transport requests to the
 * sibling ovo service.
 */
