...
export async function compileInstructionsEntry(
  ...
  if (source.sourceKind === "markdown") {
    const definition = normalizeInstructionsDefinition(
      source.definition,
      `Expected the compiled instructions definition at "${source.logicalPath}" to match the public ovo shape.`,
    );
    return {
      kind: "instructions",
      definition: {
  ...
  if (isDynamicSentinel(exportValue)) {
    rejectDynamicSentinelFallback(
      exportValue,
      `Expected the instructions export "${source.exportName ?? "default"}" from "${source.logicalPath}" to match the public ovo shape.`,
    );
