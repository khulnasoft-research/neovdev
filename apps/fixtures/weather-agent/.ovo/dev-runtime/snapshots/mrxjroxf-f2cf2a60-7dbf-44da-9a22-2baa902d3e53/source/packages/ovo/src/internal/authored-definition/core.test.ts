...
const FAILURE_MESSAGE = "Expected the agent config to match the public ovo shape.";
  ...
  it.each(["approval", "needsApproval"])("rejects the removed %s field", (field) => {
    expect(() =>
      normalizeScheduleDefinition(
        ...
        "Expected the schedule config to match the public ovo shape.",
      ),
