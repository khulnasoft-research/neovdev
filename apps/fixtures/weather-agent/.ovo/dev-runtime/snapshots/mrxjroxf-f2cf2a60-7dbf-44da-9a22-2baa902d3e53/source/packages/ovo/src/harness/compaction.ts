...
export async function compactMessages(
  ...
  while (true) {
    ...
    const result = await generateText({
      ...
      telemetry: telemetry ? { ...telemetry, functionId: "ovo.compaction" } : undefined,
      temperature: 0,
    });
...
const CAPPED_RESULT_ANNOTATION =
  "[Truncated by ovo: tool result reduced during context compaction. Re-run the tool if you need the full output.]";
