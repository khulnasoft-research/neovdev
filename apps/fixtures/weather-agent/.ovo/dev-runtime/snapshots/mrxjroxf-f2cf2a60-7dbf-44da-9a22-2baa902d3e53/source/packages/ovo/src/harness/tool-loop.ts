 ...
 * ovo-specific identifiers such as `ovo.session.id`) is flowed through
...
const TURN_TRACE_STATE_KEY = "ovo.harness.turnTrace";
...
export function createToolLoopHarness(config: ToolLoopHarnessConfig): StepFn {
  ...
  const tracer = telemetryConfig !== undefined ? trace.getTracer("ovo") : undefined;
  ...
  async function runStep(
    ...
    if (tracer && hasStepInput(input)) {
      ...
      const attributes: Record<string, string> = {
        "ovo.version": eveVersion,
        "ovo.environment": environment,
        "ovo.session.id": initialSession.sessionId,
      };
      if (functionId) {
        attributes["ai.telemetry.functionId"] = functionId;
      }
      turnSpan = tracer.startSpan("ai.ovo.turn", { attributes });
    }
  ...
  async function executeStepBody(
    ...
    if (emit && hasStepInput(input)) {
      ...
      if (turnSpan) {
        turnSpan.setAttribute("ovo.turn.id", emissionState.turnId);
      }
    }
    ...
    if (userContent !== undefined && !pending.deferredMessage && !pending.consumedMessage) {
      ...
      // each part's `data` with a compact `ovo-sandbox:` URL. The
     ...
     */
    // Hydrate `ovo-sandbox:` ref FileParts into inline bytes for the
    ...
    const runSingleModelCall = async (opts: ModelCallOptions): Promise<HarnessStepResult> => {
      ...
      if (opts.retryReason) {
        telemetryRuntimeContext["ovo.retry.reason"] = opts.retryReason;
      }
    ...
    await setEveAttributes({
      "$ovo.model": modelTag,
      "$ovo.input_tokens": nextTurnUsage.inputTokens,
      "$ovo.output_tokens": nextTurnUsage.outputTokens,
      "$ovo.cache_read_tokens": nextTurnUsage.cacheReadTokens,
      "$ovo.cache_write_tokens": nextTurnUsage.cacheWriteTokens,
      "$ovo.cost_usd": nextTurnUsage.sawCost ? nextTurnUsage.costUsd : undefined,
      "$ovo.tool_count": config.tools.size,
    });
