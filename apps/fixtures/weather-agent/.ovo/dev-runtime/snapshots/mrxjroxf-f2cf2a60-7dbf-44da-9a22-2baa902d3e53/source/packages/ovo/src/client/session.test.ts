...
describe("ClientSession", () => {
  it("cancels an accepted turn before its stream settles with freshly resolved auth", async () => {
    ...
    expect(new URL(requests[1]!.url).pathname).toBe("/ovo/v1/session/session_1/cancel");
  ...
  it("continues the session after consuming through session.waiting", async () => {
      ...
      return createStreamResponse([
        {
          ...
          data: { continuationToken: "ovo:rekeyed", wait: "next-user-message" },
        },
      ]);
    });
    const session = createSession();
    ...
    expect(new URL(postRequests[1]!.url).pathname).toBe("/ovo/v1/session/session_1");
    expect(postRequests[1]!.body).toEqual({
      continuationToken: "ovo:rekeyed",
      message: "second",
    });
  });
  ...
  it("resets the session by default after consuming through session.completed", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledTimes(3);
    const postRequests = requests.filter((request) => request.method === "POST");
    expect(new URL(postRequests[1]!.url).pathname).toBe("/ovo/v1/session");
    expect(postRequests[1]!.body).toEqual({
      message: "second",
    });
  });
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (request) => {
      ...
      if (streamRequest === 1) {
        return createStreamResponse([
          {
            type: "session.waiting",
            data: { continuationToken: "ovo:first", wait: "next-user-message" },
          },
        ]);
      }
