 ...
 *   `@vercel/connect/ovo`, or {@link defineInteractiveAuthorization}).
 */
...
/**
 * Controls Ovo runtime behavior for an inline tool auth provider.
 */
export interface ToolAuthOptions {
  ...
  /**
   * Optional Ovo auth-flow key for token caches, callback URLs, pending
...
> {
   ...
   *
   * Use the helpers from `ovo/tools/approval` for common cases:
 ...
 */
const DISABLED_TOOL_SENTINEL_KIND = "ovo:disabled-tool";
 ...
 */
const EXPERIMENTAL_WORKFLOW_TOOL_KIND = "ovo:enable-workflow-tool";
