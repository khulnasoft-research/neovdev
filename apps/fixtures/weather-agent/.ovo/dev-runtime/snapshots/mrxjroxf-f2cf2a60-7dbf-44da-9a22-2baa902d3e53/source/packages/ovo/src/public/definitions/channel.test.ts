...
describe("defineChannel", () => {
  ...
  it("declares websocket routes with an ovo-owned route discriminator", () => {
    const channel = defineChannel({
      routes: [
        WS("/ws", () => ({
  ...
  it("hands the live SessionHandle to the context() factory and exposes it on ctx.session", () => {
    ...
    const accessor: ContextAccessor = {
      get: <T>(key: { name: string }): T | undefined =>
        key.name === "ovo.continuationToken" ? (continuationToken as T) : undefined,
      has: () => false,
      require: () => {
    ...
    expect(writes).toEqual([["ovo.continuationToken", "slack:C123:T456"]]);
  });
