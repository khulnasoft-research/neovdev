...
export interface DiscordChannelCredentials extends DiscordCredentials {
  /** Custom inbound webhook verifier. When supplied, ovo skips the `DISCORD_PUBLIC_KEY` fallback and delegates verification to it. */
...
export interface DiscordChannelConfig {
  ...
  /** Override the default interaction route path (`/ovo/v1/discord`). */
...
export interface DiscordHandle {
  ...
  /** Discord channel id. */
  readonly channelId: string;
  /** Current ovo conversation id, usually the Discord anchor message id. */
  readonly conversationId: string;
