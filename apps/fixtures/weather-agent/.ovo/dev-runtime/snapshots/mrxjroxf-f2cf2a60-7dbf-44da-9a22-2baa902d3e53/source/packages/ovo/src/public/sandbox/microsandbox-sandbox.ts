 ...
 * are ovo-owned rather than a raw passthrough so the public surface can
...
export interface MicrosandboxSandboxCreateOptions {
  /**
   * OCI image used as the base runtime. ovo prepares this image with
   ...
   * runtime. By default ovo installs both automatically when missing —
   * the npm package with the project's package manager (during
   * `ovo dev` only), the runtime via microsandbox's own installer.
   */
  readonly setup?: {
