...
function registerHandler(
  ...
  const virtualId = `#ovo-route-handler/${options.method ?? "ALL"} ${options.route}`;
...
/**
 * Describes a workflow queue entrypoint ovo will register as an in-process
 ...
 * required for `ovo dev` on Windows where the worker → main → worker proxy
...
function buildWorkflowFileHandlerSource(input: {
  ...
  if (input.directHandlers.length > 0 && input.runtimeImportSpecifier !== undefined) {
    ...
    lines.push(
      "  }",
      "} catch (err) {",
      '  console.warn("[ovo] Failed to register direct workflow queue handlers:", err);',
      "}",
    );
  }
...
function addFrameworkVirtualHandler(
  ...
  const virtualId = `#ovo-route${input.route}`;
...
/**
 * Wires ovo's package-owned app, channel, workflow inspection, dev-control,
...
/**
 * Wires the subset of ovo's package-owned endpoints that belong to the given
