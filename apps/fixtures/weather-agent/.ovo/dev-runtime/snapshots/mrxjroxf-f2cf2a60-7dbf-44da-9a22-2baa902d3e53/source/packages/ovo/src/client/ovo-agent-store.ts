...
/**
 * Framework-agnostic state machine for an ovo agent session.
 *
...
export class EveAgentStore<TData> {
  ...
  async send<TOutput = unknown>(input: SendTurnPayload<TOutput>): Promise<void> {
    if (this.#status === "streaming" || this.#status === "submitted") {
      throw new Error("ovo session is already processing a turn.");
    }
  ...
  #createOwnedSession(): ClientSession {
    if (!this.#createSession) {
      throw new Error("Cannot create an owned ovo session from an external session.");
    }
