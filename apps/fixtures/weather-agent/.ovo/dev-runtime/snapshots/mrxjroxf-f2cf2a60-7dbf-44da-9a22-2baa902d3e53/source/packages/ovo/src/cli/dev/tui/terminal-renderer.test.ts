...
describe("TerminalRenderer (inline scrollback)", () => {
  it("prints the dim wordmark tag as the parting line after a Ctrl-C exit", async () => {
    ...
    expect(lines.at(-1)).toMatch(/^☰ovo {2}v\d+\.\d+\.\d+/u);
    expect(screen.rawOutput()).toContain(`\x1b[2m☰ovo  v`);
    // Once, ever — repeated teardown must not repeat the tag.
    renderer.shutdown();
    expect(screen.snapshot().match(/☰ovo/gu)).toHaveLength(1);

    // A renderer that never went live exits silently.
    const idle = makeRenderer();
    idle.renderer.shutdown();
    expect(idle.screen.snapshot()).not.toContain("☰ovo");
  });
  ...
  it("renders the brand line with the agent name and a tip", () => {
    ...
    expect(snapshot).toContain("ovo Weather Agent");
  ...
  it("breathes between an answered question and the next tool block", async () => {
    ...
    expect(screen.snapshot()).toContain("⎿  ovo\n\n");
    renderer.shutdown();
  });
  ...
  it("sweeps a preparing placeholder whose call never parses", async () => {
    ...
    expect(snapshot).toContain("Searched ovo framework");
  ...
  it("stores long stderr diagnostics and shows concise copy by default", () => {
    ...
    expect(screen.snapshot()).toContain("details: .ovo/logs/dev.log");
    expect(screen.snapshot()).not.toContain("at fourth");

    renderer.setLogDisplayMode("all");
    expect(screen.snapshot()).toContain("at fourth");
    expect(screen.snapshot()).not.toContain("details: .ovo/logs/dev.log");
  ...
  it("subscribes the recorder to log records, displays them, and releases on shutdown", () => {
    ...
    expect(screen.snapshot()).toContain("[ovo:harness.tool-loop] tool execution failed");
  ...
  it("records sandbox log lines in the diagnostic log", () => {
    ...
    renderer.renderSandboxLog('ovo: sandbox template "root" (microsandbox): apt-get update');
  ...
  it("shows sandbox stdout lines and hides ordinary stdout under the sandbox log level", () => {
    ...
    process.stdout.write('ovo: sandbox template "root" (microsandbox): checking cached snapshot\n');
    process.stdout.write("ovo: initializing 3 sandbox templates...\n");
    process.stdout.write('ovo: built sandbox template "root" on backend "microsandbox".\n');
  ...
  it("renders subscribed sandbox logs under the sandbox log level", () => {
    ...
    renderer.renderSandboxLog?.('ovo: sandbox template "root" (docker): checking Docker daemon');
    renderer.renderSandboxLog?.("ovo: initializing 3 sandbox templates...");
    renderer.renderSandboxLog?.('ovo: built sandbox template "root" on backend "docker".');
  ...
  it("delays dev rebuild errors until explicitly flushed", () => {
    ...
    process.stderr.write("[ovo:dev] rebuild failed: expected default export\n");

    expect(screen.snapshot()).not.toContain("expected default export");

    renderer.flushDelayedDevBuildErrors();

    expect(screen.snapshot()).toContain("│ [ovo:dev] rebuild failed");
  ...
  it("delays multi-line dev rebuild errors as one block", () => {
    ...
    process.stderr.write("[ovo:dev] rebuild failed: first line\nsecond line\n");
  ...
  it("drops delayed dev rebuild errors after a successful rebuild", () => {
    ...
    process.stderr.write("[ovo:dev] rebuild failed: missing export\n");
    process.stdout.write(`${AUTHORED_ARTIFACTS_UPDATED_LOG_LINE}\n`);
    renderer.flushDelayedDevBuildErrors();
