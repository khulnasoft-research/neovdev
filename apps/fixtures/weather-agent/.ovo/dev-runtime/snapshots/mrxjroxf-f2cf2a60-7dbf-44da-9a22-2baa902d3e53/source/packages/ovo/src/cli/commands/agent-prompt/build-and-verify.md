...
Work from the project directory. Once ovo is installed, the full docs are bundled
...
- Put the purpose in `agent/instructions.md` (the always-on system prompt),
  ...
  do.
- Add a first typed tool under `agent/tools/` with `defineTool` from `ovo/tools`
  and a Zod `inputSchema`.

`{{devCommand}}` starts ovo's HMR development server and opens the agent's
terminal REPL. It does not start or control this coding-agent session, so don't
use the bare command as a background verification process. Start ovo without the
...
`POST /ovo/v1/session`, attach to `GET /ovo/v1/session/:id/stream`, and send a
