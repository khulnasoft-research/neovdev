 ...
 * named context slot.
 *
 * All operations require an active ovo context (ALS scope) and throw
...
const RESERVED_STATE_NAME_PREFIX = "ovo.";
 ...
 *
 * The name must not start with the reserved `"ovo."` prefix (reserved
 * for framework context keys); doing so throws.
 *
 * All operations require an active ovo context. Calling `get()` or
...
export function defineState<T>(name: string, initial: () => T): StateHandle<T> {
  if (name.startsWith(RESERVED_STATE_NAME_PREFIX)) {
    throw new Error(
      `defineState() name "${name}" uses the reserved "${RESERVED_STATE_NAME_PREFIX}" prefix, which ovo reserves for its own framework context keys (e.g. "ovo.channel", "ovo.bundle"). Colliding with one silently corrupts context serialization. Use your own namespace, e.g. "my-agent.${name.slice(RESERVED_STATE_NAME_PREFIX.length)}".`,
    );
  }
