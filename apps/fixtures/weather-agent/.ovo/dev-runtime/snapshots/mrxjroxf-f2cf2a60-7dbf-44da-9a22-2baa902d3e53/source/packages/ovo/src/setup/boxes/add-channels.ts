...
const SLACK_HEADLESS_ERROR =
  "Slack setup is interactive. Run `ovo channels add slack` from an interactive terminal.";
  ...
  switch (result.state) {
    case "not-installed":
      return {
        ...
        followUp:
          "Continuing without Slack — the install timed out and was cleaned up; re-run `ovo channels add slack` to try again.",
      };
    case "cleanup-failed":
      return {
    ...
    case "connector-lookup-failed":
      return {
        ...
        followUp:
          "Continuing without Slack — restore Vercel CLI access, then re-run `ovo channels add slack`.",
      };
    case "installation-check-failed":
      return {
        reason: SLACKBOT_INSTALLATION_CHECK_FAILED_ERROR,
        followUp:
          "Continuing without Slack — verify Vercel Connect is reachable, then re-run `ovo channels add slack`.",
      };
    ...
    case "create-failed":
      return {
        ...
        followUp: "Continuing without Slack — add it later with `ovo channels add slack`.",
      };
  }
}
...
export interface AddChannelsOptions {
  ...
  headless?: boolean;
  /**
   * ovo package metadata for the scaffolded web `package.json`. When omitted,
  ...
  /** Overwrite existing channel files (`ovo channels add --force`). */
  force?: boolean;
  /**
   * Override for the web scaffold's Vercel services config. Defaults to
   * `hasVercelProject(state)`; `ovo channels add` pins it to true so an
   ...
   * slackbot. Only the `ovo channels add` composition sets this; onboarding
   ...
   * default, "abort", fails the whole box — right for `ovo channels add slack`,
   ...
   * a later `ovo channels add slack` starts clean).
   */
...
function warnCompetingNextConfigFiles(
  ...
  for (const filePath of files ?? []) {
    log.warning(
      `Found competing Next.js config at ${filePath}; merge any needed settings into next.config.ts and remove it before starting the preview, or Next.js may ignore the generated ovo rewrite.`,
    );
  }
}
 ...
 * dependencies the scaffold added to `package.json` so a running `ovo dev`
...
export function addChannels(
  ...
  async function installChannelDependencies(
    ...
    // succeeds, `ovo dev` cannot load the new channel modules.
    log.warning(
