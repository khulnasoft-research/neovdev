 ...
 * package is intentionally not bundled with ovo — the backend is
 * opt-in — so when it is missing ovo installs it into the project
 * during `ovo dev` (unless `autoInstall: false`) and otherwise fails
...
async function loadJustBashModule(input: {
  ...
  justBashModulePromise ??= loadOptionalEnginePackage<JustBashModule>({
    ...
    missingMessage:
      ...
      "with ovo. Install it in your application (for example `pnpm add -D just-bash`), or use " +
