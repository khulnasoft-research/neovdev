...
const PORTABILITY_CASES: readonly PortabilityCase[] = [
  {
    descriptor: {
      files: {
        "agent/sandbox.ts": `import { defaultBackend, defineSandbox } from "ovo/sandbox";
import { docker } from "ovo/sandbox/docker";
import { justbash } from "ovo/sandbox/just-bash";
import { microsandbox } from "ovo/sandbox/microsandbox";
...
`,
  ...
  },
  {
    ...
    include: ["src/public/channels/auth.ts", "src/public/channels/ovo.ts"],
    ...
    packageExports: {
      ...
      "./channels/ovo": {
        types: "./dist/src/public/channels/ovo.d.ts",
      },
    },
  },
];
...
async function expectPortableFixtureToTypecheck(testCase: PortabilityCase): Promise<void> {
  const scratchRoot = await createScratchDirectory("ovo-public-api-portability-");
  const emittedPackageRoot = join(scratchRoot, "ovo");
  ...
  await cp(emittedPackageRoot, join(appRoot, "node_modules", "ovo"), {
    recursive: true,
  });
  await writeFile(
