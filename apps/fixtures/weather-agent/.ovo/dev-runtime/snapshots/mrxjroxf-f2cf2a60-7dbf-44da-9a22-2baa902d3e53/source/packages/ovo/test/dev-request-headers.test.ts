...
describe("createDevelopmentRequestHeadersAsync", () => {
  it("adds the Vercel protection bypass header for preview-scoped ovo routes", async () => {
    ...
    const headers = await createDevelopmentRequestHeadersAsync({
      ...
      resourceUrl: new URL("https://example.com/preview/ovo/v1"),
    });
  ...
  it("skips the bypass header for non-ovo routes", async () => {
    vi.stubEnv("VERCEL_AUTOMATION_BYPASS_SECRET", "secret_123");
  ...
  it("prefers the forwarded Vercel runtime OIDC header when Authorization is absent", async () => {
    const headers = await createDevelopmentRequestHeadersAsync({
      ...
      resourceUrl: new URL("https://example.com/ovo/v1"),
    });
  ...
  it("skips the OIDC bearer for 127.0.0.1 targets too", async () => {
    ...
    const headers = await createDevelopmentRequestHeadersAsync({
      resourceUrl: new URL("http://127.0.0.1:3000/ovo/v1"),
    });
  ...
  it("does not call getVercelOidcToken when the target ovo route is on localhost", async () => {
    vi.mocked(getVercelOidcToken).mockResolvedValue("oidc_token_456");

    const headers = await createDevelopmentRequestHeadersAsync({
      resourceUrl: new URL("http://localhost:3000/ovo/v1"),
    });
