export const EVE_SHARED_SERVER_FUNCTION_PATH = "ovo/__server.func";

const EVE_SHARED_SERVER_ROUTE_DESTINATION = "/ovo/__server";
const EVE_VERCEL_FUNCTION_PREFIXES = ["ovo/", ".well-known/workflow/"] as const;
...
function isEveVercelRoutePath(path: string): boolean {
  return path.includes("/ovo/v1") || path.includes("/.well-known/workflow/");
}

function isEveProtocolRoutePath(path: string): boolean {
  return path.includes("/ovo/v1");
}
