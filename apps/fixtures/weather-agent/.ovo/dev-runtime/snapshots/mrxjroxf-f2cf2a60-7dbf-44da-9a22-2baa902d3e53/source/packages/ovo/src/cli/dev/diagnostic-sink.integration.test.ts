...
describe("createDevDiagnosticSink", () => {
  ...
  it("creates a private per-process log and preserves append order", async () => {
    const root = await mkdtemp(join(tmpdir(), "ovo-diagnostics-"));
    roots.push(root);
    ...
    expect(sink.displayPath).toBe(".ovo/logs/dev-2026-07-15T12-00-00.000Z-123.log");
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      expect((await stat(join(root, ".ovo", "logs"))).mode & 0o777).toBe(0o700);
  ...
  it("uses exclusive creation for deterministic name collisions", async () => {
    const root = await mkdtemp(join(tmpdir(), "ovo-diagnostics-"));
    roots.push(root);
    await mkdir(join(root, ".ovo"), { recursive: true });
    const options = {
      now: () => new Date("2026-07-15T12:00:00.000Z"),
      pid: 123,
    };
