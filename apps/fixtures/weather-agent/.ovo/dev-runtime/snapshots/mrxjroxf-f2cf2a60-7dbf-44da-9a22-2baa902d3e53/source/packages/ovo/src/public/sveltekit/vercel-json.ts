...
/**
 * Ensure `vercel.json` declares the SvelteKit web service and the ovo agent
 * service so a Vercel deployment ships both from one project.
 *
 * Existing services are preserved untouched; an already-configured ovo
...
export async function ensureEveVercelJson(input: {
  ...
  const configuredEveService = findServiceByFramework(existingServices, "ovo");
