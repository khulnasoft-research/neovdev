...
const MOUNTED_EXTENSION = Symbol.for("ovo.mounted-extension");

const CONFIG_REGISTRY = Symbol.for("ovo.extension-config-registry");
 ...
 */
const EXT_CONFIG_SCOPE = Symbol.for("ovo.ext-config-scope");
...
export interface ExtensionHandle<S extends StandardSchemaV1 = StandardSchemaV1> {
  ...
  /** The declared config schema; read by `ovo extension build`. */
  readonly schema: S;
}
...
/**
 * Declares an ovo extension. Optionally takes a `config` schema — any Standard
