...
development it starts ovo on a random available port and rewrites same-origin
ovo endpoints like `/ovo/v1/session` to that server.
...
the default app, while Vercel routes public ovo endpoints directly to the ovo
service before filesystem routing.

For non-Vercel production hosts, set `EVE_NEXT_PRODUCTION_ORIGIN` to the public
origin that serves the ovo service namespace before building the Next.js app.
For local production builds, `withEve()` uses `http://127.0.0.1:4274` as the
stable ovo origin. Set `EVE_NEXT_PRODUCTION_PORT` before `next build` and
