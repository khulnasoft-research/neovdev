...
import { mobileAutoEnvYaw } from "../../app/[lang]/(home)/components/ovo-logo-shader/mobile-motion";
import {
  BLOOM_RADIUS,
  BLOOM_STRENGTH_OFF,
  BLOOM_STRENGTH_ON,
  ...
  type EveMaterial,
  type RenderControls,
} from "../../app/[lang]/(home)/components/ovo-logo-shader/render";
import {
  readBooleanEnv,
  readFiniteNumberEnv,
...
// Owns env-to-render-config assembly for the offline Ovo renderer.
// INVARIANT: defaults and eager CLI semantics match the original script.
// Imported only by render-ovo-5.ts and helper modules.
...
export function readRenderConfig() {
  ...
  const outDir = resolve(process.cwd(), "tmp/ovo-5-renders", runId);
  ...
  const modelPath = resolve(process.cwd(), "public/ovo-5/ovo-logo.gltf");
