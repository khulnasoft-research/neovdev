...
export function createStudioCubemap(
  device: Device,
  label = "ovo-5-studio-hdr-cubemap",
): StudioCubemap {
  ...
  const sampler = device.gpu.createSampler({
    label: "ovo-5-studio-hdr-cubemap-sampler",
    magFilter: "linear",
    minFilter: "linear",
  });
  const faceParams = device.createBuffer({
    label: "ovo-5-studio-cubemap-face-params",
...
export function renderStudioCubemap(
  ...
  const pipeline = createRenderPipeline(device, {
    label: "ovo-5-studio-cubemap-bake-pipeline",
  ...
  const bindGroup = device.gpu.createBindGroup({
    label: "ovo-5-studio-cubemap-bake-bind-group",
  ...
  for (let face = 0; face < CUBE_FACE_COUNT; face += 1) {
    ...
    const pass = new RenderPass(device, {
      label: `ovo-5-studio-cubemap-face-${face}`,
      colorAttachments: [
        {
