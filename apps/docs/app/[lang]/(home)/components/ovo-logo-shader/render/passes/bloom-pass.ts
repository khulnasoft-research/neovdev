...
export function renderBlur(
  ...
  const bindGroup = device.gpu.createBindGroup({
    label: "ovo-5-bloom-blur-bind-group",
  ...
  const pass = new RenderPass(device, {
    label: `ovo-5-bloom-${direction[0] > 0 ? "horizontal" : "vertical"}-pass`,
    colorAttachments: [
...
export function renderComposite(
  ...
  const bindGroup = device.gpu.createBindGroup({
    label: "ovo-5-bloom-composite-bind-group",
  ...
  const pass = new RenderPass(device, {
    label: "ovo-5-composite-tonemap-pass",
