...
export function createBloomTargetCache(device: Device) {
  ...
  return {
      ...
      targets = {
        ...
        paddingRadius,
        scene: createBloomTexture(device, "ovo-5-scene-linear-hdr", padded.width, padded.height),
        backMaterial: createBloomTexture(
          device,
          "ovo-5-back-material-linear-hdr",
          padded.width,
          padded.height,
        ),
        backDepth: createBloomTexture(
          device,
          "ovo-5-back-camera-axis-depth",
          padded.width,
          padded.height,
        ),
        backSurfaceDepth: createBackDepthTexture(
          device,
          "ovo-5-back-surface-depth",
          padded.width,
          padded.height,
        ),
        horizontal: createBloomTexture(
          device,
          "ovo-5-bloom-horizontal",
          padded.width,
          padded.height,
        ),
        vertical: createBloomTexture(device, "ovo-5-bloom-vertical", padded.width, padded.height),
      };
      return targets;
    },
    dispose,
