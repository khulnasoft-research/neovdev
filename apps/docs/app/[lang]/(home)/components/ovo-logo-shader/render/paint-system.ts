...
export function createPaintSystem(device: Device, resources: RendererResources, mesh: MeshData) {
  ...
  const ensure = (logicalWidth: number, logicalHeight: number, gridScaleMultiplier?: number) => {
    ...
    dispose();
    const ping = createPaintTexture(device, "ovo-5-paint-ping", cols, rows);
    const pong = createPaintTexture(device, "ovo-5-paint-pong", cols, rows);
    const staticNoise = createPaintStaticNoiseTexture(
      device,
      "ovo-5-paint-static-noise",
      cols,
      rows,
    );
    const voronoiValue = createVoronoiNoiseTexture(device, "ovo-5-ascii-voronoi-value", cols, rows);
    const voronoiEdge = createVoronoiNoiseTexture(device, "ovo-5-ascii-voronoi-edge", cols, rows);
  ...
  const renderVoronoiNoise = (
    ...
    const pass = new RenderPass(device, {
      label: "ovo-5-ascii-voronoi-noise-pass",
      colorAttachments: [
        {
