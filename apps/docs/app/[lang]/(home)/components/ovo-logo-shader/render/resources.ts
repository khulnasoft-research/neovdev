// Owns long-lived GPU resources for one Ovo renderer instance.
...
export function createResources(
  ...
  const studioCubemap = createStudioCubemap(device, "ovo-5-studio-hdr-cubemap");
  ...
  const previewSampler = device.gpu.createSampler({
    label: "ovo-5-render-target-preview-sampler",
    magFilter: "linear",
    minFilter: "linear",
    addressModeU: "clamp-to-edge",
    addressModeV: "clamp-to-edge",
  ...
  const previewMode = device.createBuffer({
    label: "ovo-5-render-target-preview-mode",
    size: 16,
    usage: ["uniform", "copy_dst"],
  });
